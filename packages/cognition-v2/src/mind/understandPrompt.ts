...
function extractDates(prompt: string): string[] {
  return prompt.match(
    /\b(?:\d{1,2}[/-]\d{1,2}[/-]\d{2,4}|(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday))\b/gi
  ) ?? [];
}
...
function buildCognitiveWorld(
  ...
  const cognitivePlaces = places.map(text => ({
    text,
    kind: "mentioned"
  }));
  ...
  const narrative = {
    hasBeginning:
      memory.past ||
      domains.includes("memory") ||
      dates.length > 0,

    hasTransformation:
      intent.includes("create") ||
      intent.includes("discover"),

    hasRelationship:
      intent.includes("connect") ||
      audience.types.includes("couple"),

    hasMemory:
      memory.past ||
      intent.includes("remember"),

    hasConflict:
      emotions.includes("fear"),

    hasMilestone:
      intent.includes("celebrate"),
...
export function understandPrompt(
  ...
  const people =
    extractPeople(expression);

  const places =
    extractPlaces(expression);

  const objects =
    extractObjects(expression);

  const dates =
    extractDates(expression);

  const times =
    extractTimes(expression);

  const cognitiveWorld =
    buildCognitiveWorld(
      expression,
      intent,
      people,
