...
export type WriterMove =
  | "observer" | "trickster" | "poet" | "director" | "historian" | "comic" | "suspense" | "tender" | "reversal" | "understatement"
  | "detail-first" | "place-first" | "time-first" | "consequence-first" | "implication-first" | "reframing";

export type WriterDraft = { text: string; move: WriterMove; details: string[] };

const clean = (value: string) => value.replace(/\s+/g, " ").trim().replace(/[.!?]+$/, "");
const lower = (value: string) => clean(value).toLowerCase();

function dominantObject(event: WorldEvent): string | undefined { return event.object || event.details.find((detail) => detail.length >= 3); }
...
function movePool(lens: CognitiveLens, event: WorldEvent): WriterMove[] {
  if (lens === "comedy") return ["trickster", "comic", "reframing", "understatement", "detail-first"];
  if (lens === "horror") return ["suspense", "observer", "reversal", "reframing", "implication-first"];
  if (lens === "romance") return ["tender", "historian", "poet", "reframing", "detail-first"];
  if (lens === "mysterious") return ["suspense", "observer", "reversal", "reframing", "place-first"];
  if (lens === "wild") return ["trickster", "comic", "director", "reversal", "consequence-first"];
  if (event.details.length >= 2) return ["observer", "poet", "historian", "reframing", "detail-first"];
  return ["observer", "poet", "historian", "understatement", "reframing"];
}

function entryVariants(event: WorldEvent, lens: CognitiveLens): WriterDraft[] {
  const raw = clean(event.raw); const object = dominantObject(event); const place = event.place; const time = event.time;
  const detail = event.details.find((value) => lower(value) !== lower(object ?? "")); const drafts: WriterDraft[] = [];
...
function semanticReframe(event: WorldEvent, kernel: string | undefined, lens: CognitiveLens): WriterDraft[] {
  const raw = clean(event.raw); const object = dominantObject(event); const noun = object || event.place || event.details[0];
  if (!noun) return [];
  const drafts: WriterDraft[] = [];
  const add = (text: string, ...details: string[]) => drafts.push({ text: `${clean(text)}.`, move: "reframing", details });
  switch (kernel) {
    case "ornament": add(`${noun} was supposed to finish the look; instead, it became the detail that finished the story`, "function inversion", "ornament-as-payoff"); break;
    case "memory-artifact": add(`${noun} was keeping a different kind of time: the kind made of people and places`, "memory metaphor", "double meaning"); break;
    case "voice": add(`${noun} was not just making sound; it was giving the moment a voice of its own`, "object-to-voice metaphor"); break;
    case "witness": add(`${noun} started as a recorder and ended up as a witness`, "role transformation", "media personification"); break;
    case "talisman": add(`${noun} had a practical job until the story quietly assigned it a second one`, "double function", "symbolic elevation"); break;
    case "traveler": add(`${noun} carried the route; the people around it supplied the reason for going`, "journey reframing", "human context"); break;
    case "threshold": add(`${noun} was ordinary architecture until the moment gave it something to separate`, "threshold metaphor", "state transition"); break;
    case "stage": add(`${noun} did not change, but it started behaving like a stage once the details took over`, "setting personification", "scene framing"); break;
    case "atmosphere": add(`${noun} was not weather anymore; it was part of the meaning`, "atmosphere-as-meaning", "sensory interpretation"); break;
    default: add(`${noun} became more interesting once it stopped being only what it was`, "semantic reinterpretation");
  }
  if (lens === "comedy") add(`${noun} had apparently accepted a second job as the reason this got memorable`, "comic reinterpretation");
  if (lens === "horror") add(`${noun} remained exactly what it was, which was suddenly not reassuring`, "horror reinterpretation");
  if (lens === "romance") add(`${noun} was ordinary enough to overlook and specific enough to keep`, "romantic reinterpretation");
  return drafts;
}

function writerDrafts(event: WorldEvent, world: WorldModel, previous?: WorldEvent, next?: WorldEvent): WriterDraft[] {
  const raw = clean(event.raw); const noun = contextualNoun(event); const object = dominantObject(event); const kernel = objectKernel(object);
  const subject = event.participants.join(" and ") || event.place || object || "the moment";
  const moves = movePool(world.lens, event); const drafts: WriterDraft[] = [...entryVariants(event, world.lens), ...semanticReframe(event, kernel, world.lens)];
  const add = (text: string, move: WriterMove, ...details: string[]) => { const value = clean(text); if (value && lower(value) !== lower(raw)) drafts.push({ text: `${value}.`, move, details }); };
...
export function generateWriterDrafts(world: WorldModel, event: WorldEvent, previous?: WorldEvent, next?: WorldEvent): WriterDraft[] { return writerDrafts(event, world, previous, next); }
