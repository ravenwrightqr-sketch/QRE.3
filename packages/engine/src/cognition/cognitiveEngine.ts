...
const STOP = new Set([
  ...
  "want", "need", "give", "get", "tell", "show", "i",
]);
...
function extractEntities(prompt: string, context: StoryCompilerContext): ExperienceEntities {
  ...
  const products = unique(lower.match(/\b(qr|nfc|tag|keychain|sticker|card|poster|shirt|book|product|watch|gift|surfboard|vehicle|guitar|artwork|jewelry)\b/g) ?? []);
  const places = unique([
...
function inferSubject(prompt: string, entities: ExperienceEntities): CognitiveClaim<string> {
  ...
  const observed = Boolean(possessive || direct || entities.people[0] || entities.products[0] || entities.events[0]);
  return {
    value,
    status: observed ? "observed" : "derived",
    confidence: possessive || direct ? 0.95 : observed ? 0.8 : 0.65,
    evidence: [promptEvidence(`subject candidate: ${value}`, possessive || direct ? 0.95 : observed ? 0.8 : 0.65)],
  };
}
...
function inferParticipants(prompt: string, context: StoryCompilerContext): CognitiveClaim<string[]> {
  ...
  return {
    ...
    confidence: result.length ? 0.8 : 0,
    evidence: result.length ? [promptEvidence(`participants: ${result.join(", ")}`, 0.8)] : [],
  };
}
...
function opportunities(prompt: string, entities: ExperienceEntities) {
  ...
  return {
    ...
    geographic: unique([
      /\b(place|venue|restaurant|bar|shop|hotel|beach|park|where|location|near|travel|journey|route|wave)\b/.test(lower) ? "connect the experience to meaningful places and routes" : "",
    ]),
    social: unique([
    ...
    temporal: unique([
      /\b(today|tonight|live|event|return|again|future|anniversary|milestone|next|evolving|growing)\b/.test(lower) ? "change behavior across time, repeat participation, or milestones" : "",
    ]),
    commercial: unique([
...
function hypothesis(kind: ExperienceHypothesisKind, premise: string, rationale: string, evidence: CognitiveEvidence[], dimensions: ExperienceHypothesis["dimensions"]): ExperienceHypothesis {
  const score = clamp(
    dimensions.subjectFit * 0.2 +
      dimensions.emotionalResonance * 0.15 +
      dimensions.interactionNaturalness * 0.15 +
      dimensions.memoryPotential * 0.1 +
      dimensions.discoveryPotential * 0.1 +
      dimensions.socialPotential * 0.08 +
      dimensions.temporalPotential * 0.07 +
      dimensions.commercialPotential * 0.05 +
      dimensions.novelty * 0.05 +
      dimensions.feasibility * 0.05,
  );
...
export function understandExperience(prompt: string, context: StoryCompilerContext = {}): CognitiveExperienceState {
  ...
  const base = {
    subjectFit: 0.8,
    emotionalResonance: emotionalIntent.length ? 0.85 : 0.55,
    interactionNaturalness: 0.8,
    memoryPotential: opportunity.memory.length ? 0.9 : 0.35,
    discoveryPotential: opportunity.discovery.length ? 0.9 : 0.4,
    socialPotential: opportunity.social.length ? 0.85 : 0.3,
    temporalPotential: opportunity.temporal.length ? 0.8 : 0.35,
    commercialPotential: opportunity.commercial.length ? 0.75 : 0.2,
    novelty: 0.7,
    feasibility: 0.9,
  };
  ...
  if (!context.location && opportunity.geographic.length === 0) assumptions.push({ statement: "No geographic behavior is required initially.", reason: "The prompt did not establish a meaningful place dependency.", confidence: 0.8 });

  return {
    prompt: text,
    subject,
    participants,
    motivations: { value: motivations, status: motivations.length ? "derived" : "unknown", confidence: 0.75, evidence },
    entities,
    affordances,
    emotionalIntent,
