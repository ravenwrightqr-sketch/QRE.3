...
function extractNames(text: string): string[] {
  return unique([...text.matchAll(/\b[A-Z][A-Za-z'’-]*(?:\s+[A-Z][A-Za-z'’-]*)?\b/g)].map((m) => m[0]).filter((v) => !/^(?:I|We|The|Then|At|And|My|Our|This|A|An|By|Friday|Saturday|Sunday|Monday|Tuesday|Wednesday|Thursday)$/i.test(v)));
}
...
function deriveEntities(events: RealityEvent[]): ExperienceEntities {
  const people = unique(events.flatMap((e) => e.actor ?? "").filter((v) => /^[A-Z][A-Za-z'’-]*(?:\s+[A-Z][A-Za-z'’-]*)?$/.test(v)));
...
function planFor(world: World): CognitiveExperiencePlan {
  ...
  for (const event of world.events) {
    if (event.actor && event.place) relations.push({ from: "subject", to: "place", relation: "experienced_at", confidence: 1, evidence: promptEvidence(event.raw) });
    if (event.actor && event.object) relations.push({ from: "subject", to: "artifact", relation: "interacted_with", confidence: 1, evidence: promptEvidence(event.raw) });
  }
  const premise: CognitivePremise = { slots, relations };
  const subject = world.events.find((e) => e.actor)?.actor ?? world.entities[0] ?? "the moment";
...
function meaningFor(subject: string, participants: string[], world: World): ExperienceMeaning {
  return {
    ...
    relationship: participants.length ? { subject, object: participants[0]!, type: "shared_experience" } : undefined,
    emotions: [world.lens],
...
function anchorCoverage(candidate: string, event: RealityEvent): number {
  ...
  let score = 0;
  for (const item of required) if (lower(candidate).includes(lower(item))) score += 12; else score -= 40;
  return score;
}
...
function pickCandidate(event: RealityEvent, world: World, used: Set<string>): string {
  const ranked = creativeCandidates(event, world)
    ...
    .map((candidate) => {
      ...
      const transformed = lower(candidate.text) !== lower(event.raw) ? 7 : 0;
  ...
  return sentence(ranked[0]?.text ?? event.raw);
}
...
function directive(event: RealityEvent, kind: StoryBeatKind): CognitiveBeatDirective {
  return {
    ...
    relationalFocus: [event.object, event.place, event.time].filter(Boolean) as string[],
...
function titleFor(world: World): string {
  const subject = world.events.find((e) => e.actor)?.actor ?? world.entities[0] ?? "This Experience";
  const place = world.places[0];
  return place ? `${subject} at ${place}` : `${subject}: What Happened`;
}

function subjectFor(world: World): string { return world.events.find((e) => e.actor)?.actor ?? world.entities[0] ?? "the experience"; }
...
export function compileCognitiveExperience(prompt: string, context: UniversalMindContext = {}): UniversalMindResult {
  ...
  const participants = unique([...built.events.map((e) => e.actor ?? ""), ...(context.event?.participants ?? [])]);
  const world: World = {
    prompt,
    ...
    events: built.events,
    relations: built.events.flatMap((e) => e.actor && e.object ? [{ from: e.actor, relation: "interacted_with", to: e.object, evidenceId: e.id }] : []),
  ...
  const beats: StoryBeat[] = world.events.map((event, index) => {
    ...
    return {
      ...
      entities: unique([event.actor, event.object, event.place].filter(Boolean) as string[]),
  ...
  if (beats.length === 1 && world.events[0]?.raw && lower(beats[0].text) === lower(world.events[0].raw)) {
    const second = `The detail was worth keeping: ${sentence(world.events[0].raw)}.`;
  ...
  const moments: ExperienceMoment[] = beats.map((beat, index) => {
    ...
    return {
      ...
      subtitle: index === 0 ? subject : undefined,
      text: beat.text,
      description: beat.text,
      editable: true,
      demo: false,
      order: index,
      payload: { source: "universal-mind", realityEventId: event?.id, evidence: event?.evidence.map((e) => e.detail) ?? [] },
