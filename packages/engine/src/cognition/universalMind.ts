...
function splitSemanticClauses(value: string): string[] {
  ...
  for (const match of text.matchAll(joiner)) {
    ...
    // Never split a coordinated subject such as "Alex and Sam went...".
  ...
  for (const item of sentences) {
    for (const clause of splitSemanticClauses(item)) {
      ...
      for (const part of parts) {
        ...
        const isNewEvent = Boolean(current) && Boolean(semantic) && Boolean(currentSemantic);
        if (isNewEvent && /^(?:then|but)\b/i.test(part)) {
          output.push(current);
          current = part.replace(/^(?:then|but)\s+/i, "");
        } else if (isNewEvent && /^and\s+/i.test(part)) {
          output.push(current);
...
function placeOf(text: string): string | undefined {
  ...
  if (named) {
    ...
    if (PLACE_RE.test(last)) return tokens.slice(Math.max(0, tokens.length - 2)).join(" ");
...
function participantsOf(text: string, fallback: string[] = []): string[] {
  ...
  const explicitPair = prefix.match(/^([A-Z][A-Za-z'’-]*)\s+(?:and|&)\s+([A-Z][A-Za-z'’-]*)$/);
  return explicitPair ? unique([explicitPair[1]!, explicitPair[2]!]) : names.length ? names : fallback;
}
...
function buildEvents(prompt: string, context?: UniversalMindContext) {
  ...
  let carryParticipants: string[] = unique(context?.event?.participants ?? []);
  let carryPlace: string | undefined = context?.location?.label ?? context?.event?.venue;
  chunks.forEach((raw, index) => {
    ...
    const evidenceItems: Evidence[] = [evidence(`event-${index}-raw`, raw, action ? "event" : "history", action || state ? 0.95 : 0.8, resolved.place ? "memory" : "prompt")];
    for (const participant of participants) evidenceItems.push(evidence(`event-${index}-participant-${participant}`, participant, "entity", 1));
    if (place) evidenceItems.push(evidence(`event-${index}-place`, place, "place", 1, resolved.place ? "memory" : "prompt"));
    if (time) evidenceItems.push(evidence(`event-${index}-time`, time, "time", 1));
    if (object) evidenceItems.push(evidence(`event-${index}-object`, object, "entity", 0.95));
    if (state) evidenceItems.push(evidence(`event-${index}-state`, state, "state", 0.8));
    events.push({ id: `event-${index + 1}`, raw, participants, action, state, object, place, time, order: index, evidence: evidenceItems, resolvedFromMemory: Boolean(resolved.place) });
  });
...
function deriveEntities(events: RealityEvent[]): ExperienceEntities {
  ...
  const dates = unique(events.map((event) => event.time ?? "").filter((value) => /\d{4}|(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)/i.test(value)));
  ...
  const keywords = unique(events.flatMap((event) => `${event.raw} ${event.state ?? ""}`.split(/[^A-Za-z0-9'’-]+/).filter((word) => word.length >= 5))).slice(0, 100);
...
function premiseFor(world: World): CognitivePremise {
  ...
  const add = (role: CognitivePremise["slots"][number]["role"], values: string[], salience: number) => { const cleanValues = unique(values); if (!cleanValues.length) return; slots.push({ role, values: cleanValues, status: "observed", confidence: 1, salience, evidence: cleanValues.map((value) => ({ source: "prompt", detail: value, confidence: 1 })) }); };
  add("subject", world.participants.slice(0, 3), 1); add("participants", world.participants, 1); add("event", world.events.map((event) => event.action ?? event.raw), 0.95); add("artifact", world.events.map((event) => event.object ?? ""), 0.9); add("place", world.places, 1); add("temporal", world.times, 1); add("emotion", world.events.map((event) => event.state ?? ""), 0.7);
  const relations: CognitivePremise["relations"] = [];
  for (const event of world.events) for (const participant of event.participants) { if (event.place) relations.push({ from: "participants", to: "place", relation: "experienced_at", confidence: 1, evidence: [{ source: "prompt", detail: event.raw, confidence: 1 }] }); if (event.object) relations.push({ from: "participants", to: "artifact", relation: "interacted_with", confidence: 1, evidence: [{ source: "prompt", detail: event.raw, confidence: 1 }] }); }
  return { slots, relations };
}
function chooseType(prompt: string): ExperienceType { const text = lower(prompt); if (/\b(?:ticket|concert|rave|festival|wedding|birthday|conference|event|party|ceremony|convention)\b/.test(text)) return "event"; if (/\b(?:memory|remember|grandma|grandfather|family|years ago|milestone|anniversary)\b/.test(text)) return "memory"; if (/\b(?:collection|collectible|card|watch|coin|sneaker|guitar|artwork)\b/.test(text)) return "collection"; return "story"; }
...
function participantPhrase(event: RealityEvent): string { return event.participants.length > 1 ? event.participants.join(" and ") : event.participants[0] ?? ""; }
function faithfulText(event: RealityEvent): string { const raw = sentence(event.raw); if (!event.participants.length) return raw; return event.participants.some((p) => !lower(raw).includes(lower(p))) ? `${participantPhrase(event)} ${raw}` : raw; }
function creativeCandidates(event: RealityEvent, world: World): { text: string; creativity: number }[] {
  const faithful = faithfulText(event); const subject = participantPhrase(event); const candidates = [{ text: faithful, creativity: 0 }];
  if (world.lens === "comedy") { if (subject && /\b(?:arrived|entered|came|walked)\b/i.test(event.action ?? "")) candidates.push({ text: `${subject} arrived like legal counsel was already on the way.`, creativity: 9 }); if (subject && event.object && /\b(?:stole|chewed|broke|tore|ate)\b/i.test(event.action ?? "")) candidates.push({ text: `${subject} took ${event.object} with the confidence of someone who had already made the decision.`, creativity: 8 }); if (subject && /\b(?:groomed|cleaned|repaired|restored|transformed)\b/i.test(event.action ?? "")) candidates.push({ text: `${subject} started with one version of the day and left with another.`, creativity: 7 }); }
  if (world.lens === "romance" && subject && event.place && RETURN_RE.test(event.raw)) candidates.push({ text: `${subject} were back at ${event.place}, where the story had started.`, creativity: 9 });
  ...
  if (world.lens === "wild" && subject) candidates.push({ text: `${subject} did not ease into the moment. They arrived in it at full speed.`, creativity: 8 });
  return candidates;
}
function coverage(text: string, event: RealityEvent): number { const anchors = [...event.participants, event.object, event.place, event.time].filter(Boolean) as string[]; return anchors.reduce((score, anchor) => score + (lower(text).includes(lower(anchor)) ? 18 : -70), 0); }
function pick(event: RealityEvent, world: World, used: Set<string>): string { return creativeCandidates(event, world).filter((candidate) => !LEAK_RE.test(candidate.text) && !GENERIC_RE.test(candidate.text)).map((candidate) => ({ ...candidate, score: candidate.creativity + coverage(candidate.text, event) + (used.has(lower(candidate.text)) ? -100 : 0) })).sort((a, b) => b.score - a.score || a.text.length - b.text.length)[0]?.text ?? faithfulText(event); }
function beatKind(index: number, total: number): StoryBeatKind { if (total <= 1) return "payoff"; if (index === 0) return "orientation"; if (index === total - 1) return "payoff"; if (index === total - 2) return "transformation"; return index % 2 === 0 ? "discovery" : "escalation"; }
function directive(event: RealityEvent, kind: StoryBeatKind): CognitiveBeatDirective { return { kind, intent: "perform the strongest truthful change or meaningful detail supported by the event", subject: participantPhrase(event), action: event.action ?? event.state ?? "", stateBefore: "", stateAfter: event.state ?? "", relationalFocus: [...event.participants, event.object, event.place, event.time].filter(Boolean) as string[], evidence: event.evidence, confidence: 1 }; }
function planFor(world: World): CognitiveExperiencePlan { const premise = premiseFor(world); return { direction: world.events.length > 1 ? "story" : "memory", centralSubject: world.participants[0] ?? world.entities[0] ?? "the experience", audience: [], whyInteract: ["experience the supplied reality rather than read a report"], emotionalIntent: [world.lens === "neutral" ? "memorable" : world.lens], purpose: "turn observed reality into a coherent experience", interactionModel: ["open or scan and play sequentially"], storyStructure: ["reality", "attention", "change", "consequence", "payoff"], memoryModel: ["preserve observed evidence", "connect new events to prior context", "leave room for continuation"], geographicModel: world.places, socialModel: world.participants, discoveryModel: ["repetition", "return", "relationships", "unusual details"], rewardModel: [], commerceModel: [], progressionModel: ["each new event can change what becomes meaningful next"], contentModel: world.entities, dynamicBehavior: ["resolve known memory before asking", "preserve identity independently of grammar", "adapt creative performance to context"], futureEvolution: ["new events can extend the same world", "accepted and rejected creative preferences can influence later performance"], creativePossibilities: ["contrast", "personification", "understatement", "escalation", "callback", "reveal", "earned payoff"], premise, realization: { direction: world.events.length > 1 ? "story" : "memory", directives: world.events.map((event, index) => directive(event, beatKind(index, world.events.length))), semanticArc: world.events.map((event) => event.raw), conservedRoles: premise.slots.map((slot) => slot.role), confidence: 1 } as CognitiveExperienceRealization }; }
function meaningFor(world: World): ExperienceMeaning { const participants = world.participants; const subject = participants[0] ?? world.entities[0] ?? "the experience"; return { why: "Turn observed reality into an experience worth remembering and returning to.", relationship: participants.length > 1 ? { subject: participants[0]!, object: participants[1]!, type: "shared_experience" } : undefined, emotions: [world.lens], memories: ["persistent", "continuation"], desiredFeeling: [world.lens === "neutral" ? "memorable" : world.lens], transformation: world.events.length > 1 ? "separate facts become a connected experience" : `a supplied reality becomes an experience about ${subject}` }; }
function titleFor(world: World): string { const subject = world.participants.length > 1 ? world.participants.join(" + ") : world.participants[0] ?? world.entities[0] ?? "This Experience"; return world.places[0] ? `${subject} at ${world.places[0]}` : `${subject}: What Happened`; }
function makeMoment(event: RealityEvent, text: string, index: number, total: number, world: World): ExperienceMoment { const type: ExperienceMoment["type"] = index === 0 ? "introduction" : index === total - 1 ? "completion" : "story"; return { type, component: "story", title: index === 0 ? titleFor(world) : undefined, subtitle: index === 0 ? participantPhrase(event) || undefined : undefined, text: `${sentence(text)}.`, description: `${sentence(text)}.`, editable: true, demo: false, order: index, payload: { source: "universal-mind", realityEventId: event.id, participants: event.participants, evidence: event.evidence.map((item) => item.detail), place: event.place, time: event.time }, meta: { source: "universal-mind", lens: world.lens, realityEventId: event.id, place: event.place, time: event.time, duration: index === total - 1 ? 5200 : 3600 } }; }
...
export function compileCognitiveExperience(prompt: string, context: UniversalMindContext = {}): UniversalMindResult {
  const built = buildEvents(prompt, context); const lens = lensOf(prompt, context); const participants = unique([...built.events.flatMap((event) => event.participants), ...(context.event?.participants ?? [])]); const entities = deriveEntities(built.events);
  const world: World = { prompt, entities: unique([...entities.people, ...entities.places, ...entities.products]), participants, places: entities.places, times: entities.times, events: built.events, relations: built.events.flatMap((event) => event.participants.flatMap((participant, index) => [ ...(event.place ? [{ from: participant, relation: "experienced_at", to: event.place, evidenceId: event.id }] : []), ...(event.object ? [{ from: participant, relation: "interacted_with", to: event.object, evidenceId: event.id }] : []), ...event.participants.slice(index + 1).map((other) => ({ from: participant, relation: "shared_event", to: other, evidenceId: event.id })) ])), lens, memoryMatches: built.memoryMatches };
  const plan = planFor(world); const used = new Set<string>(); const beats: StoryBeat[] = world.events.map((event, index) => { const text = pick(event, world, used); used.add(lower(text)); return { id: `mind-${event.id}`, kind: beatKind(index, world.events.length), order: index, purpose: "perform reality with attention and creative fidelity", text: `${sentence(text)}.`, emotionalTarget: lens, entities: unique([...event.participants, event.object, event.place].filter(Boolean) as string[]), provenance: [{ kind: "observed", source: event.resolvedFromMemory ? "memory" : "prompt", confidence: 1 }], directive: directive(event, beatKind(index, world.events.length)) }; });
  if (beats.length === 1) { const event = world.events[0]!; const payoff = event.object ? `${cap(event.object)} became part of the day.` : event.place ? `The moment stayed with ${event.place}.` : `The moment became part of the memory.`; if (lower(payoff) !== lower(beats[0]!.text)) beats.push({ ...beats[0]!, id: `${beats[0]!.id}-payoff`, order: 1, kind: "payoff", text: `${payoff}.` }); }
  const moments = beats.map((beat, index) => makeMoment(world.events[Math.min(index, world.events.length - 1)]!, beat.text, index, beats.length, world));
  const cinematicScenes: CinematicScene[] = moments.map((moment, index) => ({ id: `mind-scene-${index + 1}`, type: index === 0 ? "intro" : index === moments.length - 1 ? "emotion" : "action", duration: Number(moment.meta?.duration ?? 3600), moment, order: index, transition: index === 0 ? "none" : lens === "horror" ? (index % 2 ? "fade" : "flash") : lens === "romance" ? "cinematic" : lens === "wild" ? "zoom" : "fade", visual: lens === "horror" ? { theme: "dark", animation: "glitch" } : lens === "romance" ? { theme: "cinematic", animation: "slow_zoom" } : lens === "wild" ? { theme: "cinematic", animation: "particles" } : { theme: "cinematic", animation: index === 0 ? "slow_zoom" : "parallax" }, preload: index < moments.length - 1 }));
  ...
  const type = chooseType(prompt); const blueprint: ExperienceBlueprint = { title: titleFor(world), type, tone: tones(lens), meaning: meaningFor(world), moments, entities, cognitivePlan: plan, metadata: { archetypes: [type, lens, "universal_entity_experience"], themes: unique([...participants, ...world.places, ...world.times]).slice(0, 20), dna: ["reality-first", "memory-aware", "participant-preserving", "adaptive", "creative-hypothesis-search", "experience-moment-canonical"] } };
  const learningSignals = unique([...(context.feedback?.accepted ?? []).map((value) => `accepted:${value}`), ...(context.feedback?.rejected ?? []).map((value) => `rejected:${value}`), ...(context.creativePreferences ?? []).map((value) => `preference:${value}`)]);
  return { title: blueprint.title, blueprint, plan, flowSteps, moments, cinematicScenes, estimatedDuration: moments.reduce((sum, moment) => sum + Number(moment.meta?.duration ?? 3600), 0), momentCount: moments.length, world, adaptiveQuestions: built.adaptiveQuestions, discoveries: unique([...built.memoryMatches.map((match) => `This experience connects to ${match}.`), ...(RETURN_RE.test(prompt) && world.places[0] ? [`You returned to ${world.places[0]}.`] : []), ...(world.participants.length > 1 ? [`Shared experience between ${world.participants.join(" and ")}.`] : [])]), learningSignals };
}
