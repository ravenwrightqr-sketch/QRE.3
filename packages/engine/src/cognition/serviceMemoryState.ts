...
export function evolveRuntimeMemory(moments: ExperienceMoment[], prior?: MemorySnapshot | null): MemorySnapshot {
  ...
  const highlights = unique([
    ...(prior?.highlights ?? []),
    ...moments.filter((moment) => Array.isArray(moment.payload?.creativeDetails) && moment.payload.creativeDetails.length > 0).map(momentText),
    ...texts,
  ]).slice(-8);
  ...
  const summary = priorSummary ? `${priorSummary} ${texts[0] ?? ""}`.trim() : texts.slice(0, 3).join(" ");
  return {
