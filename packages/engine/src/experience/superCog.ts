...
export function compileSuperCogExperience(prompt: string): CompiledCognitiveExperience {
  ...
  const story = {
    ...
    continuation: result.story.continuation ? replaceSubject(result.story.continuation, oldSubject, subject) : undefined,
    beats: result.story.beats.map((beat) => ({ ...beat, text: replaceSubject(beat.text, oldSubject, subject), purpose: replaceSubject(beat.purpose, oldSubject, subject), entities: beat.entities.map((entity) => entity === oldSubject ? subject : entity) })),
  };
  ...
  const blueprint = {
    ...
    cognitivePlan: plan,
    moments: result.blueprint.moments.map((moment) => ({ ...moment, title: replaceSubject(moment.title, oldSubject, subject), subtitle: moment.subtitle ? replaceSubject(moment.subtitle, oldSubject, subject) : moment.subtitle, description: moment.description ? replaceSubject(moment.description, oldSubject, subject) : moment.description })),
  };
  ...
  return {
    ...
    scenePlan,
    cinematicScenes,
    model: { ...result.model, title: blueprint.title, moments: blueprint.moments },
