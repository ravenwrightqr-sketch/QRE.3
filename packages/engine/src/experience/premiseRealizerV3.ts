...
/**
 * Preserve the selected cognitive outcome as presentation evidence.
 *
 * A direction such as memory is a semantic commitment, not a decorative tag.
 * If a downstream language pass has retained the subject and medium but lost
 * the human outcome (for example "remember"), the story has become semantically
 * thinner even though its metadata still looks correct. This helper restores
 * only the directional outcome already selected by cognition.
 */
function semanticOutcome(plan?: CognitiveExperiencePlan): string {
...
function directiveText(
  ...
  switch (beat.kind) {
    case "orientation": return `${cap(subject)} establishes the experience by ${action}.`;
    case "hook": return `${cap(subject)} gives the interaction a reason to continue: ${action}.`;
    case "need": return `${cap(subject)} begins with the immediate need: ${action}.`;
    case "threshold": return `${cap(subject)} crosses from the known into the next layer by ${action}.`;
    case "origin": return `${cap(subject)} brings available history into the present: ${action}.`;
    case "encounter": return `${cap(subject)} brings the next relationship into the experience: ${action}.`;
    case "challenge": return `${cap(subject)} presents a condition that must be resolved: ${action}.`;
    case "discovery": return `${cap(subject)} makes the next relationship meaningful by ${action}.`;
    case "reveal": return `${cap(subject)} reveals what the evidence supports: ${action}.`;
    case "instruction": return `${cap(subject)} provides the useful next move: ${action}.`;
    case "action": return `Act on ${subject}: ${action}.`;
    case "feedback": return `The result becomes evidence for ${subject}: ${action}.`;
    case "contribution": return `${cap(subject)} changes when participation becomes contribution: ${action}.`;
    case "escalation": return `${cap(subject)} increases the consequence of what happened before: ${action}.`;
    case "transformation": return `${cap(subject)} changes through the accumulated interaction: ${action}.`;
    case "reflection": return `${cap(subject)} retains what the interaction means now: ${action}.`;
    case "provenance": return `${cap(subject)} carries the available evidence: ${action}.`;
    case "identity": return `${cap(subject)} becomes an identity-bearing subject through ${action}.`;
    case "milestone": return `${cap(subject)} reaches a meaningful state: ${action}.`;
    case "unlock": return `${cap(subject)} opens the next state through ${action}.`;
    case "earned_access": return `${cap(subject)} earns the next state through ${action}.`;
    case "payoff": return `${cap(subject)} reaches the intended result: ${action}.`;
    case "next_step": return `${cap(subject)} uses the current state to continue: ${action}.`;
    case "continuation": return `${cap(subject)} remains open to what comes next: ${action}.`;
    default: return `${cap(subject)} advances the experience through ${action}.`;
  }
}
...
function fallbackText(beat: StoryBeat, plan?: CognitiveExperiencePlan): string {
  ...
  switch (beat.kind) {
    ...
    case "action":
      return detail ? `Act on ${subject}: ${detail}.` : `${cap(subject)} becomes the next concrete action.`;
    case "feedback":
      return detail
...
function preserveSemanticOutcome(
  text: string,
  beat: StoryBeat,
  plan?: CognitiveExperiencePlan,
): string {
  const outcome = semanticOutcome(plan);
  if (!outcome || lower(text).includes(lower(outcome))) return text;

  // Outcome language belongs where the narrative actually resolves or carries
  // forward. Keeping it out of every beat prevents repetitive, robotic prose.
  if (["hook", "payoff", "reflection", "continuation"].includes(beat.kind)) {
    const subject = first(plan, "subject") || plan?.centralSubject || beat.entities?.[0] || "the experience";
    if (outcome === "remember") {
      return `${sentence(text)} It is meant to be remembered.`;
    }
    if (outcome === "discover") {
      return `${sentence(text)} It gives people something new to discover.`;
    }
    if (outcome === "connect") {
      return `${sentence(text)} It gives people a reason to connect.`;
    }
    if (outcome === "play") {
      return `${sentence(text)} It gives people something to play with.`;
    }
    if (outcome === "return") {
      return `${sentence(text)} It gives people a reason to return.`;
    }
    if (outcome === "change") {
      return `${sentence(text)} It leaves ${subject} changed.`;
    }
  }
...
function preserveSemanticAction(text: string, beat: StoryBeat, plan?: CognitiveExperiencePlan): string {
  const directive = plan?.realization?.directives.find((candidate) => candidate.kind === beat.kind);
  const action = sentence(directive?.action);
...
export function realizePremiseBeatV3(beat: StoryBeat, plan?: CognitiveExperiencePlan): string {
  ...
  text = preserveSemanticOutcome(text, beat, plan);
