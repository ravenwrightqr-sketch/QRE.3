...
/**
 * ============================================================
 * SUPER COG STORY REALIZER
 * ============================================================
 *
 * Cognition has already chosen the semantic direction. This layer is the
 * final language realization pass: it turns that direction into a story
 * that inhabits the subject instead of narrating the compiler.
 *
 * PRINCIPLE:
 *   LEAN INTO THE PREMISE.
 *   LEAN INTO THE GENRE.
 *   LEAN INTO THE SUBJECT.
 *   LEAN INTO THE SENSORY / SOCIAL / TEMPORAL WORLD.
 *   LEAN INTO THE PAYOFF.
 *
 * This is intentionally not an industry-template registry. It uses the
 * subject and semantic plan already present in the beat, then selects prose
 * by story function, emotional intent, and observed vocabulary.
 */
...
function hasIntent(plan: CognitiveExperiencePlan | undefined, value: string): boolean {
  const text = [
    ...(plan?.emotionalIntent ?? []),
    ...(plan?.affordances ?? []),
    ...(plan?.creativePossibilities ?? []),
  ].join(" ");
  return lower(text).includes(value);
}
...
function storyBeat(
  ...
  const content = concretePlanLanguage(plan);
  ...
  if (plan?.direction === "utility") {
    switch (beat.kind) {
      ...
      case "instruction":
        return content
          ? `Here is the part that actually helps: ${content}.`
      ...
      case "feedback":
        return `Look at the result. What changed tells the story better than another paragraph of instructions.`;
      case "next_step":
        return future
          ? `From here, it can keep getting smarter: ${future}.`
          : `The next step follows from what just happened.`;
    }
  }
  ...
  if (plan?.direction === "game") {
    switch (beat.kind) {
      ...
      case "challenge":
        return progression
          ? `The challenge tightens the screws: ${progression}.`
          : `The easy part is over. The next move has to be earned.`;
      case "discovery":
        return detail
          ? `The clue points toward ${detail}. That changes the hunt.`
          : `The clue opens a door that was not visible a moment ago.`;
      case "escalation":
        return `Now it gets harder. The game has enough information to stop being polite about it.`;
      case "payoff":
        return reward
          ? `You earned it: ${reward}.`
          : `The payoff lands because the player actually had to get there.`;
    }
  }
  ...
  if (plan?.direction === "memory") {
    switch (beat.kind) {
      ...
      case "origin":
        return detail
          ? `It starts with ${detail}, then opens into everything that came after.`
          : `The first layer is where the story began.`;
      case "encounter":
        return detail
          ? `${cap(detail)} pulls another piece of ${subject} back into the room.`
          : `A remembered detail suddenly feels present again.`;
      case "reflection":
        return mood === "playful"
      ...
      case "continuation":
        return future
          ? `And it is not finished: ${future}.`
          : `There is room for another story the next time someone comes back.`;
    }
  }
  ...
  if (plan?.direction === "discovery") {
    switch (beat.kind) {
      ...
      case "reveal":
        return detail
          ? `Then ${detail} changes the picture.`
          : `The first reveal changes what you thought you were looking at.`;
      case "discovery":
        return content
          ? `The deeper layer is where it gets interesting: ${content}.`
          : `One discovery leads naturally to another.`;
      case "payoff":
        return `The reveal earns its place because you cannot look at ${subject} quite the same way afterward.`;
      case "continuation":
        return future
          ? `There is still more beyond this layer: ${future}.`
          : `The door stays open for another discovery.`;
    }
  }
  ...
  if (plan?.direction === "social") {
    switch (beat.kind) {
      ...
      case "continuation":
        return future
          ? `And it keeps moving: ${future}.`
          : `The next person gets to change it again.`;
    }
  }
  ...
  if (plan?.direction === "commerce") {
    switch (beat.kind) {
      ...
      case "discovery":
        return detail
          ? `${cap(detail)} becomes part of the reason to come back.`
          : `The experience gives the relationship another layer.`;
      case "payoff":
        return reward
          ? `The return feels earned: ${reward}.`
          : `The reason to return comes from what happened here.`;
      case "continuation":
        return future
          ? `The relationship keeps evolving: ${future}.`
          : `The next visit can build on this one.`;
    }
  }
  ...
  if (plan?.direction === "journey") {
    switch (beat.kind) {
      ...
      case "discovery":
        return detail
          ? `${name} picks up another chapter around ${detail}.`
          : `The road supplies another chapter.`;
      case "transformation":
        return `By now, ${name} carries evidence of the distance it has traveled.`;
      case "continuation":
        return future
          ? `And the route is still open: ${future}.`
          : `There is another chapter waiting down the road.`;
    }
  }

  // General story mode: commit to the subject's implied world instead of
  // explaining what the compiler is doing.
  switch (beat.kind) {
    case "orientation":
      if (flavor === "pampering") {
      ...
      if (mood === "playful") {
        return `${name} is where the fun starts.`;
      }
    ...
    case "hook":
      ...
      return why
        ? `${cap(why)}. ${name} gives that idea somewhere real to happen.`
        : `${name} gives the story something worth following.`;
    ...
    case "encounter":
      ...
      if (detail) {
        return `${cap(detail)} enters the scene, and suddenly ${subject} has a different problem—or possibility.`;
      }
      return `${name} meets the first thing that makes the story move.`;
    ...
    case "payoff":
      ...
      if (mood === "playful") {
        return `And that is the part people will retell.`;
      }
    ...
    case "continuation":
      return future
        ? `And it does not have to end here: ${future}.`
        : `${name} can keep going. The next chapter can grow from what happened here.`;

    case "discovery":
    case "reveal":
      return detail
        ? `${cap(detail)} changes what ${subject} means in this story.`
        : `The next layer earns its place by changing what you thought you were seeing.`;

    case "challenge":
      return progression
        ? `The pressure builds: ${progression}.`
        : `Something has to give before ${subject} can move forward.`;

    case "milestone":
      return progression
        ? `This is the point where the story can feel the difference: ${progression}.`
        : `This is a point worth remembering because something actually changed.`;
    ...
    case "earned_access":
      return reward
        ? `The next layer opens because it was earned: ${reward}.`
        : `The next layer is open now.`;

    case "need":
      return why
        ? `${cap(why)}. That is where this story needs to begin.`
        : `${name} has a reason to move.`;

    case "instruction":
      return content
        ? `The useful part is concrete: ${content}.`
    ...
    case "origin":
      return detail
        ? `It begins with ${detail}. Everything else grows from there.`
        : `It begins with what was already there.`;

    case "next_step":
      return future
        ? `Next: ${future}.`
        : `There is a clear next move.`;
...
function sanitize(text: string, fallback: string): string {
  ...
  for (const pattern of FORBIDDEN_META_LANGUAGE) {
    if (pattern.test(value)) return fallback;
  }
  return value;
}

export function superCogRealizeStoryBeat(
  beat: StoryBeat,
  plan?: CognitiveExperiencePlan,
): string {
  const subject = cap(subjectFromBeat(beat));
  const value = storyBeat(beat, plan);
  return sanitize(value, `${subject} gives the story somewhere real to begin.`);
}

export function superCogRealizeStoryBeats(
  beats: StoryBeat[],
  plan?: CognitiveExperiencePlan,
): StoryBeat[] {
  return beats.map((beat) => ({
    ...beat,
    text: superCogRealizeStoryBeat(beat, plan),
  }));
}
