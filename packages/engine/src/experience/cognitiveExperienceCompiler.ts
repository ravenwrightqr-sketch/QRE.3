...
import { realizeProvenanceAwareBeats } from "./provenanceAwareNarrativeRealizer.js";
...
const REQUEST_WORDS = new Set([
  "show", "make", "create", "build", "turn", "teach", "write", "tell", "give", "send", "want", "need", "run", "use", "for", "about", "into", "with", "this", "that", "something", "another",
]);

const ANCHOR_STOPWORDS = new Set([
  "the", "a", "an", "and", "or", "but", "that", "this", "these", "those", "from", "into", "over", "under", "after", "before", "while", "where", "when", "what", "which", "who", "whose", "than", "then", "just", "very", "really", "more", "most", "less", "some", "every", "each", "all", "any", "people", "person", "thing", "things", "someone", "everyone",
]);
...
function promptAnchors(prompt:string): string[] {
  ...
  return u(tokens.filter((token)=>{
    const normalized=token.toLowerCase();
    if (ANCHOR_STOPWORDS.has(normalized) || REQUEST_WORDS.has(normalized)) return false;
    if (/^(?:qr|nfc)$/i.test(token)) return true;
...
function orderedBeatKinds(
  trajectory: StoryBeatKind[],
  directives: readonly CognitiveBeatDirective[] | undefined,
): StoryBeatKind[] {
  const required = directives?.map((directive) => directive.kind) ?? [];
  ...
  for (const kind of required) {
    ...
    for (let index = 0; index < result.length; index += 1) {
      if ((phase[result[index] ?? ""] ?? 50) > target) {
        insertAt = index;
        break;
      }
    }
    result.splice(insertAt, 0, kind);
  }
...
function compose(prompt:string,c:CompiledExperienceV16,x:CognitiveExperienceState){
  ...
  const beatsKinds=orderedBeatKinds(t.beats,x.plan.realization?.directives);
  const p=pv(x);
  const es=names(x.entities,x.subject.value||c.movie.subject);
  const anchors=promptAnchors(prompt);
  const presentationEntities=u([...es,...anchors]);

  const seeds:StoryBeat[]=beatsKinds.map((kind,i)=>{
    const src=c.movie.beats[i];
    const d=x.plan.realization?.directives.find(v=>v.kind===kind);
    const b:StoryBeat={id:`cognitive-${i+1}`,kind,order:i,purpose:d?.intent??kind,text:directiveText(d)??src?.text??"",emotionalTarget:x.emotionalIntent[i%(x.emotionalIntent.length||1)],entities:presentationEntities,provenance:[...(src?.sourceFactIds??[]).map(id=>({kind:"observed" as const,source:id,confidence:1})),...p,...(d?.evidence??[]).filter(e=>e.source==="creative_realization").map(e=>({kind:"playful" as const,source:e.detail,confidence:e.confidence}))],directive:d};
    if(!b.text)b.text=realizePremiseBeat(b,x.plan);
    return b;
  });

  const guarded=guardCognitiveStory(seeds,x.plan);
  const rendered=realizeProvenanceAwareBeats(guarded,x.plan);
