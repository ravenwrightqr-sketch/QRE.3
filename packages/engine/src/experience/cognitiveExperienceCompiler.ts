...
function compose(prompt:string,c:CompiledExperienceV16,x:CognitiveExperienceState){
  ...
  const beats:StoryBeat[]=t.beats.map((kind,i)=>{ const src=c.movie.beats[i]; const d=x.plan.realization?.directives.find(v=>v.kind===kind); const b:StoryBeat={id:`cognitive-${i+1}`,kind,order:i,purpose:d?.intent??kind,text:directiveText(d)??src?.text??"",emotionalTarget:x.emotionalIntent[i%(x.emotionalIntent.length||1)],entities:es,provenance:[...(src?.sourceFactIds??[]).map(id=>({kind:"observed" as const,source:id,confidence:1})),...p,...(d?.evidence??[]).filter(e=>e.source==="creative_realization").map(e=>({kind:"playful" as const,source:e.detail,confidence:e.confidence}))],directive:d}; if(!b.text)b.text=realizePremiseBeat(b,x.plan); return b; });
  const guarded=guardCognitiveStory(beats,x.plan).map((b,i)=>({...b,text:(b.text||c.movie.beats[i]?.text||realizePremiseBeat(b,x.plan)||`${x.subject.value||c.movie.subject} moves into the next moment.`).replace(/[.!?]+$/g,"")+"."}));
  const story:ExperienceStory={title:c.title,hook:guarded[0]?.text??c.title,logline:`${x.subject.value||c.movie.subject} unfolds through ${guarded.length} connected moments.`,beats:guarded,ending:guarded.at(-1)?.text??c.title,continuation:x.plan.futureEvolution[0]??c.movie.beats.at(-1)?.text,tone:[...c.blueprint.tone],provenance:p};
  const moments:Moment[]=guarded.map((b,i)=>({type:"message",order:i,text:b.text,meta:{beatId:b.id,kind:b.kind,duration:4000}}));
  const scenePlan:StoryScenePlan[]=guarded.map((b,i)=>({id:`cognitive-scene-${i+1}`,order:i,beatId:b.id,purpose:b.purpose,text:b.text,emotionalTarget:b.emotionalTarget,entities:b.entities,duration:4,transition:i===0?"none":i===guarded.length-1?"cinematic":"fade",visual:{theme:"cinematic",animation:i===0?"slow_zoom":"parallax"},provenance:b.provenance}));
  const scenes:CinematicScene[]=moments.map((m,i)=>({id:`cognitive-cinematic-${i+1}`,type:i===0?"intro":i===guarded.length-1?"emotion":"action",duration:4000,moment:m,order:i,transition:scenePlan[i]?.transition,visual:scenePlan[i]?.visual,preload:i<moments.length-1,meta:{beatId:guarded[i]?.id,kind:guarded[i]?.kind}}));
  return{story,moments,scenePlan,scenes,candidates:t.candidates};
}

function blueprint(b:ExperienceBlueprint,x:CognitiveExperienceState,m:Moment[]):ExperienceBlueprint{ const moments:ExperienceMoment[]=m.map((v,i)=>({type:i===0?"introduction":i===m.length-1?"completion":"story",component:"story",title:i===0?"The beginning":i===m.length-1?"The moment that stayed":"And then",subtitle:x.subject.value,description:v.type==="message"?v.text:"",editable:true,demo:false,order:i,payload:{beatId:v.meta?.beatId,source:"cognitive-experience"}})); return{...b,cognitivePlan:x.plan,moments,metadata:{...b.metadata,archetypes:u([...(b.metadata?.archetypes??[]),x.selectedHypothesis.kind]),dna:u([...(b.metadata?.dna??[]),"canonical-cognitive-compiler","premise-conserved","single-language-authority","super-cog-authoritative-realization","generic-actor-subject-repair"])}}; }
...
export function compileCognitiveExperience(prompt:string,context:ExperienceCompilerContext={}):CognitiveCompiledExperience{
  ...
  const realization=augmentCreativeRealization({prompt,plan:x.plan,premise,realization:baseRealization}); x={...x,plan:{...x.plan,premise,realization}};
