...
function compose(prompt:string,c:CompiledExperienceV16,x:CognitiveExperienceState){
  ...
  const beats:StoryBeat[]=t.beats.map((kind,i)=>{
    ...
    const b:StoryBeat={
      ...
      entities:es,
      provenance:[
        ...(src?.sourceFactIds??[]).map(id=>({kind:"observed" as const,source:id,confidence:1})),
        ...p,
        ...(d?.evidence??[]).filter(e=>e.source==="creative_realization").map(e=>({kind:"playful" as const,source:e.detail,confidence:e.confidence})),
      ],
      directive:d,
    };
  ...
  const guarded=guardCognitiveStory(beats,x.plan).map((b,i)=>({
    ...b,
    text:(b.text||c.movie.beats[i]?.text||realizePremiseBeat(b,x.plan)||`${x.subject.value||c.movie.subject} moves into the next moment.`).replace(/[.!?]+$/g,"")+"."
  }));

  const story:ExperienceStory={
    title:c.title,
    hook:guarded[0]?.text??c.title,
    logline:`${x.subject.value||c.movie.subject} unfolds through ${guarded.length} connected moments.`,
    beats:guarded,
