...
const ABSTRACT_DIRECTIVE: RegExp[] = [
  /make .*?\bmeaningful\b/i, /make .*?\bexplicit\b/i, /connect .*? to meaning/i,
  /connect .*? with identity/i, /surface .*? evidence/i, /preserve .*? context/i,
  /adapt to accumulated/i, /adapt to .*? history/i, /allow participants to/i,
  /let participants/i, /enter living memory/i, /witness .*? contribute/i,
  /affect shared state/i, /change what can happen next/i, /determine what happens next/i,
  /use the current state/i, /carry .*? into the present/i, /recognize what .*? means/i,
  /recognize .*? significance/i, /create a reason to continue/i,
  /provide the next relevant knowledge/i, /resolve the current experience/i,
  /continue from the current state/i, /advance the selected cognitive direction/i,
  /the intended experiential result/i, /the useful target/i, /the next available relationship/i,
  /the next supported condition/i, /go further than before/i, /increase the active condition/i,
  /carry the preceding state/i, /reach the result produced by what happened before/i,
];

const COMPILER_FRAGMENT: RegExp[] = [
  /^gets increasingly\b/i, /^becomes increasingly\b/i, /^increasingly\b/i,
  /^the current condition\b/i, /^the changed state\b/i, /^the next state\b/i,
  /^the resulting state\b/i, /^the active condition\b/i, /^the accumulated state\b/i,
  /^the preceding state\b/i, /^what happens next\b/i, /^what follows\b/i,
  /^the situation\b/i, /^the experience\b/i, /^the interaction\b/i, /^the result\b/i,
];

const EXTRACTION_ARTIFACT: RegExp[] = [
  /^n\b/i, /^n\s+/i, /\band gets$/i, /\bgets\s*$/i,
  /^the interaction$/i, /^the experience$/i, /^the situation$/i,
];
...
function safe(value: string): boolean {
  ...
  return Boolean(text) && !matches(text, DEAD_PROSE) && !matches(text, ABSTRACT_DIRECTIVE) && !matches(text, COMPILER_FRAGMENT) && !matches(text, EXTRACTION_ARTIFACT);
}
...
function evidenceCandidates(beat: StoryBeat, plan?: CognitiveExperiencePlan): string[] {
  ...
  const roleWeight: Partial<Record<CognitivePremiseRole, number>> = {
    event: 8, artifact: 7, medium: 6, place: 6, temporal: 5, transformation: 5,
...
function signals(beat: StoryBeat, plan?: CognitiveExperiencePlan): Signal[] {
  const source = lower(expressiveSource(beat, plan));
  const result: Signal[] = [];
  ...
  if (has(/\b(?:uncertainty|uncertain|unknown|not known|can't tell|cannot tell|unclear|unresolved|doubt)\b/)) result.push("uncertainty");
...
function expressive(beat: StoryBeat, plan?: CognitiveExperiencePlan): string | undefined {
  ...
  if (active.includes("memory") && ["origin", "reflection", "provenance", "payoff"].includes(beat.kind)) return detail ? `${name} brings ${detail} back into the present.` : `${name} brings a remembered detail back into the present.`;
  if (active.includes("discovery") && beat.kind === "discovery") return detail ? `${name} discovers another layer in ${detail}.` : `${name} discovers another layer that was not visible before.`;
  if (active.includes("reveal") && beat.kind === "reveal") return detail ? `${name} sees ${detail} clearly for the first time.` : `${name} sees a hidden detail clearly for the first time.`;
  if (active.includes("surprise") && ["encounter", "discovery", "reveal"].includes(beat.kind)) return detail ? `${name} encounters ${detail} in an unexpected turn.` : `${name} encounters an unexpected turn.`;
  if (active.includes("escalation") && beat.kind === "escalation") return detail ? `${name} goes further with ${detail}.` : `${name} goes further, adding another concrete turn to what is already happening.`;
  if (active.includes("delight") && ["encounter", "payoff"].includes(beat.kind)) return detail ? `${name} gets an unexpectedly funny turn when ${detail} enters the moment.` : `${name} gets an unexpectedly funny turn that demands attention.`;
  if (active.includes("transformation") && beat.kind === "transformation") return detail ? `${name} is visibly different after ${detail}.` : `${name} is visibly different from where the experience began.`;
  return undefined;
}
...
function eventText(beat: StoryBeat, plan?: CognitiveExperiencePlan): string {
  ...
  const action = executableAction(directiveFor(beat, plan)?.action);
  ...
  switch (beat.kind) {
    case "orientation": return pair ? `${name} enters with ${pair} already in view.` : `${name} enters the experience.`;
    case "hook": return pair ? `${name} notices ${pair}, creating the first active turn.` : `${name} encounters the first active turn.`;
    case "need": return result ? `${name} has a concrete target: ${result}.` : pair ? `${name} has to deal with ${pair}.` : `${name} faces the immediate problem.`;
    case "threshold": return pair ? `${name} moves into ${pair}.` : `${name} crosses into the next stage.`;
    case "origin": return pair ? `${name} brings ${pair} into the present.` : `${name} starts from what is already known.`;
    case "encounter": return pair ? `${name} encounters ${pair}.` : `${name} encounters a concrete new condition.`;
    ...
    case "transformation": return transformation.length >= 2 ? `${name} moves from ${sentence(transformation[0])} to ${sentence(transformation[1])}.` : pair ? `${name} is visibly different after ${pair}.` : `${name} ends in a different state than the one established at the beginning.`;
    ...
    case "provenance": return pair ? `${name} preserves ${pair} as part of the record.` : `${name} preserves the origin in the experience.`;
    case "identity": return pair ? `${name} becomes identifiable through ${pair}.` : `${name} establishes a distinct identity through what has happened.`;
    ...
    case "payoff": return result ? `${name} reaches ${result}.` : pair ? `${name} reaches a result shaped by ${pair}.` : `${name} reaches the payoff.`;
    ...
    case "continuation": return pair ? `${name} carries ${pair} into what comes next.` : `${name} leaves a concrete next turn available.`;
  }
}
...
function preserveEvidence(text: string, beat: StoryBeat, plan?: CognitiveExperiencePlan): string {
  const missing = evidenceCandidates(beat, plan).filter((value) => !lower(text).includes(lower(value))).slice(0, 2);
...
export function classifyPremise(beat: StoryBeat, plan?: CognitiveExperiencePlan): Record<string, boolean> {
  const text = lower([beat.text, subject(beat, plan), ...ROLES.flatMap((role) => premiseValues(plan, role)), ...(plan?.emotionalIntent ?? [])].join(" "));
  return {
