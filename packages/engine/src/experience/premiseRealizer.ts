...
function evidenceForBeat(beat: StoryBeat, plan?: CognitiveExperiencePlan, limit = 3): string[] {
  const scored = semanticEvidence(beat, plan).map((value, index) => {
    ...
    const roleBonus = slot?.role === "event" ? 1.4 : slot?.role === "artifact" ? 1.3 : slot?.role === "medium" ? 1.2 : slot?.role === "outcome" ? 1.15 : slot?.role === "transformation" ? 1.1 : slot?.role === "constraint" ? 1.05 : 0;
...
function normalizeDirectiveAction(value: unknown): string {
  ...
  action = action.replace(/^the\s+concrete\s+action\s+is\s+to\s*/i, "").replace(/^the\s+action\s+is\s+to\s*/i, "").replace(/^action\s*:\s*/i, "").replace(/^payoff\s+action\s*:\s*/i, "").trim();
  return sentence(action);
}

function lexicalEvidence(value: string): string[] {
  return unique(words(value).filter((word) => !STOP.has(lower(word))));
}
...
const MECHANIC_OPERATIONS: Record<string, StoryBeat["kind"][]> = {
  anticipation: ["hook", "threshold"], uncertainty: ["threshold", "encounter", "reveal"], suspense: ["threshold", "encounter", "reveal"],
  discovery: ["discovery", "reveal"], surprise: ["reveal", "transformation"], reversal: ["reveal", "transformation"],
  participation: ["action", "feedback", "contribution"], agency: ["action", "feedback", "next_step"], consequence: ["action", "feedback", "transformation"],
  competition: ["challenge", "escalation"], mastery: ["challenge", "feedback", "milestone"], contribution: ["encounter", "contribution", "feedback"],
  authorship: ["action", "contribution", "identity"], reciprocity: ["encounter", "action", "feedback"], accumulation: ["contribution", "milestone"],
  momentum: ["encounter", "escalation", "next_step"], escalation: ["escalation", "payoff"], transformation: ["transformation"],
  contrast: ["orientation", "transformation"], reveal: ["reveal"], memory: ["origin", "reflection"], ritual: ["origin", "action", "continuation"],
  continuation: ["continuation"], adaptation: ["feedback", "next_step"], pampering: ["encounter", "transformation"], indulgence: ["encounter", "escalation", "transformation"],
  excess: ["escalation", "payoff"], spectacle: ["encounter", "escalation", "payoff"], delight: ["encounter", "transformation", "payoff"],
  euphoria: ["escalation", "payoff"], celebration: ["encounter", "milestone", "payoff"], prestige: ["threshold", "identity", "payoff"], novelty: ["discovery", "reveal"],
  curation: ["discovery", "action", "feedback"], scarcity: ["threshold", "challenge", "unlock"], recognition: ["identity", "milestone", "payoff"],
  ownership: ["identity", "milestone", "payoff"], legacy: ["reflection", "provenance", "continuation"], resonance: ["reflection", "payoff", "continuation"],
  intimacy: ["encounter", "reflection", "payoff"], catharsis: ["escalation", "transformation", "payoff"], relief: ["challenge", "payoff"],
  wonder: ["threshold", "discovery", "reveal"], awe: ["encounter", "escalation", "payoff"], embodiment: ["threshold", "action", "feedback"], immersion: ["threshold", "encounter", "transformation"],
};

function activeMechanics(beat: StoryBeat, plan?: CognitiveExperiencePlan): string[] {
  return inferExperienceMechanics({ plan, premise: plan?.premise })
...
function contextEvidence(beat: StoryBeat, plan?: CognitiveExperiencePlan): string {
  ...
  return evidence.length === 1 ? evidence[0] : evidence.length >= 2 ? `${evidence[0]} and ${evidence[1]}` : "";
}

function realizeMechanic(mechanic: string, beat: StoryBeat, plan?: CognitiveExperiencePlan): string | undefined {
  ...
  switch (mechanic) {
    ...
    case "ownership": return context ? `${name} claims ${context} as something they can keep or shape.` : `${name} claims a part of the experience and can shape what comes next.`;
    ...
    case "suspense":
    case "uncertainty": return context ? `${name} can see ${context}, but what it leads to remains unresolved and partly withheld.` : `${name} can see enough to continue, but the important outcome remains unresolved and partly withheld.`;
    ...
    case "resonance": return context ? `${name} recognizes how ${context} carries meaning into the present.` : `${name} recognizes how the result carries forward into the present.`;
