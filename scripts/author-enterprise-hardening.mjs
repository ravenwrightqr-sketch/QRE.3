...
replaceOnce(
  ...
  `function unsupportedConcreteRisk(\n  text: string,\n  envelope: RealityEnvelope,\n): number {\n  const value = clean(text);\n  if (!value) return 1;\n  if (processRisk(value)) return 1;\n\n  const candidate = meaningful(value);\n  const supplied = meaningful(\n    [\n      envelope.subject,\n      ...envelope.events.map((event) => event.label),\n      ...envelope.suppliedEntities,\n      ...envelope.suppliedActions,\n      ...envelope.suppliedStates,\n      ...envelope.suppliedPhrases,\n    ].join(" "),\n  );\n\n  const sourceOverlap = overlap(candidate, supplied);\n  const semanticSignal = semanticOverlap(value, {\n    order: 1,\n    role: "reveal",\n    change: "",\n    next: "",\n    attentionFunction: "",\n    eventIds: envelope.events.map((event) => event.id),\n  });\n\n  /*\n   * Creative language is allowed to be lexically novel. Safety is therefore\n   * based on provenance and approved semantic context, not a finite English\n   * verb dictionary. Supplied absurdity remains authoritative.\n   */\n  if (sourceOverlap >= 0.12 || semanticSignal >= 0.12) return 0;\n\n  return 0;\n}\n\nfunction creativeEvidenceOverlap`,
