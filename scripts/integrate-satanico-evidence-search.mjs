...
const helperReplacement = helperNeedle + `function addSatanicoEvidenceCandidates(candidates: LatentMovieCandidate[], graph: RealityGraph, lens?: string, subject?: string, limit = 8): void {\n  const subsets = searchSatanicoEvidenceSubsets(graph, limit);\n  for (let index = 0; index < subsets.length; index += 1) {\n    addTrajectoryCandidate(candidates, graph, "movie-satanico-" + (index + 1), subsets[index]!, lens, subject);\n  }\n}\n`;
