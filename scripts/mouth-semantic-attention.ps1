...
function Replace-Exact([string]$old, [string]$new, [string]$label) {
  if (-not $script:text.Contains($old)) {
    throw "Semantic attention anchor not found: $label"
  }
...
'@ @'
    "For a sequence of ordinary service facts, do not write all five facts in order. Example pattern only: one anchor fact → attention turn → one supporting fact → callback/reframe → earned ending.",
    "Example of behavioral compression (style only, not reusable content): 'Kitchen first.' → 'Bath next.' → 'Living room last.' → '11:11.' → 'Done.' The point is the changing expectation and forward movement, not the exact words.",
    "A sequence can create interruption through a change in scale, state, location, time, or emphasis. Curiosity can come from forward movement. Contrast can come from a meaningful state change. Payoff can be closure or a callback. None requires a trigger word.",
'@ 'semantic examples'
...
Write-Host "Semantic Mouth attention scoring applied: $path"
