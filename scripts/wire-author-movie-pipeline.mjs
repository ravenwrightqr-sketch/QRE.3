...
if (!source.includes('import { authorBrainUniversal } from "./authorBrainUniversal.js";')) {
  if (source.includes('import { authorMoviePipeline } from "./authorMoviePipeline.js";')) {
    console.log(`Already wired ${path}: authorMoviePipeline import present.`);
    process.exit(0);
  }
...
const replacementHelpers = [
  'function planBeatKindToExperienceBeatKind(kind: "text" | "photo" | "cta"): ExperienceBeat["kind"] {',
...
const oldBlock = /  try \{\n    const authored = await authorBrainUniversal\(authorInput\);[\s\S]*?  \} catch \(error\) \{\n    console\.warn\("\[QRE\]\[AUTHORING\] Author Brain unavailable; preserving deterministic compiled experience\."[\s\S]*?\n  \}/;
