...
const masterAuthorSource = existsSync(join(root, canonical)) ? read(canonical) : "";
if (/function\s+validCut\s*\(/.test(masterAuthorSource)) {
  fail("Master Author contains a duplicate local validCut(); semantic cut evaluation must live only in authorCutPolicy.ts");
}
if (!/authorCutPolicy\.js/.test(masterAuthorSource)) {
  fail("Master Author must import the canonical authorCutPolicy.ts");
}
...
console.log(`CUT POLICY: ${cutPolicy}`);
