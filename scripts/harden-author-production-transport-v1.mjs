...
const candidatePath = path.join(root, "apps/api/src/services/authorMouthCandidateSearch.ts");
        ...
        try {
          ...
          if (
            ...
            const variants = value.variants
              ...
              .filter(Boolean)
              .slice(0, 8);
            if (variants.length) entries.push({ order: Number(value.order), variants });
          }
        } catch {
...
function hardenCandidateParser() {
  let source = read(candidatePath);
  ...
  write(candidatePath, source);
}

function hardenRuntimeFallback() {
  let source = read(runtimePath);

  const modelNameStart = source.indexOf("function modelName(): string {");
  const modelNameEnd = source.indexOf("}\n\nfunction timeoutMs", modelNameStart);

  if (modelNameStart < 0 || modelNameEnd < 0) {
    throw new Error("Could not locate canonical local-model modelName function.");
  }
  ...
  source = source.slice(0, modelNameStart) + replacement + source.slice(modelNameEnd + 2);
  write(runtimePath, source);
}

hardenCandidateParser();
hardenRuntimeFallback();
...
console.log("- local model fallback resolves through QRE_LOCAL_MODEL/QRE_AI_MODEL to qwen2.5vl:7b");
console.log("- hardener uses stable function boundaries and is idempotent");
