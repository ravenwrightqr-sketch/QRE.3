...
**MOUTH-VALIDATION RULE:** every candidate mouth line must pass the same validation gate regardless of whether it came from structured sequence cuts or raw-text recovery. Raw model recovery is a parsing fallback, never a validation bypass.
...
- raw-text recovery bypassed the mouth validator and reintroduced rejected output.

The canonical path now addresses these by separating hidden cognition from finished language, enforcing grounded creative search, normalizing harmless model vocabulary drift, and applying one validation gate to every path into the mouth.
