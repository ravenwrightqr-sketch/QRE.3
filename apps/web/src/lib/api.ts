...
export const getAdminAssets = () => apiGet("/api/admin/assets");
export const assignAdminAsset = (assetId: string) => apiPost(`/api/admin/assets/${encodeURIComponent(assetId)}/assign`);

export const createExperienceAsset = (payload: { slug: string; prompt: string; priceCents?: number }) =>
  apiPost("/api/admin/assets/create-experience", payload);

export const getAssetFlows = (assetId: string) => apiGet(`/api/flow/asset/${assetId}`);
export const attachFlow = (assetId: string, flowId: string, priority: number = 0) => apiPost("/api/flow/assign-flow", { assetId, flowId, priority });
...
export const getScan = (slug: string, geo?: { lat: number; lng: number; accuracy?: number }) => {
