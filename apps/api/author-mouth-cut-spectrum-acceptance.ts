...
const envelope = (events: string[]): RealityEnvelope => ({
  ...
  suppliedActions: events.filter((value) => /walk|roll|run|chase|left|enter/i.test(value)),
  suppliedStates: [],
  recurringSignals: [],
  sensorySignals: [],
  unresolvedTensions: [],
...
const cases = [
  ...
  {
    name: "mock_authority_attitude",
    text: "Already call the lawyer.",
    labels: ["Coco walked in"],
    events: ["Coco walked in"],
    expectAccepted: true,
  },
  {
    name: "status_attitude",
    text: "Fabulous exit.",
    labels: ["Coco left the groomer"],
    events: ["Coco left the groomer"],
    expectAccepted: true,
  },
  {
    name: "temporary_peace_attitude",
    text: "Peace is temporary.",
