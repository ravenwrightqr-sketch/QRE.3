...
import { searchSatanicoRelations } from "./src/services/authorSatanicoRelationSearch.js";
...
for (const test of cases) {
  ...
  const relationships = searchSatanicoRelations({
    graph,
    subject: test.subject,
    limit: 5,
  });
  ...
  console.log("--- SATANICO RELATION SEARCH ---");
  if (!relationships.length) {
    console.log("NO GROUNDED RELATION CANDIDATES");
  } else {
    relationships.forEach((candidate, index) => {
      const [left, right] = candidate.eventIds;
      console.log(
        `[${index + 1}] ${candidate.mechanism} · score=${candidate.score} · ${left} + ${right}`,
      );
      console.log(`    EVIDENCE: ${candidate.evidence.join(", ")}`);
      console.log(`    WHY: ${candidate.reason}`);
    });
  }
  console.log("--- END SATANICO RELATION SEARCH ---");
...
console.log("REALITY TRUTH · RELATIONAL DISCOVERY · PROVENANCE · CROSS-DOMAIN");
console.log("============================================================");
