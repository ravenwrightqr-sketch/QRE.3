...
export async function scanRoute(
  ...
  try {
    ...
    // Scans become immutable episodic memory.
    // This is deliberately a separate write path
    // from factual memory so runtime behavior cannot
    // masquerade as a new truth about the world.
    if (result.asset?.id) {
      try {
        ...
        // Memory must never make the scan itself
        // unavailable.
        console.error(
          "Scan memory consolidation failed:",
          memoryError,
        );
      }
    }
