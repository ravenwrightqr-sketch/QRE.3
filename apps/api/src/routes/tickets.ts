...
router.post("/create", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const attendeeName =
      typeof req.body?.attendeeName === "string"
        ? req.body.attendeeName.trim()
        : null;
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      rows.push({
        assetId,
        eventId,
        attendeeName: count === 1 ? attendeeName : null,
        ticketCode,
        qrUrl: url,
        qrSvg,
    ...
    const tickets = await db.eventTicket.createManyAndReturn({
      data: rows,
      select: {
        id: true,
        assetId: true,
    ...
    try {
      ...
      if (ticket.status !== "ACTIVE") {
        ...
        return res.status(409).json({
          success: false,
          error: "Ticket is no longer active.",
      ...
      const updated = await db.$transaction(async (tx) => {
        const redeemed = await tx.eventTicket.updateMany({
          where: {
            id: ticket.id,
            status: "ACTIVE",
          },
        ...
        if (redeemed.count !== 1) {
          throw new Error("Ticket was already redeemed");
        }
        ...
        const transactionalAnalytics =
          createAnalyticsRepository(tx);

        await transactionalAnalytics.trackEvent({
          assetId: ticket.assetId,
      ...
      return res.status(409).json({
        success: false,
        error: "Ticket could not be redeemed.",
      });
    }
  },
);
