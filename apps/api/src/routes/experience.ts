...
function parseGeoAnchor(raw: unknown): GeoAnchorInput | undefined {
  ...
  const role = value.role === "physical_site" || value.role === "experience_place" || value.role === "event_venue" || value.role === "memory_place" || value.role === "reference_place" ? value.role : "experience_place";
  return { latitude, longitude, label: typeof value.label === "string" ? value.label.trim() : undefined, time: typeof value.time === "string" ? value.time.trim() : undefined, role, source: typeof value.source === "string" ? value.source : "dashboard" };
}
...
async function enrichGeo(anchor: GeoAnchorInput): Promise<GeoAnchorInput> {
  ...
  return { ...anchor, label: anchor.label || resolved.label || undefined, city: anchor.city || resolved.city || undefined, region: anchor.region || resolved.region || undefined, country: anchor.country || resolved.country || undefined };
}

function getCollaboration(asset: any): CollaborationState {
  const data = asset?.templateData;
  const collaboration = data && typeof data === "object" && !Array.isArray(data) ? (data as any).collaboration : undefined;
  return { enabled: collaboration?.enabled === true, inviteOnly: collaboration?.inviteOnly === true };
}

function planForAsset(asset: any): AccountPlan {
  const plan = String(asset?.account?.plan ?? asset?.User?.tier ?? "CONSUMER").toUpperCase();
  return plan === "PRO" || plan === "BUSINESS" ? plan : "CONSUMER";
}

function hasCollaborationEntitlement(asset: any) {
  return Boolean(ENTITLEMENT_RULES[planForAsset(asset)]?.collaborativeMemory);
}

async function ownedAsset(assetId: string, userId?: string) {
  if (!userId) return null;
  return db.asset.findFirst({
    where: { id: assetId, OR: [{ ownerId: userId }, { account: { AccountUser: { some: { userId } } } }] },
    include: { account: { select: { id: true, plan: true } }, User: { select: { id: true, tier: true } } },
  });
}

/** ELITE: public API allowlist. Internal Author/compiler state never crosses this boundary. */
function publicExperienceView(experience: any, sessionId: string, geo: GeoAnchorInput | undefined, warnings: string[]): Record<string, unknown> {
  const view: Record<string, unknown> = {
    ...
    geoStory: null,
    cinematicScenes: Array.isArray(experience?.cinematicScenes) ? experience.cinematicScenes : [],
    memorySnapshot: null,
    receipt: null,
    ...
    estimatedDuration: typeof experience?.estimatedDuration === "number" ? experience.estimatedDuration : undefined,
    movieMode: experience?.movieMode === true,
    warnings,
  };
  if (geo) {
    view.geo = { label: geo.label ?? null, city: geo.city ?? null, region: geo.region ?? null, country: geo.country ?? null, role: geo.role ?? "experience_place" };
  }
...
router.post("/compile", requireAuth, async (req, res) => {
  try {
    ...
    if (typeof prompt !== "string" || !prompt.trim()) return res.status(400).json({ success: false, error: "Experience prompt is required." });
    const geo = rawGeo ? await enrichGeo(rawGeo) : undefined;
    const sessionId = randomUUID();
    const experience = await compileExperience({ prompt, assetId, userId: req.user?.userId, sessionId, memoryRepository: assetId ? createMemoryRepository() : undefined, geoAnchor: geo, movieMode, lens });
    const warnings = [...(experience.warnings ?? [])];
    if (assetId && geo?.latitude !== undefined && geo.longitude !== undefined) {
      try {
        await createPresenceRepository().createGeoProof({ assetId, sessionId, userId: req.user?.userId, lat: geo.latitude, lng: geo.longitude, source: `authoring:${geo.role ?? "experience_place"}`, label: geo.label, city: geo.city, region: geo.region, country: geo.country });
      } catch (error) {
        console.warn("[QRE][AUTHORING] GeoProof persistence failed", error);
        warnings.push("geo_persistence_failed");
      }
    }
    return res.json({ success: true, experience: publicExperienceView(experience, sessionId, geo, warnings), geo: geo ? { label: geo.label ?? null, city: geo.city ?? null, region: geo.region ?? null, country: geo.country ?? null, role: geo.role ?? "experience_place" } : null });
  } catch (error) {
...
router.post("/memory/:assetId", requireAuth, async (req, res) => {
  try {
    ...
    const facts = context.facts.filter((fact) => fact.status === "active" && fact.confidence >= 0.7).map((fact) => `${fact.predicate}: ${fact.value}`).slice(0, 80);
    const sourceMoments = [prompt, ...context.events.slice(0, 20).map((event) => event.summary)];
    const graph = buildAuthorRealityGraph({ prompt, facts, sourceMoments, memoryContext: memoryContextToCognitiveSummary(context) });
    const batch = buildExperienceMemoryBatch({ assetId, userId: req.user?.userId, graph, source: "user" });
    await repository.writeBatch(batch);
    const updated = await repository.loadContext({ assetId, userId: req.user?.userId });
    return res.status(201).json({ success: true, memory: { entities: batch.entities.length, facts: batch.facts.length, relations: batch.relations.length, events: batch.events.length }, recommendations: recommendMemories(updated, prompt), interpretation: { prompt, places: [...new Set(graph.events.map((event) => event.place).filter(Boolean))], participants: [...new Set(graph.events.flatMap((event) => event.entities))], events: graph.events.length } });
  } catch (error) {
    console.error(error);
...
router.get("/contribute/:slug", async (req, res) => {
  try {
    ...
    const asset = await db.asset.findUnique({ where: { slug }, select: { id: true, slug: true, displayName: true, status: true, templateData: true, account: { select: { plan: true } }, User: { select: { tier: true } } } });
    if (!asset || asset.status !== "active") return res.status(404).json({ success: false, error: "QRE object not found." });
    const entitled = hasCollaborationEntitlement(asset);
    const collaboration = getCollaboration(asset);
    return res.json({ success: true, enabled: entitled && collaboration.enabled, inviteOnly: collaboration.inviteOnly === true, asset: { slug: asset.slug, displayName: asset.displayName } });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ success: false, error: "Failed to load contribution settings." });
  }
});
