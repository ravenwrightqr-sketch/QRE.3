...
import { localModelConfig, localModelHealthy } from "../services/localModelRuntime.js";

const router = express.Router();

router.get("/status", requireAuth, async (_req, res) => {
  const local = await localModelHealthy();
  return res.json({
    configured: aiConfigured(),
    provider: "qre-local",
    local,
    config: localModelConfig(),
    external: process.env.QRE_EXTERNAL_AI_ENABLED === "true",
  });
});
...
router.post("/write", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    return res.json({ draft, provider: "qre-local" });
  } catch (error) {
...
router.post("/vision", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    return res.json({ facts, provider: "qre-local" });
  } catch (error) {
