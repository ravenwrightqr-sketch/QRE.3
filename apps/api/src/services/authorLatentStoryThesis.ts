...
function incidentRelationStrength(
  graph: RealityGraph,
  id: string,
): number {
...
function chooseCarrierIds(
  ...
  const endpoint =
    candidate.trajectory.at(-1)?.eventIds.at(-1) ?? "";
  ...
  const scores = unique(
    ...
    .map((id) => {
      const participation = candidate.trajectory.filter((step) =>
        step.eventIds.includes(id),
      ).length;
      const relationStrength = incidentRelationStrength(graph, id);
      const score = participation * 0.45 +
        Math.min(1, relationStrength / 2) * 0.55;
      return { id, score };
    })
...
function buildPayoffDependency(
  ...
  const preceding = candidate.trajectory
    .slice(0, -1)
    .flatMap((step) => step.eventIds);
  ...
  const strongest = direct
    .slice()
    .sort((a, b) => b.strength - a.strength)[0];

  if (strongest) {
    const other =
      strongest.from === endpoint
        ? strongest.to
        : strongest.from;
...
function counterfactualDependency(
  ...
  const totalRelations = Math.max(
    1,
    relationCountFor(graph, supportIds),
  );

  const carrierRelations = carriers.reduce(
    (sum, id) =>
      sum + graph.relations.filter(
        (relation) =>
  ...
  const trajectoryCoverage = carriers.reduce(
    (sum, id) =>
