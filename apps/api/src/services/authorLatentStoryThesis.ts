...
function endpointId(candidate: LatentMovieCandidate): string { const step = candidate.trajectory[candidate.trajectory.length - 1]; return step?.eventIds?.[step.eventIds.length - 1] ?? ""; }
...
function relationBetween(graph: RealityGraph, left: string, right: string): RealityRelation | undefined {
  return graph.relations.filter((relation) => (relation.from === left && relation.to === right) || (relation.from === right && relation.to === left)).sort((a, b) => b.strength - a.strength)[0];
}
...
export function deriveLatentStoryThesis(graph: RealityGraph, candidate: LatentMovieCandidate): LatentStoryThesis {
  ...
  const hypothesisAlignmentScore = hypothesis ? Math.round((hypothesisEvidence.size / Math.max(1, orderedIds(candidate).length)) * 1000) / 1000 : 0;
