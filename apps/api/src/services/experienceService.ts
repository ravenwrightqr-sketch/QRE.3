...
export type CompiledExperienceResult={
 title:string; blueprint:Record<string,unknown>; flowSteps:Array<Record<string,unknown>>; moments:Array<Record<string,unknown>>; cinematicScenes:Array<Record<string,unknown>>; beats:ExperienceBeat[]; estimatedDuration:number; momentCount:number; plan:unknown; world:Record<string,unknown>; discoveries:string[]; learningSignals:string[]; cognition:unknown; authorExperienceState:unknown; authorDiagnostics:unknown; memory:{entities:number;facts:number;relations:number;events:number}|null; geo:GeoAnchorInput|null; geoStory:unknown; memorySnapshot:unknown; presence:ExperiencePresenceContext|null; warnings:string[];
};
...
export async function compileExperience(input:{prompt:string;subject?:string;facts?:string[];sourceMoments?:string[];assetId?:string;userId?:string;sessionId?:string;operationId?:string;memoryRepository?:MemoryRepository;geo?:GeoAnchorInput;geoAnchor?:GeoAnchorInput;movieMode?:boolean;lens?:string}):Promise<CompiledExperienceResult>{
 ...
 if(input.assetId&&input.memoryRepository){try{const batch=buildExperienceMemoryBatch({operationId:input.operationId??input.sessionId??`author:${input.assetId}:${prompt}`,assetId:input.assetId,userId:input.userId,graph:authored.reality,sessionId:input.sessionId,source:"prompt"});await input.memoryRepository.writeBatch(batch);memoryCounts={entities:batch.entities.length,facts:batch.facts.length,relations:batch.relations.length,events:batch.events.length};const previous=memoryContext?extractExperienceStates(memoryContext):[];const candidate=authored.cognition.candidates[0];if(candidate)state=buildExperienceState({graph:authored.reality,candidate,lens:authored.proposition.pattern,memoryContext:learningLines,priorExperienceStates:previous,round:(presence?.visitNumber??1)});}catch{warnings.push("memory_persistence_failed");}}
