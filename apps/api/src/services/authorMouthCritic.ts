...
export async function critiqueMouthCandidates(input: {
  ...
  const system = [
    ...
    "In particular, idiomatic lines such as 'kept the groomer in knots' may be excellent when the source supports groomer + bows/balls and the phrase reads as comic framing rather than a literal claim about the groomer's physical condition.",
    ...
    "Compression: prefer 3-7 words. A line over 7 words is not automatically bad if the extra words make the punch materially better, but brevity is a strong virtue for receipts.",
    ...
    "When one candidate is punchy, source-specific, grounded, and memorable, ACCEPT it even if it is not the most literal candidate.",
