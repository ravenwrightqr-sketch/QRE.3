...
function accumulation(graph: RealityGraph, trajectory: readonly LatentMovieTrajectoryStep[]): number {
  ...
  const recurringEvidence = graph.recurringSignals.length
    ? trajectory.reduce((sum, step) => sum + Math.max(...graph.recurringSignals.map((signal) =>
        Math.max(...step.eventIds.map((id) => overlap(eventLabel(graph, id), signal))),
      0)), 0) / Math.max(1, trajectory.length)
    : 0;
