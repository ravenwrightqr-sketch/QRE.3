 ...
 *
 * This is not a phrase bank and not a benchmark-specific hack.
 * It derives grounded candidate expressions from explicit relationships.
 * The Universal Author decides whether a candidate belongs in the movie.
 *
 * Important: recurrence is NEVER inferred from dislike/preference alone.
 * Recurrence requires actual memory, trajectory, or repeated evidence.
 */

const clean = (value: unknown): string => String(value ?? "").replace(/\s+/g, " ").trim();

const RELATION_PATTERNS: Array<{ regex: RegExp; op: (subject: string, object: string) => string[] }> = [
  {
    regex: /^(.+?)\s+(?:hates|dislikes|can't stand|avoids)\s+(.+)$/i,
    op: (_subject, object) => [`${object}: no.`].map(clean),
  },
  {
    regex: /^(.+?)\s+(?:loves|likes|prefers|adores?)\s+(.+)$/i,
    op: (_subject, object) => [`${object}: yes.`].map(clean),
  },
  {
    regex: /^(.+?)\s+(?:is|was)\s+(.+)$/i,
    op: (_subject, state) => [`${state}.`].map(clean),
  },
];
