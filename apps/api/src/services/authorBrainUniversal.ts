...
type Candidate = { lines: string[]; beatMap?: Array<{ line: number; operation?: string; source?: string }> };
...
const PEOPLE = /\b(?:person|people|guest|guests|buyer|buyers|seller|sellers|couple|client|clients|customer|customers|lawyer|lawyers|mechanic|mechanics|dj|owner|employee|employees|staff|team|roommate|roommates)\b/i;
const PLACE_WORDS = /\b(?:office|room|street|restaurant|hotel|house|kitchen|bathroom|store|shop|court|church|school|hospital|floor|table|counter|dresser|park|car)\b/i;
const OBJECT_WORDS = /\b(?:agreement|contract|clause|bow|towel|dessert|dish|warning|light|connection|song|floor|offer|showing|speech|reception|car|house|room)\b/i;
...
function buildEntityProfile(subject: string, source: string[], intent: string): EntityProfile {
  ...
  const explicitIdentity = text.match(/\b(?:he|she|him|her|his|hers|husband|wife|partner|girlfriend|boyfriend|man|woman|male|female|boy|girl)\b/gi) ?? [];
  const explicitPeople = text.match(new RegExp(PEOPLE.source, "gi")) ?? [];
  const explicitPlaces = text.match(new RegExp(PLACE_WORDS.source, "gi")) ?? [];
  const explicitObjects = text.match(new RegExp(OBJECT_WORDS.source, "gi")) ?? [];
  const explicitBodyDetails = text.match(new RegExp(EXPLICIT_BODY.source, "gi")) ?? [];
  return {
    subject,
    subjectIsEstablished: text.toLowerCase().includes(subject.toLowerCase()),
    explicitIdentity: uniq(explicitIdentity),
    explicitPeople: uniq(explicitPeople),
    explicitPlaces: uniq(explicitPlaces),
    explicitObjects: uniq(explicitObjects),
    explicitBodyDetails: uniq(explicitBodyDetails),
  };
}
...
function purposeFor(operation: Operation, packet: Omit<Packet, "directives">): string {
  switch (operation) {
    ...
    case "payoff_setup": return packet.payoff.target ? `Prepare the immediate condition that makes the destination land: ${packet.payoff.target}` : "Prepare the final beat so the ending feels earned.";
    default: return packet.payoff.target ? `Land the destination as a payoff, not as a detached slogan: ${packet.payoff.target}` : "Land the strongest earned payoff and stop.";
  }
}
...
function buildPacket(input: AuthorBrainTruth): Packet {
  ...
  const base = {
    ...
    subject,
    identityRule: "The subject is exactly the supplied subject. Use that name or neutral language. Never infer identity from a name or domain. Pronouns are authorized only when explicitly established in the packet. Other people, places, objects, and body details are authorized only when established in the packet or directly required by the prompt.",
    entityProfile,
    reality: source,
    intent,
    arc,
  ...
  const directives = Array.from({ length: count }, (_, index) => {
    ...
    return { order: index + 1, operation, source: sourceTarget, purpose: purposeFor(operation, base), viewerChange: viewerChangeFor(operation) };
  });
  return { ...base, directives };
}

function modelMessage(packet: Packet): Array<{ role: "user"; content: string }> {
  const compactPacket = { mission: packet.mission, subject: packet.subject, identityRule: packet.identityRule, entityProfile: packet.entityProfile, reality: packet.reality, intent: packet.intent, arc: packet.arc, payoff: packet.payoff, directives: packet.directives, style: packet.style, constraints: packet.constraints, output: packet.output };
  return [{ role: "user", content: [
    ...
    "Use the supplied subject name or neutral language. Do not infer identity. Do not introduce a new person, place, object, relationship, body detail, or physical event unless it is explicitly authorized by the packet or the creative operation clearly changes an established element without adding a literal event.",
    "Do not copy the facts mechanically. Realize them with current, compressed language while preserving their meaning.",
...
function parseCandidate(raw: string, count: number): Candidate | undefined {
  ...
  try {
    ...
    try { const parsed = JSON.parse(match[0]) as Record<string, unknown>; const lines = Array.isArray(parsed.lines) ? parsed.lines.map(normalizeLine).filter(Boolean) : []; return lines.length === count ? { lines } : undefined; }
    catch { return undefined; }
  }
}

function identityViolation(line: string, packet: Packet): string | undefined {
  if (!PRONOUN_REFERENCE.test(line)) return undefined;
  const known = new Set(packet.entityProfile.explicitIdentity.map((value) => value.toLowerCase()));
  const used = line.match(PRONOUN_REFERENCE)?.[0]?.toLowerCase();
  if (used && !known.has(used)) return "unsupported_identity_reference";
  return undefined;
}

function worldExpansion(line: string, packet: Packet): string | undefined {
  const knownText = packet.reality.join(" ").toLowerCase();
  for (const [pattern, label] of [[EXPLICIT_RELATIONSHIP, "relationship"], [EXPLICIT_PLACE, "place"], [EXPLICIT_BODY, "body_detail"]] as const) {
    ...
    if (match && !new RegExp(`\\b${match[0].replace(/[.*+?^${}()|[\\]\\]/g, "\\$&")}\\b`, "i").test(knownText)) return `unsupported_${label}`;
  }
  return undefined;
}

function beatQuality(candidate: Candidate, packet: Packet): number {
  if (!packet.directives.length) return 0;
  let total = 0;
  for (let i = 0; i < packet.directives.length; i += 1) {
    const line = candidate.lines[i] ?? "";
    const directive = packet.directives[i]!;
    const direct = overlap(line, directive.source);
    const intent = overlap(line, packet.intent);
    const novelty = 1 - overlap(line, candidate.lines[i - 1] ?? "");
    let op = 0.75;
    if (directive.operation === "anchor") op = direct > 0 ? 1 : 0.75;
    if (directive.operation === "change") op = novelty > 0.15 ? 1 : 0.72;
    if (directive.operation === "disruption") op = novelty > 0.25 ? 1 : 0.7;
    if (directive.operation === "payoff_setup") op = novelty > 0.1 ? 1 : 0.7;
    if (directive.operation === "payoff") op = packet.output.endingExact ? 1 : 0.8;
    total += op * 0.7 + Math.max(direct, intent) * 0.2 + novelty * 0.1;
  }
  return metric(total / packet.directives.length);
}
...
function validate(candidate: Candidate, packet: Packet): Validation {
  ...
  candidate.lines.forEach((line, index) => {
    ...
    const expansion = worldExpansion(line, packet); if (expansion) reasons.push(`line_${index + 1}:${expansion}`);
  });
  if (packet.output.endingExact) {
    ...
    for (const line of candidate.lines.slice(0, -1)) if (overlap(line, packet.output.endingExact) > 0.9) reasons.push("ending_revealed_early");
  }
  const normalized = candidate.lines.map((line) => line.toLowerCase()); if (new Set(normalized).size !== normalized.length) reasons.push("duplicate_lines");
  const quality = beatQuality(candidate, packet); if (quality < MIN_SCORE) reasons.push(`quality_below_floor:${quality}`);
  return { ok: reasons.length === 0, reasons, score: quality };
}
