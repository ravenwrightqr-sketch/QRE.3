...
const STATE_WORDS = /\b(?:nervous|calm|anxious|confident|quiet|loud|happy|sad|angry|excited|tired|ready|late|early|busy|empty|full|broken|fixed|clean|dirty|fresh|approved|rejected|missing|gone|fabulous|muddy|still|already|finally)\b/i;
const ACTION_WORDS = /\b(?:came|arrived|left|got|stole|found|sent|ordered|changed|ran|returned|noticed|redlined|repaired|disappeared|stayed|moved|laughed|waited|opened|closed|called|signed|checked|cleaned)\b/i;
...
function buildEntityProfile(subject: string, source: string[], intent: string): EntityProfile {
  ...
  return {
    ...
    explicitObjects: uniq(text.match(/\b(?:bow|towel|dessert|dish|agreement|contract|clause|warning|light|connection|song|offer|house|car|room|reception|speeches|showing)\b/gi) ?? []),
...
function buildReferencePolicy(subject: string): ReferencePolicy {
  return {
    ...
    instruction: `SUBJECT REFERENCE IS CLOSED. Refer to the subject as exactly "${subject}". Never infer or substitute identity attributes.`,
  };
}

function deriveThesis(arc: Packet["arc"], subject: string, ending: string): { thesis: string; transformation: string } {
  const baselineState = STATE_WORDS.test(arc.baseline) ? arc.baseline : `the starting condition for ${subject}`;
  const changeState = ACTION_WORDS.test(arc.change) || STATE_WORDS.test(arc.change) ? arc.change : `the next change for ${subject}`;
  const disruption = arc.disruption || arc.change;
  const result = arc.apparentResolution || arc.change;
  const thesis = `${subject} moves from ${baselineState} through ${changeState}; ${disruption} becomes the turn, and ${result} creates an apparent resolution${ending ? ` before ${ending} recontextualizes it` : ""}.`;
  const transformation = `Move the viewer from the initial read to a changed read, use the most distinctive established element as the turn, create apparent resolution, then make the ending reinterpret what just happened.`;
...
function realizationGoal(operation: Operation, packet: Omit<Packet, "directives">): string {
  switch (operation) {
    case "anchor":
      return `Establish ${packet.arc.baseline} quickly. State the world; do not explain it.`;
    case "change":
      return `Show the shift caused by ${packet.arc.change}. Express the change in meaning or state; do not simply restate the source fact.`;
    case "disruption":
      return `Make ${packet.arc.disruption} newly matter. Reframe, invert, personify, escalate, or juxtapose an established element without adding a literal event.`;
    case "payoff_setup":
      return packet.payoff.target
        ? `Create apparent resolution while leaving the established pressure alive. Prepare the exact destination: ${packet.payoff.target}.`
        : "Create apparent resolution that makes the final beat earned.";
    default:
      return packet.payoff.target
        ? `Land the exact destination as the consequence or reframe of the sequence: ${packet.payoff.target}.`
        : "Land the strongest earned consequence and stop.";
  }
}
...
function purposeFor(operation: Operation, target: string): string {
  switch (operation) {
    ...
    case "change": return "Make the supplied change visible as a change in meaning or state.";
    case "disruption": return "Make an established element newly relevant.";
    case "payoff_setup": return target ? `Prepare the destination without revealing it: ${target}` : "Prepare the ending.";
    default: return target ? `Land the destination as payoff: ${target}` : "Land the strongest earned payoff.";
  }
}
...
function buildPacket(input: AuthorBrainTruth): Packet {
  ...
  const derived = deriveThesis(arc, subject, ending);
  const base = {
    mission: "Create a contemporary, compressed sequence from supplied reality. QRE decides the thesis, transformation, progression, and destination. The model realizes that decision in language.",
    ...
    payoff: {
      ...
      setup: ending
        ? "Build backward from the ending. The penultimate beat must create apparent resolution while preserving the pressure that makes the final line land."
        : "Make the final beat the earned consequence or newly relevant read of the preceding progression.",
      requirement: ending
        ? "Do not reveal the ending early. The ending must change the interpretation of the sequence, not merely repeat it."
        : "Finish on the strongest earned consequence and stop.",
    },
    style: [
      "Contemporary spoken intelligence: direct, compressed, specific, alive.",
      "Prefer strong verbs, clean syntax, varied rhythm, and implication over explanation.",
      ...
      "Trust the reader. Make the line do the work instead of explaining the work.",
      "Use fragments only when deliberately sharp.",
    ],
    constraints: [
      "Literal reality is immutable unless the prompt explicitly requests fiction or transformation.",
      "Creative realization may change framing, attitude, status, implication, juxtaposition, rhythm, personification, metaphorical read, or emphasis without inventing a new literal event.",
      ...
      "Do not mechanically copy the source facts into lines 2 through the penultimate line.",
...
function modelMessage(packet: Packet): Array<{ role: "user"; content: string }> {
  ...
  return [{
    ...
    content: [
      ...
      "You are the last language renderer in a deterministic creative system.",
      "QRE has already decided what the sequence means, what changes, what turns, and where it lands. Do not create a second story.",
      `RETURN JSON ONLY: {\"lines\":[\"...\"]}. EXACTLY ${packet.output.lineCount} lines.`,
      `Each non-final line: ${packet.output.maxWords} words maximum.`,
      packet.output.endingExact ? `FINAL LINE EXACTLY: ${packet.output.endingExact}` : "End on the strongest earned result.",
      "Do not copy the facts as a receipt. Render the thesis and each realization goal.",
      "Line 1 establishes. Line 2 changes the read. Middle lines make an established element newly matter. The penultimate line creates apparent resolution. The final line pays off the destination.",
      packet.referencePolicy.instruction,
      "Use only the established world. Creative language may be new; literal facts may not.",
...
function exactFactReplay(line: string, source: string): boolean {
  ...
  if (!a || !b) return false;
  if (a === b) return true;
  return overlap(a, b) >= 0.92 && meaningful(a).length >= 3;
}
...
function beatQuality(candidate: Candidate, packet: Packet): number {
  ...
  for (let index = 0; index < expected.length; index += 1) {
    ...
    let op = 0.68;
    if (directive.operation === "anchor") op = sourceSupport > 0 ? 1 : 0.75;
    if (directive.operation === "change") op = novelty > 0.15 ? 1 : 0.72;
    if (directive.operation === "disruption") op = novelty > 0.2 ? 1 : 0.7;
    if (directive.operation === "payoff_setup") op = novelty > 0.15 ? 1 : 0.72;
    if (directive.operation === "payoff") op = packet.output.endingExact ? 1 : 0.86;
    const thesisSignal = Math.max(sourceSupport, intentSupport);
    total += op * 0.68 + thesisSignal * 0.17 + novelty * 0.15;
  }
...
function validate(candidate: Candidate, packet: Packet): Validation {
  ...
  candidate.lines.forEach((line, index) => {
    ...
    if (index > 0 && index < candidate.lines.length - 1) {
      const source = packet.directives[index]?.source ?? "";
      if (source && exactFactReplay(line, source)) reasons.push(`line_${index + 1}:mechanical_fact_replay`);
    }
  });
  ...
  if (packet.output.endingExact) {
    ...
    for (const line of candidate.lines.slice(0, -1)) {
      if (overlap(line, packet.output.endingExact) > 0.9) reasons.push("ending_revealed_early");
    }
  }
