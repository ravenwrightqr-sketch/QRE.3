...
function rankFacts(facts: string[], ending: string): RealityFact[] {
  return facts.map((text, index) => ({
    ...
    index,
    novelty: metric(
      0.2 + Math.min(0.3, tokens(text).size * 0.05) +
  ...
  for (let i = 0; i < ordered.length; i += 1) {
    for (let j = i + 1; j < ordered.length; j += 1) {
      ...
      if (a.recurring || b.recurring) {
        kind = "recurrence"; strength = 0.9; reason = "A supplied detail or state returns.";
      } else if (shared >= 0.45) {
        kind = "overlap"; strength = 0.7 + Math.min(0.2, shared * 0.4); reason = "The facts share a concrete detail.";
      } else if (a.action && b.state) {
        kind = "transition"; strength = 0.92; reason = "An observed action produces or accompanies a later state.";
      } else if (changedState) {
        kind = "transition"; strength = 0.78; reason = "The later fact changes the earlier state.";
      } else if (CONTRAST.test(`${a.text} ${b.text}`) || (a.state && b.state)) {
        kind = "contrast"; strength = 0.72; reason = "The facts create an opposing or changed read.";
      } else if (CAUSAL.test(b.text)) {
        kind = "continuation"; strength = 0.7; reason = "The later fact contains causal or continuation language.";
      }
