 ...
 *
 * Continuity law:
 *   A cut may sharpen, compress, contrast, imply, callback, or frame
 *   approved reality. It may not quietly become a new fact.
 */
...
const INTERNAL = /\b(?:cognition|planner|planning|beat|candidate|semantic|trajectory|viewer|audience|observer|objective|curiosity|prediction error|state shift|sequence|author|mouth|canonical|supplied evidence|semantic turn|relation kind|payoff dependency|memory projection|future thread|realization boundary)\b/i;
...
const ABSTRACT_NOUN = /\b(?:lightness|stillness|softness|warmth|tension|pressure|presence|absence|recognition|connection|possibility|momentum|energy|rhythm|silence|distance|closeness|uncertainty|comfort|relief|contentment|satisfaction|release|ease|bloom|weight|space|pull|gravity|dissonance|acknowledgement|acknowledgment|resonance|meaning|significance|transformation|consequence)\b/i;
...
const PHYSICAL_VERB = /\b(?:smiled|smile|laughed|laugh|walked|walk|moved|move|looked|look|watched|watch|stared|stare|blinked|blink|winked|wink|nodded|nod|shrugged|shrug|touched|touch|held|hold|reached|reach|stood|stand|sat|sit|ran|run|jumped|jump|wagged|wag|barked|bark|kissed|kiss|hugged|hug|grabbed|grab|opened|open|closed|close|entered|enter|returned|return|called|call|talked|talk|spoke|speak|heard|hear|saw|see|breathed|breathe|pointed|point|turned|turn|carried|carry)\b/i;

const BODY = /\b(?:eye|eyes|face|mouth|shoulder|shoulders|hand|hands|head|tail|fur|coat|body|room|door|window|floor|wall|table|chair|car|road|street|sky|shadow|light|sound|scent|voice|water|phone|screen|breath)\b/i;
...
const ASSERTED_NEW_FACT = /^(?:there was|there were|someone|somebody|another|a stranger|a woman|a man|a person|the room|the door|the window|the car|the street|the sky|the shadow|the light|the sound|the smell|she|he|they|it)\b/i;

function meaningfulTokens(value: string): Set<string> {
  return new Set(
    [...tokenSet(value)].filter((token) => !STOP.has(token)),
  );
}
...
function roleIsActuallySupplied(
  role: string,
  envelope: RealityEnvelope,
): boolean {
  ...
  return worldEvidence(envelope).some((item) =>
    normalize(item).includes(normalizedRole),
  );
}
...
function isFrameOnly(text: string): boolean {
  ...
  if (FRAME_NOUN.test(value) && (FRAME_VERB.test(value) || STATUS.test(value))) {
    return true;
  }
...
function unsupportedConcrete(
  ...
  /*
   * Exact source and strongly source-covered cuts are maximally safe.
   * Creative framing can escape wording, but not reality.
   */
  const exact = labels.some(
    (label) => normalize(label) === normalize(value),
  );
  if (exact || local >= 0.78 || global >= 0.72) return 0;
  ...
  if (BODY.test(value)) {
    const suppliedBody = worldEvidence(envelope).some((item) =>
      BODY.test(item) &&
      overlap(
  ...
  /* A newly asserted concrete subject is unsafe unless the supplied world names it. */
  if (
    ASSERTED_NEW_FACT.test(value) &&
    global < 0.55 &&
    local < 0.7
  ) {
    return 1;
  ...
  /*
   * Do not invent concrete nouns merely because one semantic word overlaps.
   * Pure framing remains allowed; concrete paraphrase needs real support.
   */
  ...
  if (hasConcreteSurface) return 1;

  /* Abstract / rhetorical language is Mouth's safe creative region. */
  return 0;
}
...
function explanationPenalty(text: string): number {
  return EXPLANATION.test(clean(text)) || INTERNAL.test(clean(text))
    ? 1
    : 0;
}
...
function formScore(text: string): number {
  ...
  if (
    /\b(?:but|yet|still|until|finally|again|already|apparently|anyway|for now|temporary|temporarily)\b/i.test(
      value,
    )
  ) {
    score += 0.12;
  }
  if (/^(?:a|an|the)\s+/i.test(value) && ABSTRACT_NOUN.test(value)) {
    score -= 0.4;
  }
...
function payoffScore(
  ...
  if (
    /\b(?:peace|for now|temporary|temporarily|exit|fab|fabulous|dapper|done|made it|win|winner|finished|approved|cleared)\b/i.test(
      value,
    )
  ) {
    score += 0.25;
...
function questionScore(
  ...
  if (
    attention === "question" ||
    attention === "hook" ||
    clean(beat.role).toLowerCase().includes("question")
  ) {
    return 1;
  }
...
function continuityScore(
  ...
  const value = clean(text);
  const next = clean(beat.next);
  const terminal = Boolean(beat.paysOff?.length);

  if (terminal) {
    return payoffScore(value, beat);
  }

  if (!next) return 0.45;

  const question = questionScore(value, beat);
  const closure = STATUS.test(value) ? 0.25 : 0.7;
  const forwardMarker = /\b(?:but|yet|still|until|then|again|already|why|how|what|until)\b/i.test(value)
    ? 0.95
    : 0;

  return metric(question * 0.45 + closure * 0.25 + forwardMarker * 0.3);
}
...
function candidateScore(
  ...
  const continuity = continuityScore(value, beat);
  const novelty = priorTexts.length
    ? metric(
        1 -
          Math.max(
            ...priorTexts.map((prior) =>
              overlap(
                meaningfulTokens(value),
                meaningfulTokens(prior),
              ),
            ),
            0,
          ),
  ...
  const worldOverlap = overlap(
    meaningfulTokens(value),
    meaningfulTokens(worldEvidence(envelope).join(" ")),
  );

  const supportedEventIds =
    beat.eventIds?.length && sourceOverlap >= 0.25
      ? [...beat.eventIds]
      : [];
  const supportedRelationPairs =
    beat.relationKinds?.map((kind) => String(kind)).filter(Boolean) ?? [];
  ...
  const obligation = metric(
    (beat.eventIds?.length ? 0.45 : 0.25) * 0.42 +
      baseSemantic * 0.38 +
      (supportedEventIds.length ? 0.2 : 0),
  );

  const transition = metric(
    Number(beat.viewerState?.stateShift) || 0.45,
  );

  const meaning = metric(
    baseSemantic * 0.5 +
      form * 0.16 +
      (STATUS.test(value) ? 0.08 : 0) +
      payoff * 0.26 -
      abstract * 0.18,
  );
  ...
  if (continuity >= 0.62) reasons.push("sequence-continuity");
  if (payoff >= 0.62) reasons.push("viewer-reward");
  if (abstract > 0.35) reasons.push("abstract-nominalization");
  if (
    /^(?:a|an|the)\s+/i.test(value) &&
    ABSTRACT_NOUN.test(value)
  ) {
    reasons.push("article-abstract-fragment");
  }
  ...
  return {
    ...
    relationContractScore: metric(
      supportedRelationPairs.length ? 0.75 : 0.35,
    ),
    forbiddenMoveRisk: forbidden,
    cohesionScore: metric(
      0.55 + novelty * 0.25 + meaning * 0.2,
    ),
    noveltyScore: novelty,
    compressionScore: form,
...
function buildSystemPrompt(): string {
  return [
    ...
    "FEEL IT. DO NOT EXPLAIN IT.",
    "Truth comes first. Never trade a real supplied fact for a more cinematic invented fact.",
    "Continue means continue the approved reality and semantic sequence, not invent a missing scene between beats.",
    ...
    "A strong cut may be a fragment, headline, verdict, question, callback, dry observation, or compressed status line.",
    ...
    "Do not silently introduce a new person, object, location, or concrete circumstance just to make a sentence feel cinematic.",
    ...
    "An abstract or rhetorical cut may be creatively phrased when it remains tethered to the supplied reality and approved semantic movement.",
    ...
    "Earlier cuts establish expectations. Later cuts may cash them in. Do not resolve every beat immediately.",
    "When the beat has a next promise, leave enough narrative oxygen for the next cut instead of manufacturing closure early.",
  ...
  return [
    { role: "system", content: buildSystemPrompt() },
    {
      ...
      content: JSON.stringify({
        ...
        output: {
          variantsByBeat:
            "exactly 3 viewer-facing variants for every beat, in order",
        },
      }),
    },
  ];
...
export function parseMouthCandidateBatch(
  ...
  try {
    const parsed = JSON.parse(clean(raw)) as {
      variantsByBeat?: unknown;
    };

    if (
      !Array.isArray(parsed?.variantsByBeat) ||
      parsed.variantsByBeat.length === 0
    ) {
      return undefined;
    }
    ...
    const orders = [...variantsByBeat.map((item) => item.order)].sort(
      (a, b) => a - b,
    );
    ...
    if (
      variantsByBeat.some(
        (item) =>
          new Set(
            item.variants.map((value) => value.toLowerCase()),
          ).size !== 3,
      )
    ) {
      return undefined;
    ...
    return {
      variantsByBeat: variantsByBeat.sort(
        (a, b) => a.order - b.order,
      ),
    };
  } catch {
