...
function realityShapeForBeat(
  ...
  const eventCount = shapes.filter(
    (shape) => shape === "event",
  ).length;

  const stableCount = shapes.filter(
    (shape) => shape === "stable",
  ).length;

  const stateCount = shapes.filter(
    (shape) => shape === "state",
  ).length;

  if (
    eventCount >=
    Math.max(stableCount, stateCount)
  ) {
    return "event";
  }

  if (
    stableCount > eventCount &&
    stableCount >= stateCount
  ) {
    return "stable";
  }

  if (
    stateCount > eventCount &&
    stateCount > stableCount
  ) {
    return "state";
  }
...
function figurativeExpression(
  text: string,
): boolean {
  const value = clean(text);

  return (
    /\b(?:like|as\s+if|as\s+though|almost\s+like|felt\s+like|seemed\s+like)\b/i.test(
      value,
    ) ||
    /\b(?:metaphorically|figuratively)\b/i.test(
      value,
    )
  );
}
...
function unsupportedConcreteClaimRisk(
  ...
  const candidateTokens =
    meaningfulTokenSet(value);

  const sourceTokens =
    meaningfulTokenSet(sourceWorld);
...
function directTransformationSignal(
  ...
  const transformationStructure =
    /\b(?:\w+(?:ness)?\s+(?:became|turned|faded|eased|lifted|softened|opened|gave|flowed|dissolved|bled|released|settled|loosened|lightened|changed|shifted|broke|melted)|(?:became|turned|faded|eased|lifted|softened|opened|gave|flowed|dissolved|bled|released|settled|loosened|lightened|changed|shifted|broke|melted))\b/i.test(
      value,
    );
...
function experientialConsequenceSignal(
  ...
  const experientialStructure =
    /\b(?:something|nothing|everything|it|that|this|already|somehow|suddenly|finally|still|almost|again|too|just)\b/i.test(
      value,
    )
      ? 0.14
      : 0;
  ...
  const consequenceVerb =
    /\b(?:opened|closed|changed|shifted|deepened|collapsed|eased|tightened|loosened|faded|settled|melted|flowed|stayed|remained|became|turned|pulled|released|connected|felt|feel|wanted|want|needed|need|lost|found|held)\b/i.test(
      value,
    )
      ? 0.22
      : 0;
  ...
  const beatAuthority =
    (beat.eventIds?.length ?? 0) > 0 ||
    Boolean(beat.change) ||
    Boolean(beat.attentionFunction) ||
    (beat.relationKinds?.length ?? 0) > 0
      ? 0.12
      : 0;
...
function expressiveRealizationSignal(
  ...
  const localOverlap =
    overlap(current, source);

  const directTransformation =
    directTransformationSignal(value);

  const experientialConsequence =
    experientialConsequenceSignal(
      value,
      sourceLabels,
      beat,
  ...
  const activeVerb =
    /\b(?:became|turned|faded|eased|lifted|softened|opened|gave|flowed|dissolved|bled|released|settled|loosened|lightened|changed|shifted|broke|melted|stayed|keep|kept|continued|waited|felt|feel)\b/i.test(
      value,
    )
      ? 0.2
      : 0;
  ...
  const contrastive =
    /\b(?:but|yet|still|almost|only|except|instead|rather|never|not|nothing|everything|then|before|after|until)\b/i.test(
      value,
    )
      ? 0.12
      : 0;
  ...
  const imageableAbstraction =
    expressiveVocabulary(value)
      ? 0.18
      : 0;

  const bareArticleLabel =
    /^(?:a|an|the)\s+[a-z][a-z'-]*(?:\s+[a-z][a-z'-]*){0,2}[.!?]?$/i.test(
      value,
    );

  const conceptualLabel =
    /\b(?:assent|approval|recognition|connection|possibility|momentum|lightness|warmth|ease|release|permission|agreement|confirmation)\b/i.test(
      value,
    );
...
function observerDiscoverySignal(
  text: string,
  sourceLabels: readonly string[],
  beat: MouthCandidateBeat,
): number {
  ...
  /*
   * EXPLANATION:
   * The line tells the observer what the experience means.
   */
  const directExplanation =
    /\b(?:this|that|it)\s+(?:means|shows|proves|reveals|demonstrates|makes|became|was|is)\b/i.test(
      value,
    );

  const explicitSignificance =
    /\b(?:important|meaningful|special|connection|relationship|bond|lesson|purpose|reason|significant)\b/i.test(
      value,
    );

  /*
   * STATE LABEL:
   * Names the resulting feeling/state instead of letting the observer infer it.
   *
   * Important: "wanting" is NOT banned. "The wanting lingered." is a valid
   * experiential residue. We only penalize explicit emotional conclusions.
   */
  const explicitEmotionalLabel =
    /\b(?:in love|loved|happy|sad|relieved|comforted|safe|close|connected|lonely|excited|afraid|hurt|grateful|fulfilled|satisfied)\b/i.test(
      value,
    );

  /*
   * RESIDUE:
   * Something remains, continues, stretches, doesn't quite end, or carries on.
   *
   * This is one of the strongest observer-discovery structures.
   */
  const residue =
    /\b(?:lingered|linger|remained|remain|stayed|stay|continued|continue|kept|still|yet|again|lasted|last|stretched|stretches|didn't end|did not end|not yet|wasn't over|was not over|carried on|went on)\b/i.test(
      value,
    )
      ? 0.28
      : 0;
  ...
  /*
   * CONSEQUENCE:
   * Shows what became different without explaining the larger meaning.
   *
   * "The certainty slipped."
   * "More words felt possible."
   * "The edges softened."
   */
  const consequence =
    /\b(?:shifted|changed|softened|opened|closed|eased|lifted|slipped|faded|deepened|loosened|settled|released|lightened|turned|became|felt|felt possible|felt different|grew|narrowed|widened)\b/i.test(
      value,
    )
      ? 0.24
      : 0;
  ...
  /*
   * CONTRAST / REVERSAL:
   * The line lets the observer notice a mismatch between expectation and
   * what actually happened.
   */
  const contrast =
    /\b(?:but|yet|still|instead|rather|not|never|wasn't|was not|didn't|did not|without|except|only|unexpectedly|unbidden)\b/i.test(
      value,
    )
      ? 0.18
      : 0;
  ...
  /*
   * IMPLICIT RELATION:
   * Pronouns / compressed references can make the observer participate in
   * completing the meaning. This is useful only when the beat is grounded.
   */
  const implication =
    /\b(?:something|nothing|it|that|this|there|still|somehow|almost|enough|another|more)\b/i.test(
      value,
    )
      ? 0.12
      : 0;
  ...
  /*
   * QUESTION / OPEN EDGE:
   * A line can preserve curiosity by leaving something unresolved.
   */
  const openEdge =
    /(?:\?|not yet|unfinished|left open|for now|still there|still possible)\b/i.test(
      value,
    )
      ? 0.16
      : 0;
  ...
  /*
   * Concision helps discovery because unnecessary explanation closes the
   * inference gap.
   */
  const concise =
    value.split(/\s+/).filter(Boolean).length <= 10;

  /*
   * Grounding is mandatory. Observer discovery cannot float free of the
   * supplied beat.
   */
  const grounded =
    sourceOverlap >= 0.08 ||
  ...
  return metric(
    0.18 +
      ...
      openEdge +
      (concise ? 0.08 : 0) +
      (sourceOverlap >= 0.15 ? 0.1 : 0) +
      ((beat.eventIds?.length ?? 0) > 0 ? 0.1 : 0) -
      (explicitEmotionalLabel ? 0.16 : 0) -
      (explicitSignificance ? 0.28 : 0) -
      (directExplanation ? 0.4 : 0),
  );
}
...
function groundedSurpriseForCandidate(
  ...
  interpretation: ReturnType<
    typeof evaluateMouthInterpretation
  >,
  lensInput: string | undefined,
): number {
  const sourceLabels =
...
export function buildMouthCandidateMessages(
  ...
  const domainContextInstruction =
    contextText
      ? [
  ...
  const observerExperienceInstruction =
    observerExperience
      ? [
...
export function scoreMouthCandidate(input: {
  ...
  const lensInput =
    activeLensByBeat.get(
      input.beat as object,
    ) || undefined;
  ...
  const groundedSurprise =
    groundedSurpriseForCandidate(
      input.text,
      input.beat,
      input.envelope,
      legacy,
  ...
  const strongExperientialConsequence =
    experientialConsequence >= 0.58 &&
    groundedSurprise >= 0.58;
