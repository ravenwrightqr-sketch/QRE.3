...
export async function createAuthorExperience(input: {
  ...
  const system = [
    ...
    "Prefer compact bursts that can be felt in motion; most beats should land in roughly 2 to 7 words.",
    ...
    [
      { role: "system", content: system },
      {
        ...
        content: JSON.stringify({
          ...
          instruction: "Write the viewer-facing beats first as a moving sequence of compact screen moments. Use the selected read as the creative seed and the full supplied reality as material. Prefer lived voice, reaction, fragments, and compressed identity over explanation. Follow the most alive possibility, shift status, surprise, and land. After the beats are finished, ground each one from the supplied reality actually used.",
        }),
      },
    ],
    "json",
