...
export type AuthorCognitivePlan = {
  ...
  selectedFrame: CharacterFrameCandidate;
...
function inferMode(input: AuthorCognitionInput): AuthorCognitivePlan["mode"] {
  ...
  if (/wedding|memory|anniversary|family|remember|memorial/.test(text)) return evidence ? "living_memory" : "concept";
...
function scoreCandidate(strategy: string, input: AuthorCognitionInput, text: string): number {
  ...
  if (input.round && input.round > 1 && strategy === "callback_and_continuity") score += 12;
...
function findContradictions(values: string[], graph?: RealityGraph): string[] {
  ...
  const pairs: Array<[RegExp, RegExp, string]> = [
    ...
    [/missing|lost|vanished|gone/, /packed|moved|loaded|finished/, "completion vs unresolved absence"],
    [/same|again|returned|back/, /different|changed|new/, "repetition vs change"],
  ];
...
function deriveCoreTraits(input: AuthorCognitionInput): string[] {
  return uniq([...input.facts, ...input.sourceMoments].filter((value) => /\b(?:nervous|scared|shy|fierce|sweet|gentle|wild|goofy|stubborn|obsessed|proud|confident|quiet|loud|funny|mischievous|tired|calm|excited)\b/i.test(value)), 8);
}

function deriveObjectRelationships(input: AuthorCognitionInput): string[] {
  return uniq([...input.facts, ...input.sourceMoments].filter((value) => /\b(?:bow|bows|ball|balls|tie|ties|bath|bathroom|kitchen|box|bag|car|tag|keychain|record|records|card|cards|song|pier|manual|paint|hvac|mirror)\b/i.test(value)), 10);
}

function deriveStatusPosture(traits: string[], contradictions: string[]): string {
  const joined = `${traits.join(" ")} ${contradictions.join(" ")}`.toLowerCase();
  if (/nervous|scared/.test(joined) && /fierce|wild|confident|stubborn/.test(joined)) return "guarded but defiant; resists looking powerless";
  if (/quiet|silent/.test(joined) && /mirror|appearance|fade|reveal/.test(joined)) return "controlled and observant; lets the result speak";
  if (/missing|lost|vanished/.test(joined) && /packed|moved|finished/.test(joined)) return "apparently complete while carrying an unresolved problem";
  if (/happy|proud|excited/.test(joined) && /nervous|scared|shy/.test(joined)) return "emotionally mixed; outward state is ahead of inner certainty";
  if (/old|vintage|inherited|family/.test(joined)) return "custodian of continuity; meaning lives in small details";
  return "defined by the strongest supplied contradiction";
}

function deriveEmotionalPosture(traits: string[], contradictions: string[]): string {
...
function deriveCreativeFrames(input: AuthorCognitionInput, contradictions: string[]): CharacterFrameCandidate[] {
  const text = `${input.prompt} ${input.lens ?? ""} ${input.subject ?? ""} ${input.facts.join(" ")} ${input.sourceMoments.join(" ")}`.toLowerCase();
  const frames: CharacterFrameCandidate[] = [];
  const add = (frame: string, reason: string, confidence: number) => frames.push({ frame, reason, confidence });

  if (/nervous|fierce|hates|steals|stole|rebellion/.test(text)) {
    add("negotiation", "character attitude makes an ordinary interaction feel like a status contest", 0.92);
    add("rebellion", "the supplied resistance or stealing can be framed as a tiny act of defiance", 0.88);
  }
  if (/groom|grooming|barber|salon|cleaning|repair|service/.test(text)) {
    add("operation", "the routine service can be experienced as a focused operation when the facts create pressure or personality", 0.78);
    add("transformation", "before/after state change can carry the experience without becoming a template", 0.74);
  }
  if (/moving|packed|missing|lost|vanished|box/.test(text)) {
    add("investigation", "an unresolved missing object naturally creates a question worth following", 0.94);
    add("spy extraction", "the move can acquire extraction energy when the missing item creates an unresolved thread", 0.86);
  }
  if (/wedding|vows|ceremony|love|romantic/.test(text)) {
    add("romantic tension", "the supplied emotional contrast can create anticipation without inventing events", 0.9);
    add("release", "a supplied pre/post emotional shift can carry the payoff", 0.86);
  }
  if (/same|again|returned|return|different year|back/.test(text)) {
    add("return", "revisiting a supplied place or ritual can change its meaning", 0.92);
    add("memory loop", "repetition with changed context creates a natural callback", 0.88);
  }
  if (/record|records|birthday card|same song|sundays|memorial|remember/.test(text)) {
    add("refrain", "a recurring supplied detail can become the emotional anchor", 0.94);
    add("portrait", "small recurring details can imply a person without inventing biography", 0.9);
    add("quiet observation", "the material is stronger when observed than dramatized", 0.92);
  }
  if (/filthy|dirty|brand new|before|after|clean|restored/.test(text)) add("reveal", "a visible supplied state change can create a compact reveal", 0.8);

  if (contradictions.length) add("status inversion", `the strongest contradiction is ${contradictions[0]}`, 0.84);

  const seen = new Set<string>();
  return frames
    .filter((frame) => !( /memorial/.test(text) && /spy|heist|mission|game|boss/.test(frame.frame) ))
    .filter((frame) => {
...
function selectFrame(candidates: CharacterFrameCandidate[], input: AuthorCognitionInput, contradictions: string[]): CharacterFrameCandidate {
  if (!candidates.length) return { frame: "NONE", confidence: 1, reason: "the natural reality is the strongest available lens" };
  ...
  if (input.lens && clean(input.lens).toLowerCase() !== "let qre decide") {
    return { ...top, confidence: Math.max(top.confidence, 0.9), reason: `requested lens is available; preserve it as perspective, not sequence` };
  }

  if (input.round && input.round > 1 && /return|again|back|same|next|visit/.test(text)) {
    const continuity = candidates.find((candidate) => /return|memory loop|refrain|portrait|private/i.test(candidate.frame));
    if (continuity) return continuity;
  }

  if (top.confidence < 0.72 && !contradictions.length) {
    return { frame: "NONE", confidence: 0.76, reason: "no candidate materially improves the supplied reality" };
  }

  return top;
}

function deriveCharacterRead(input: AuthorCognitionInput, contradictions: string[]): CharacterRead {
  const coreTraits = deriveCoreTraits(input);
  const objectRelationships = deriveObjectRelationships(input);
  const creativeFrames = deriveCreativeFrames(input, contradictions);
  return {
    coreTraits,
    contradictions,
    statusPosture: deriveStatusPosture(coreTraits, contradictions),
    emotionalPosture: deriveEmotionalPosture(coreTraits, contradictions),
    objectRelationships,
    creativeFrames,
    allowedMoves: [
      "metaphor", "personification", "status language", "double meaning", "character-specific exaggeration", "comic framing", "understatement", "callback", "recontextualization", "rhetorical game language when the frame genuinely increases the experience",
    ],
    avoidedMoves: [
      "invented concrete events", "invented dialogue", "invented reactions", "invented people", "invented locations", "invented physical props", "literalizing a metaphorical frame", "forced game mechanics when the evidence does not support them", "generic emotional summary",
    ],
  };
}
...
function chooseAttention(candidates: AttentionCandidate[], input: AuthorCognitionInput): string {
  ...
  if (input.round && input.round > 1 && candidates.some((candidate) => candidate.strategy === "callback_and_continuity")) return "callback_and_continuity";
  if (/nervous|scared|fierce|hates|loves|dog|poodle|bulldog/.test(text)) return "personality_contrast";
  if (/million|expensive|luxury|wealth|yacht|estate/.test(text)) return candidates.find((candidate) => candidate.strategy === "status_to_meaning" || candidate.strategy === "provenance_and_history")?.strategy ?? candidates[0]?.strategy ?? "meaning_reframe";
  if (/wedding|relationship|love|family|memory|inside joke/.test(text)) return candidates.find((candidate) => candidate.strategy === "private_meaning")?.strategy ?? candidates[0]?.strategy ?? "meaning_reframe";
...
function makeOperatorMix(chosen: string, round: number, candidates: AttentionCandidate[]): string[] {
  const secondary = candidates.find((candidate) => candidate.strategy !== chosen && candidate.score >= 68)?.strategy;
...
export function buildAuthorCognitivePlan(input: AuthorCognitionInput): AuthorCognitivePlan {
  ...
  const selectedFrame = selectFrame(characterRead.creativeFrames, input, contradictions);
  ...
  const frameSummary = characterRead.creativeFrames.length
    ? `FRAME CANDIDATES: ${characterRead.creativeFrames.slice(0, 5).map((frame) => `${frame.frame}=${frame.confidence}`).join(" | ")}. Selected lens: ${selectedFrame.frame}. A frame is a lens, never the story.`
  ...
  const authorBrief = [
    ...
    `CHARACTER TRAITS: ${characterRead.coreTraits.join(" | ") || "derive from observed behavior"}.`,
    `OBJECT RELATIONSHIPS: ${characterRead.objectRelationships.join(" | ") || "none explicit"}.`,
    `CONTRADICTIONS: ${contradictions.join(" | ") || "none detected; use tension from supplied relationships without inventing facts"}`,
  ...
  return {
    ...
    chosenAttentionStrategy: `${chosen}; FRAME LENS = ${selectedFrame.frame}`,
    operatorMix,
    callbackTargets,
    antiRepetitionRules,
    sceneRules,
