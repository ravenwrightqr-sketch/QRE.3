 ...
 *
 * Cognition discovers grounded meaning from what happened. Meaning Pressure
 * explains why that material has experiential charge. The Realizer discovers how to embody it.
 ...
 *
 * The rendered product is a QRE readout: moving screen text where each cut is one attention beat.
...
function context(input: { prompt: string; subject: string; lens: string; graph: RealityGraph; movie: LatentMovieCandidate; creativeLensBrief?: CreativeLensBrief; domainContext?: AuthorDomainContext; memoryContext?: string[]; priorScenes?: string[]; creativeLearningContext?: string[] }, repairFeedback: string) {
  ...
  return {
    ...
    selectedStructure: {
      ...
      accumulatedMeaning: input.movie.hypothesis,
      operations: input.movie.trajectory.map((step) => ({
        order: step.order,
    ...
    artistRule: "Preserve semantic truth, never the client's sentence. The selected structure carries the accumulated meaning; the entire availableReality list remains available as grounded material. Use supplied details only when they advance that meaning. Every literal world detail must remain faithful to the supplied world. Figurative language may freely bend framing without asserting that the figurative imagery literally happened.",
...
function prompt(attempt: number, feedback: string): string {
  ...
  return [
    ...
    "You are creating a QRE readout in moving screen text, not a screenplay, article, caption reel, summary, or shot list.",
    "The source facts are sacred. The source sentences are disposable.",
    "The selected experience structure is the semantic spine. The entire supplied RealityGraph is your expressive palette.",
    "The selected experience structure does NOT limit the material you may use. Hunt the whole reality for the detail that makes the readout click. Use any supplied detail when it makes the experience stronger.",
    ...
    "PERFORM THE ACCUMULATED REALIZATION. The selected structure includes accumulatedMeaning and viewerChange. Those are the semantic handoff. Make that changing perception felt in the readout without explaining it. The lens may pressure how it feels; it must not replace the meaning with genre vocabulary.",
    ...
    "ENDING FREEDOM: stop when the experience is complete. Do not append explanation, moral, generic closing copy, or a manufactured ending.",
    "The final output is a sequence of QRE readout beats. ONE BEAT = ONE SCREENFUL OF ATTENTION.",
    "Do not interpret 'one beat' as one fact or one sentence. Several facts may share one beat when their collision, compression, contrast, timing, or juxtaposition creates the meaning. Split them when separation creates the fire. The Artist chooses where the screen changes.",
    "A beat can be one word, a fragment, a sentence, or several clauses. Length is not a quality signal. Use whatever amount of language preserves the realization without explaining past it.",
    ...
    "Do not force every fact into the film. Do not force a fixed beat count. Two brutal beats can beat ten dead beats. Rich reality may deserve more beats, but richness must come from new experiential movement, not longer sentences.",
    ...
    "CREATE A READOUT WORTH EXPERIENCING. The goal is experiential compression from reality, not sanitized summary.",
    ...
    "Generate four genuinely different candidate readouts. Change the idea, rhythm, ordering, compression, point of view, joke, metaphor, callback or structure—not merely adjectives. Candidates are exploration, not compliance variants. Candidate four should be the readout you would actually ship if it remains truthful.",
    "Use however many beats the accumulated realization actually needs. Do not optimize for a product surface, tag size, fixed count, or shortness.",
