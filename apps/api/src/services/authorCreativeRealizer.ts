...
const ACTION_WORD = /\b(?:arrived?|came|left|went|met|talked?|spoke|said|gave?|got|found|lost|cleaned?|finished?|started?|opened|closed|walk(?:ed)?|ran|drove?|ate|drank|kiss(?:ed)?|married|celebrated|played|worked|visited|bought|sold|built|fixed|painted|wore|used|stayed|waited|called|laughed|cried|looked|felt|became|changed|repaired|tested|selected|cut|shaped|polished|delivered|welcomed|checked|booked|reserved|approved|groomed|dyed|tailored|installed|stole|returned|rescued|adopted|remembered|watched|heard|sang|danced)\b/i;
...
function unsupportedContextActor(text: string, subject: string, graph: RealityGraph): boolean { const actor = leadingContextActor(text); if (!actor) return false; if (new RegExp(`^${CONTEXT_CHARACTER.source.replace(/^\\b|\\b$/g, "")}$`, "i").test(clean(subject))) return false; return true; }
