...
const STATUS_METAPHOR =
  /\b(?:lawyer|ceo|boss|diva|celebrity|negotiator|negotiation|case|trial|court|verdict|crime|criminal|suspect|evidence|trophy|queen|king|royalty|hostage|rebel|rebellion|legend|star|promotion|resignation|contract|deal|terms|victory|undefeated|upper hand|in charge|calling the shots|made the rules)\b/i;
...
function deriveBeatJob(
  ...
  switch (attention) {
    case "hook":
      return "establish the sharpest supplied tension or character detail";
    case "question":
      return "preserve an earned unresolved want without asking literally";
    case "turn":
    case "reframe":
      return "change the meaning of something already established";
    case "escalation":
      return "increase supplied consequence, pressure, status tension, or uncertainty";
    case "callback":
    ...
    default:
      return "express the selected beat as a concrete, subject-specific shift";
  }
}
...
function groundingAnchors(
  ...
  const labels = [
    ...(Array.isArray(beat.setsUp)
      ? beat.setsUp.map(clean)
      : []),
    ...(Array.isArray(beat.paysOff)
      ? beat.paysOff.map(clean)
      : []),
  ].filter(Boolean);
...
function groundBeatForMouth(
  ...
  const source = sourceTerms(
    evidence,
  );

  const directSourceOverlap =
    overlap(
      rawChange,
      source,
    );
  ...
  return {
    ...
    sourceGrounding: {
      directSourceOverlap: Number(
        directSourceOverlap.toFixed(3),
      ),
      restatesAnchor,
    },
    realizationDirective:
      restatesAnchor ||
      directSourceOverlap >= 0.8
...
export function mouthCraftSystem(
  risk: string,
): string {
  return [
    ...
    "The approved sequence is ONE CUMULATIVE MOVIE, not a stack of independent captions.",
    "Each new line must inherit something from the previous line: a state, contradiction, object, status shift, expectation, or meaning. Then it must change that inherited thing.",
    ...
    "Use temporal or causal connective language when the approved source sequence genuinely supports it. Do not force connectors merely for style.",
    ...
    "For turn, reframe, callback, and payoff beats, create a genuine relationship between supplied signals when the source supports that collision.",
    ...
    "Do not use generic praise such as amazing, perfect, powerful, magical, special, memorable, journey, transformation, or cinematic unless the exact supplied source makes the word materially meaningful.",
    ...
    "The result should feel discovered, not generated from a caption template.",
...
export function mouthQualityPenalty(
  text: string,
): number {
  ...
  if (BODY_OR_REACTION.test(value)) {
    penalty += 0.15;
  }
