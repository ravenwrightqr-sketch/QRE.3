...
export function mouthCraftSystem(risk: string): string {
  return [
    ...
    "Silently draft several radically different realizations, including one direct, one compressed, one status-driven, and one bolder lens-based option when the material supports it. Choose the strongest grounded line.",
    ...
    "PAYOFF CRAFT: when the approved endpoint is a state such as happy, proud, relieved, fabulous, finished, upgraded, safe, home, or complete, do not default to a literal body reaction. Look for the strongest compressed status, send-off, verdict, identity shift, reversal, or afterimage that the supplied evidence already supports.",
    "PAYOFF CRAFT: the final cut may be radically shorter than the endpoint source phrase. Treat the endpoint as semantic truth, not a wording requirement.",
    "PAYOFF CRAFT: a strong ending can feel like a stamp, exit, title card, mic drop, tiny joke, earned label, or final attitude. It must still mean what the supplied endpoint means.",
    "PAYOFF CRAFT: do not invent a physical reaction to communicate an emotional state. 'happy' does not authorize smiling, laughing, wagging, dancing, sighing, or any other body event unless supplied.",
