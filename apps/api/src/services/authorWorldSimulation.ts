...
function questionFor(relation: WorldRelation): WorldQuestion | undefined {
  const typeByRelation: Record<string, WorldQuestionType> = {
    causes: "causal", recontextualizes: "meaning", repeats: "return", changes: "consequence",
    contrasts_with: "relational", converges_with: "meaning", precedes: "temporal", follows: "temporal", interacts_with: "relational",
  };
  const type = typeByRelation[relation.kind];
  if (!type) return undefined;
  const from = relation.from.label;
  const to = relation.to.label;
  const text = relation.kind === "recontextualizes" ? `What does ${from} make newly meaningful about ${to}?`
    : relation.kind === "repeats" ? `Why does ${from} return here?`
    : relation.kind === "changes" ? `What changed between ${from} and ${to}?`
    : relation.kind === "causes" ? `What follows from ${from} and ${to}?`
    : `What connects ${from} with ${to}?`;
  return { id: `world-question:${relation.id}`, type, text, openedByEventIds: unique(relation.evidenceEventIds), supportedByRelationIds: [relation.id], pressure: metric(relation.strength * (relation.kind === "recontextualizes" ? 1 : 0.84)), resolved: false, resolutionEventIds: [] };
}

function inferExpectation(question: WorldQuestion, relation: WorldRelation): ViewerExpectation {
  return { id: `expectation:${question.id}`, proposition: question.text, basisEventIds: [...question.openedByEventIds], basisRelationIds: [relation.id], confidence: metric(question.pressure * 0.86), horizon: question.type === "return" ? "episode" : "near", violatedByEventIds: [], fulfilledByEventIds: [] };
}

function hypothesisFor(question: WorldQuestion, relation: WorldRelation): ViewerHypothesis {
  return { id: `hypothesis:${question.id}`, interpretation: question.text, supportingEventIds: [...question.openedByEventIds], supportingRelationIds: [relation.id], contradictingEventIds: [], confidence: metric(question.pressure * 0.72), status: "forming", competingHypothesisIds: [] };
}
...
function predictionErrorFor(expectation: ViewerExpectation, observedEventId: string, observedLabel: string, magnitude: number): ViewerPredictionError {
  return { id: `prediction-error:${expectation.id}:${observedEventId}`, expectationId: expectation.id, observedEventId, expected: expectation.proposition, observed: observedLabel, magnitude: metric(magnitude), causesModelUpdate: magnitude >= 0.58 };
}
...
function lensPressure(label: string | undefined): LensPressure | undefined {
  ...
  if (!preset) return { id: `lens:${normalized}`, label: normalized, attentionPriorities: ["novelty", "relationship_change", "recurrence", "salience"], preferredRelations: ["changes", "contrasts_with", "recontextualizes", "repeats", "converges_with"], preferredQuestionTypes: ["meaning", "relational", "consequence", "prediction"], realizationMoves: ["compression", "implication", "callback"], statusFrames: ["contextual"], juxtapositionModes: ["detail_against_context"], permissiblePersonification: "light", explanationPressure: 0.08, intensity: 0.55 };
...
function opportunities(graph: RealityGraph, relations: WorldRelation[], questions: WorldQuestion[], lens?: LensPressure): InterpretationOpportunity[] {
  ...
  for (const relation of strongestRelations(relations, 0.5)) {
    ...
    bucket.push(relation); grouped.set(key, bucket);
  }
  return [...grouped.values()].slice(0, 24).map((bucket, index) => {
    ...
    return { id: `opportunity:${index + 1}`, hypothesis: `The supplied constellation may be read through ${relationText}.`, relationIds: bucket.map((relation) => relation.id), eventIds: ids, questionIds, compatibleLenses: lens ? [lens] : [], inferenceDistance: metric(0.35 + compatibilityScore.grounding * 0.35 + (bucket.length > 1 ? 0.2 : 0)), grounding: compatibilityScore.grounding, novelty: metric(Math.min(1, 0.32 + bucket.length * 0.11 + compatibilityScore.lensFit * 0.24)), ambiguity: metric(bucket.length > 1 ? 0.72 : 0.46), continuationPotential: metric(Math.max(...bucket.map((relation) => relation.kind === "repeats" || relation.kind === "recontextualizes" ? 0.82 : 0.42))) };
  });
}

function changedDimensions(value: string): WorldSnapshot["changedDimensions"][number] { return value as WorldSnapshot["changedDimensions"][number]; }
...
function snapshots(graph: RealityGraph, refs: WorldRef[], relations: WorldRelation[], questions: WorldQuestion[]): WorldSnapshot[] {
  return graph.events.map((current, index) => {
    ...
    return { id: `snapshot:${index + 1}`, eventIds: priorIds, activeRefs, activeRelations, changedDimensions: [...changed], stableDimensions: ["participants", "identity"].filter((dimension) => activeRefs.length > 0) as WorldSnapshot["stableDimensions"], unresolved: questions.filter((question) => question.openedByEventIds.some((id) => priorIds.includes(id)) && !question.resolved), timestampLabel: current.time };
  });
}
...
export function buildAuthorWorldSimulation(input: WorldSimulationBuildInput): WorldSimulation {
  ...
  const questions = strongestRelations(relations).map(questionFor).filter((question): question is WorldQuestion => Boolean(question)).slice(0, 24);
  ...
  for (const expectation of expectations.slice(0, 8)) {
    const observed = expectation.basisEventIds[expectation.basisEventIds.length - 1];
    ...
    const error = predictionErrorFor(expectation, observed, observedLabel, magnitude); predictionErrors.push(error);
  ...
  const viewer: ViewerSimulationState = { known: refs.filter((ref) => ref.kind === "event").slice(0, 12), expectations, hypotheses, unresolvedQuestions, predictionErrors, updates, attentionField: attentionField(refs, relations, unresolvedQuestions), confidence: metric(0.42 + relations.length / Math.max(1, reality.events.length * 2)) };
  ...
  for (let index = 0; index < Math.min(12, snapshotList.length); index += 1) {
    ...
    const eventId = snapshot.eventIds[snapshot.eventIds.length - 1];
    if (!eventId) continue;
    ...
    const role = relevantRelations.some((relation) => relation.kind === "recontextualizes") ? "recontextualize" : relevantRelations.some((relation) => relation.kind === "contrasts_with") ? "prediction_error" : relevantQuestions.length ? "question" : index === snapshotList.length - 1 ? "payoff" : index === 0 ? "establish" : "notice";
    cutObjectives.push({ id: `cut-objective:${index + 1}`, role, sourceEventIds: [eventId], sourceRelationIds: relevantRelations.map((relation) => relation.id), viewerBefore: prior ? viewerStateAt(prior, viewer, questions) : viewer, viewerAfter: viewerStateAt(snapshot, viewer, questions), desiredInference: relevantQuestions[0]?.text, forbiddenExplanation: relevantQuestions.length ? relevantQuestions[0]?.text : undefined, nextQuestion: relevantQuestions[0]?.text });
  }
  const hasSuppliedRecurrence = reality.recurringSignals.length > 0;
  const durableThreads = questions.filter((question) => question.type === "return" || question.type === "meaning" || question.pressure >= 0.7 || (hasSuppliedRecurrence && question.type === "relational")).slice(0, 8);
  const remembered = input.rememberedRefIds ?? [];
  return { version: 1, reality, refs, relations, snapshots: snapshotList, questions, interpretationOpportunities, viewer, lensPressure: lens, cutObjectives, durableThreads, reentry: { priorExperienceIds: input.priorExperienceIds ?? [], rememberedRefIds: remembered, changedContextRefIds: unique(snapshotList.slice(-2).flatMap((snapshot) => snapshot.eventIds)), eligibleCallbacks: durableThreads.map((question) => question.id), meaningCanChange: Boolean((input.priorExperienceIds?.length ?? 0) || remembered.length) } };
}
...
function viewerStateAt(snapshot: WorldSnapshot, base: ViewerSimulationState, questions: WorldQuestion[]): ViewerSimulationState {
  ...
  return { ...base, known, unresolvedQuestions, hypotheses: activeHypotheses, predictionErrors, updates: base.updates.filter((update) => update.triggerEventIds.some((id) => ids.has(id))), attentionField: base.attentionField.filter((focus) => focus.ref.sourceEventIds.some((id) => ids.has(id))), currentSnapshotId: snapshot.id, dominantHypothesisId: activeHypotheses.sort((a, b) => b.confidence - a.confidence)[0]?.id, confidence: metric(0.35 + known.length / Math.max(1, snapshot.eventIds.length + 1) * 0.4 + activeHypotheses.length * 0.03) };
}
