...
function observationCandidates(g: RealityGraph, subject: string, returning: boolean): LatentMovieCandidate[] {
  return g.events.slice().sort((a,b)=>Number(Boolean(b.salient))-Number(Boolean(a.salient))).slice(0,4).map((e,i)=>{
    const c:LatentMovieCandidate={id:`universal-observation-${e.id}`,lens:"NONE",anchorEventIds:[e.id],supportingRelationKinds:[],trajectory:[{order:1,operation:returning?"recur":"establish",eventIds:[e.id],viewerChange:"hold the supplied detail in focus",nextQuestion:"What does another supplied detail make newly noticeable?"}],payoff:e.label,unresolvedQuestion:"What deserves another look?",evidence:[e.label],hypothesis:[`${subject}: ${e.label} is distinctive enough to carry the experience without invented plot.`],truthRisk:0,novelty:.52+i*.05,specificity:.95,informationValue:.62,uncertainty:.2,attentionPotential:.72,consequencePotential:.25,callbackPotential:returning?.8:.1,compressionPotential:.9,repetitionRisk:.08,distinctiveness:.86,score:0};
...
function operationForRelationKind(kind: string): LatentMovieTrajectoryStep["operation"]|undefined {
  switch(kind){case "contrasts":return "contrast";case "changes":return "consequence";case "converges":return "converge";case "recontextualizes":return "reframe";case "repeats":return "recur";case "involves":return "reframe";default:return undefined;}
}
function deriveCutTrajectory(cuts: unknown[], g: RealityGraph): { ids:string[]; trajectory:LatentMovieTrajectoryStep[]; relationKinds:string[] }|undefined {
  ...
  const allIds=unique(normalized.flat()); if(allIds.length<2)return undefined;
  const first=normalized[0]![0]; if(!first)return undefined;
  ...
  for(let i=1;i<normalized.length;i+=1){const current=normalized[i]!;const pair=current.find(id=>id!==previousIds[previousIds.length-1])??current[0];const previous=previousIds[previousIds.length-1];if(!pair)continue;const relation=g.relations.find(r=>(r.from===previous&&r.to===pair)||(r.from===pair&&r.to===previous));
    if(relation){const operation=operationForRelationKind(relation.kind);if(operation){relationKinds.push(relation.kind);trajectory.push({order:trajectory.length+1,operation,eventIds:unique([previous,pair]),viewerChange:`the supplied relationship ${relation.kind} changes what is worth noticing`,nextQuestion:"What remains after that change?"});}}
    else trajectory.push({order:trajectory.length+1,operation:"reveal",eventIds:[pair],viewerChange:"introduce another supplied detail",nextQuestion:"What does the combination make visible?"});
    previousIds=unique([...previousIds,...current]);
  }
  if(!trajectory.some(step=>step.eventIds.length>=2))return undefined;
  const last=previousIds.at(-1)!; trajectory.push({order:trajectory.length+1,operation:"payoff",eventIds:[last],viewerChange:"land the changed reading without adding a new event",nextQuestion:"What lingers after the supplied details separate?"});
...
function normalizeModel(raw: unknown,g: RealityGraph,returning:boolean): LatentMovieCandidate[] {
  ...
  return rows.slice(0,8).flatMap((x,i)=>{if(!x||typeof x!=="object")return[];const r=x as Record<string,unknown>;const explicitIds=validIds(r.evidenceEventIds??r.evidenceIds??r.anchorEventIds,g);const canonical=normalizeCanonicalTrajectory(r.trajectory,g);const cutDerived=canonical.length?undefined:deriveCutTrajectory(Array.isArray(r.cuts)?r.cuts:[],g);const ids=unique(explicitIds.concat(cutDerived?.ids??canonical.flatMap(s=>s.eventIds)));const trajectory=canonical.length?canonical:(cutDerived?.trajectory??[]);const thesis=clean(r.thesis??(Array.isArray(r.hypothesis)?r.hypothesis[0]:undefined));
    if(g.events.length&&(!ids.length||trajectory.length<2||!trajectory.some(step=>step.eventIds.length>=2)))return[]; if(thesis&&(GENERIC.test(thesis)||PSYCH.test(thesis)||INTERNAL.test(thesis)))return[];
...
export async function buildAuthorCognitivePlan(input: AuthorCognitionInput): Promise<AuthorCognitionPlan>{const returning=Boolean(input.returning||(input.visitNumber??1)>1),explicit=clean(input.lens);const intelligence=buildAuthorCognitionIntelligence(input.realityGraph,returning,input.creativeLearningContext??[]);const compact={subject:clean(input.subject)||"unknown",place:clean(input.place)||"unknown",prompt:clean(input.prompt),returning,memory:(input.memoryContext??[]).slice(0,20),learning:(input.creativeLearningContext??[]).slice(0,20),events:input.realityGraph.events.slice(0,10).map(e=>({id:e.id,label:e.label,salient:Boolean(e.salient),place:e.place,time:e.time})),relations:input.realityGraph.relations.slice(0,20).map(r=>({from:r.from,to:r.to,kind:r.kind,strength:r.strength})),patterns:(input.realityGraph.patterns??[]).slice(0,12),tensions:(input.realityGraph.unresolvedTensions??[]).slice(0,8),sensory:(input.realityGraph.sensorySignals??[]).slice(0,12)});let parsed:Record<string,unknown>|undefined;let model="deterministic";let modelCalls=0;
