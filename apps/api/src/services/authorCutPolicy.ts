...
export type CutPolicyResult = {
  ...
  metrics: {
    ...
    memoryReentry: number;
    memoryGenericityPenalty: number;
  };
};
...
const MEMORY_GENERIC = /\b(?:the night|the event|the experience|the memory|alive|magical|electric|vibrant|epic|unforgettable|special|amazing|beautiful|incredible|living thing|bass|music|dance floor|friends dancing)\b/i;
const SENSORY = /\b(?:smell|scent|taste|flavor|warm|cold|heat|chill|wind|air|light|dark|neon|glow|sweat|skin|hands|voice|laugh|sound|silence|texture|rough|soft|sticky|wet|dry|dust|rain|salt|smoke|perfume|sugar|coffee|whistle|hum|buzz|vibration|treble|bass)\b/i;
const PERSONALITY = /\b(?:apparently|finally|again|still|already|only|just|never|always|mine|ours|me|damn|hell|wild|ridiculous|snazzy|proud|swagger|fear|smear)\b/i;
...
function isMemoryWorld(world: CutWorld): boolean {
  const corpus = [world.prompt, ...(world.memory ?? []), ...(world.trajectory ?? []), ...(world.moments ?? [])].filter(Boolean).join(" ").toLowerCase();
  return /\b(?:memory|memorial|remember|remembered|chapter|living memory|memory tag|keep this moment|relive|reliving)\b/.test(corpus);
}

function memoryReentryScore(text: string, world: CutWorld, priorCuts: readonly string[]): { score: number; genericityPenalty: number } {
  if (!isMemoryWorld(world)) return { score: 0, genericityPenalty: 0 };
  const sensory = SENSORY.test(text) ? 0.35 : 0;
  const personality = PERSONALITY.test(text) ? 0.25 : 0;
  const grounded = groundedTokenRatio(text, world) * 0.25;
  const novelty = noveltyScore(text, priorCuts) * 0.15;
  const genericityPenalty = MEMORY_GENERIC.test(text) ? 0.55 : 0;
  const score = Math.max(0, Math.min(1, sensory + personality + grounded + novelty - genericityPenalty));
  return { score, genericityPenalty };
}
...
export function evaluateCut(
  ...
  const memory = memoryReentryScore(text, world, priorCuts);
  ...
  if (isMemoryWorld(world) && memory.score < 0.22 && wordCount > 2) reasons.push("memory-generic");
  ...
  return {
    ...
    metrics: {
      ...
      memoryReentry: Number(memory.score.toFixed(3)),
      memoryGenericityPenalty: Number(memory.genericityPenalty.toFixed(3)),
    },
  };
}
