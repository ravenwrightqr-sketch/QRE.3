...
const SUBJECT_REFERENCE = /\b(?:he|she|they|it|him|her|them|his|their|its)\b/i;
...
function groundedTokenRatio(text: string, world: CutWorld): number {
  ...
  const sources = sourceText(world).map(contentWords);
  return words.filter((word) => sources.some((source) => source.includes(word))).length / words.length;
}
...
function noveltyScore(text: string, priorCuts: readonly string[]): number {
  ...
  return [...current].filter((word) => !prior.has(word)).length / current.size;
}
...
function semanticDensity(text: string): number {
  ...
  if (PHYSICAL_ACTION.test(text)) value += 0.15;
  if (/\b(?:again|already|still|yet|only|even|back|different|same|but|except|until|before|after)\b/i.test(text)) value += 0.15;
  if (GENERIC_EMOTION.test(text)) value -= 0.05;
  return metric(value);
}
...
function factRestatement(text: string, world: CutWorld): number {
  ...
  return sourceText(world).map((item) => contentWords(item).join(" ")).includes(normalized) ? 1 : 0;
}
...
function groundedCharacterInterpretation(text: string, intent: CutIntent): boolean {
  const signal = characterGroundingSignal(intent);
  if (!signal) return false;
  const interpretiveVocabulary = /\b(?:rebel|rebellion|defiant|defiance|resist|resistance|negotiate|negotiation|negotiator|lawyer|boss|ceo|diva|queen|king|royalty|terms|deal|contract|case|evidence|operation|mission|status|power|upper hand|called the shots|in charge|guarded|attitude|vulnerable|personality|mini[- ]?rebel|tiny rebel)\b/i;
  const cognitiveAnchor = /\b(?:rebel|rebellion|defian|resistan|negotiat|status|power|operation|mission|guarded|attitude|vulnerab|routine|personality|character)\b/i;
  return interpretiveVocabulary.test(text) && cognitiveAnchor.test(signal);
}

function terminalLanding(intent: CutIntent, text: string): boolean {
...
export function evaluateCut(
  ...
  const interpretiveCharacterGrounding = groundedCharacterInterpretation(text, intent);
  const interpretationAllowed = !PHYSICAL_ACTION.test(text) && ((groundedRelationship && (statusMetaphor || CHARACTER_INTERPRETATION.test(text))) || interpretiveCharacterGrounding);
  const groundingFloor = interpretationAllowed ? 0 : statusMetaphor ? 0.05 : 0.1;
