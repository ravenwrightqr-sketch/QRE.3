...
function qualityFailures(texts: readonly string[], candidates: readonly MouthCandidate[], beatCount: number, beamScore: number): string[] {
  ...
  for (const candidate of candidates) {
    ...
    if (candidate.reasons.includes("language-quality-gate") || candidate.reasons.includes("weak-natural-language") || candidate.reasons.includes("keyword-assembly") || candidate.reasons.includes("analytic-language") || candidate.reasons.includes("analytic-realization-language")) {
...
function mergeCandidateBatches(beats: readonly MouthCandidateBeat[], primary: MouthCandidateBatch | undefined, recovered: ReadonlyMap<number, string[]>, limit: number): MouthCandidateBatch {
  return {
    variantsByBeat: beats.map((beat) => {
      ...
      return {
        order: beat.order,
        variants: [...(primaryEntry?.variants ?? []), ...recoveredVariants]
...
function latentStoryDirectives(input: {
  ...
  const { envelope, beats, spine } = input;
  const directives: string[] = [];
  const endpoint = spine.endpointLabel;

  directives.push("Find the latent story already contained in the supplied evidence; do not write a receipt caption for each event.");
  directives.push("Each middle beat must make the selected movie more legible by changing the reading of earlier evidence or using a later supplied detail as concrete proof.");
  directives.push("A supplied state can be interpreted as character attitude, but the interpretation must be grounded by a supplied action, object, or later state when the graph provides one.");
  directives.push("Do not name the operation. Make the relationship felt through the line.");

  for (let index = 1; index < beats.length - 1; index += 1) {
    const current = beats[index];
    const previous = beats[index - 1];
    const currentLabels = current.eventIds?.map((id) => eventLabel(envelope, id)).filter(Boolean) ?? [];
    const nextLabels = beats[index + 1].eventIds?.map((id) => eventLabel(envelope, id)).filter(Boolean) ?? [];
    const priorLabels = previous.eventIds?.map((id) => eventLabel(envelope, id)).filter(Boolean) ?? [];
    const state = currentLabels.find((label) => envelope.suppliedStates.includes(label));
    const action = nextLabels.find((label) => envelope.suppliedActions.some((value) => label.toLowerCase().includes(value.toLowerCase())));
    if (state && action) {
      directives.push(`Beat ${current.order}: ${state} is the emerging attitude; the next supplied action "${action}" must make that attitude concrete. Do not merely place the two labels beside each other.`);
    } else if (priorLabels.length && currentLabels.length) {
      directives.push(`Beat ${current.order}: carry the prior signal ${priorLabels.join(" / ")} into ${currentLabels.join(" / ")} so the viewer feels a change, not a list.`);
    }
  }

  if (endpoint) {
    directives.push(`FINAL SCENE END IS NON-NEGOTIABLE: the final line must end on the supplied endpoint "${endpoint}". Do not replace it, weaken it, add a fifth beat, or continue after it.`);
    directives.push(`Everything before the endpoint exists to make "${endpoint}" land as the earned payoff of the accumulated meaning.`);
  }
...
function applyLatentStoryPenalty(candidate: MouthCandidate): MouthCandidate {
  ...
  return {
    ...candidate,
    score: Number(Math.max(0, candidate.score * 0.58).toFixed(3)),
    meaningScore: Number(Math.max(0, candidate.meaningScore * 0.72).toFixed(3)),
    reasons: [...new Set([...candidate.reasons, "latent-story-scaffold"])],
  };
}
...
async function recoverMissingBeatVariants(
  ...
  if (!missingBeats.length || policy.maxRecoveryCalls <= 0 || policy.mode === "no-model") return { recovered, modelCalls: 0 };
  ...
  for (const beat of missingBeats) {
    ...
    if (entry?.variants.length) recovered.set(beat.order, entry.variants.slice(0, policy.variantsPerBeat));
  }
...
async function generateBeam(
  ...
  if (policy.mode === "no-model" || policy.maxPrimaryCalls <= 0) {
    return { resultText: JSON.stringify({ variantsByBeat: [] }), texts: [], candidates: [], beamScore: 0, modelCalls: 0 };
  }
...
export async function realizeEnterpriseMouth(input: EnterpriseMouthInput): Promise<EnterpriseMouthResult> {
  ...
  const realizationSlots = buildRealizationSlots({ envelope, beats: canonicalBeats, spine: meaningSpine, fast: policy.mode === "dev-fast" });
  const enterpriseIntelligence = buildEnterpriseIntelligence({ graph: input.graph, subject: input.subject, lens: input.lens, beats: canonicalBeats });

  if (policy.mode === "no-model") {
    const cumulativeStates = buildCumulativeMeaningState(canonicalBeats, envelope);
    return {
      texts: [], candidates: [], rawModelText: JSON.stringify({ variantsByBeat: [] }), beamScore: 0,
      envelope, meaningSpine, realizationSlots, repairObjectives: [], policy, modelCallCount: 0,
      enterpriseIntelligence, cumulativeMeaningScore: evaluateCumulativeMeaning(cumulativeStates),
      creativeCritique: critiqueCreativeSelection("", []), safetyViolations: [], groundedSurprise: 0,
    };
  }
  ...
  const repairObjectives = buildMouthRepairObjectives({ candidates: current.candidates, slots: realizationSlots });
  ...
  const safetyViolations = [...new Set(current.texts.flatMap((text) => detectAuthorSafetyViolations({ text, envelope })))];
  const cumulativeStates = buildCumulativeMeaningState(canonicalBeats, envelope);
