    ...
    try {
      const retry = await localModelGenerate([
        ...messages,
        {
          role: "user",
          content: [
            "Re-realize the SAME approved sequence. Do not author a new one.",
            "The previous language attempt either stayed too close to source wording or introduced concrete reality that was not supplied.",
            "Do not revise or imitate the failed wording.",
            "Keep every approved cut, its order, and its function exactly as supplied in approvedSequence.",
            `Each realization must contain exactly ${beats.length} lines, one line for each approved cut in order.`,
            "Do not add, remove, merge, split, or reorder cuts.",
            "Your freedom is only in final expression: implication, rhythm, attitude, voice, metaphor, contrast, compression, emphasis, and phrasing.",
            "Concrete vocabulary belongs to supplied reality. Do not add new physical nouns, actions, places, people, sensory details, chronology, or events.",
            "Preferences may become wanting, anticipation, attitude, fixation, priority, interruption, or voice without becoming events.",
            "Use source words only when their placement changes meaning; do not enumerate the source.",
            "Make each line perform its approved attention function, then stop when the approved payoff lands.",
            "Create three different language realizations of the same approved sequence.",
            "Separate realizations with a line containing only ---. Return nothing else.",
          ].join(" "),
        },
