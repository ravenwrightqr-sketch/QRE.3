...
function elapsedMs(startedAt: number): number { return Date.now() - startedAt; }
...
function isTransportError(error: unknown): boolean {
  ...
  if (code && ["UND_ERR_HEADERS_TIMEOUT","UND_ERR_BODY_TIMEOUT","UND_ERR_CONNECT_TIMEOUT","UND_ERR_SOCKET","UND_ERR_DESTROYED","ECONNRESET","ECONNREFUSED","EPIPE","ETIMEDOUT"].includes(code)) return true;
...
async function request(path: string, body: LocalRequestBody, modelOverride?: string): Promise<unknown> {
  const controller = new AbortController();
  const timeout = timeoutMs();
  const startedAt = Date.now();
  const timer = setTimeout(() => { console.log("QRE LOCAL MODEL TIMEOUT FIRING", `after=${timeout}ms`); controller.abort(); }, timeout);
  ...
  try {
    ...
    console.log("QRE REQUEST NUM_PREDICT:", body.options.num_predict);
    ...
    console.log("QRE REQUEST CONTENT CHARS:", body.messages.reduce((total, message) => total + message.content.length, 0));
    console.log("QRE REQUEST TIMEOUT:", timeout);
    console.log("QRE REQUEST HEADERS TIMEOUT:", headersTimeoutMs());
    console.log("QRE REQUEST BODY TIMEOUT:", bodyTimeoutMs());
    console.log("QRE FETCH ENTER");
    const response = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: serializedBody, signal: controller.signal, dispatcher: getDispatcher() } as RequestInit & { dispatcher?: unknown });
    console.log("QRE FETCH RETURNED");
    console.log("QRE RESPONSE STATUS:", response.status);
    console.log("QRE TIME TO HEADERS MS:", elapsedMs(startedAt));
    if (!response.ok) {
    ...
    if (typeof json === "object" && json !== null) {
      ...
      console.log("QRE RESPONSE DONE_REASON:", record.done_reason);
      console.log("QRE RESPONSE PROMPT_EVAL_COUNT:", record.prompt_eval_count);
      console.log("QRE RESPONSE EVAL_COUNT:", record.eval_count);
      console.log("QRE RESPONSE EVAL_DURATION_NS:", record.eval_duration);
    }
    console.log("QRE RESPONSE JSON RECEIVED");
    console.log("QRE REQUEST TOTAL MS:", elapsedMs(startedAt));
    return json;
  } catch (error) {
    console.log("QRE LOCAL REQUEST ERROR:", error);
    if (isTransportError(error)) {
      const code = errorCode(error);
      console.log("QRE LOCAL REQUEST TRANSPORT FAILURE", `code=${code ?? "unknown"}`, `elapsedMs=${elapsedMs(startedAt)}`);
      if (["UND_ERR_SOCKET","UND_ERR_DESTROYED","ECONNRESET","EPIPE"].includes(code ?? "")) resetDispatcher();
    }
    throw error;
  } finally {
    clearTimeout(timer);
    console.log("QRE REQUEST FINISHED", `totalMs=${elapsedMs(startedAt)}`);
  }
}
...
export async function localModelGenerate(messages: LocalModelMessage[], format?: "json", options: LocalModelOptions = {}): Promise<LocalModelResult> {
  ...
  const requestBody: LocalRequestBody = {
    ...
    messages: messages.map((message) => ({ role: message.role, content: message.content, ...(message.images?.length ? { images: message.images.map(stripDataUrl) } : {}) })),
    options: { temperature: defaultTemperature(options), num_predict: defaultNumPredict(options) },
  };
  try {
    const data = await request("/api/chat", requestBody, primaryModel);
    const text = outputText(data);
    if (process.env.QRE_AUTHOR_DEBUG_RAW === "true") console.log("\n--- QRE RAW MODEL OUTPUT ---\n" + text + "\n--- END QRE RAW MODEL OUTPUT ---\n");
    return { text, model: primaryModel, provider: "local" };
  } catch (error) {
    if (!isTransportError(error) || !fallbackModel) throw error;
    console.log("QRE LOCAL MODEL FALLBACK", `primary=${primaryModel}`, `fallback=${fallbackModel}`, `code=${errorCode(error) ?? "unknown"}`);
    const data = await request("/api/chat", { ...requestBody, model: fallbackModel }, fallbackModel);
    const text = outputText(data);
    if (process.env.QRE_AUTHOR_DEBUG_RAW === "true") console.log("\n--- QRE RAW FALLBACK MODEL OUTPUT ---\n" + text + "\n--- END QRE RAW FALLBACK MODEL OUTPUT ---\n");
    return { text, model: fallbackModel, provider: "local" };
  }
}
...
export function localModelConfig() {
  ...
  return { provider: "local" as const, url: baseUrl(), model, fallbackModel: fallbackModelName(model), timeoutMs: timeoutMs(), headersTimeoutMs: headersTimeoutMs(), bodyTimeoutMs: bodyTimeoutMs(), connectTimeoutMs: connectTimeoutMs(), keepAlive: keepAlive() };
}
