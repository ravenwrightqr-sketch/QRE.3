...
const GRAMMAR = ["WRITE FOR THE SCREEN, NOT THE PAGE.","ONE SCENE = ONE SHORT THOUGHT = ONE PERCEIVABLE MOMENT.","THE SEQUENCE CARRIES THE STORY.","PREFER IMAGE OR ACTION OVER EXPLANATION.","LET EACH SCENE CHANGE THE VIEWER'S STATE.","PREFER IMPLICATION OVER SUMMARY.","END ON AN EARNED REVEAL, REVERSAL, TRANSFORMATION, OR AFTER-IMAGE."];
...
function infer(input: CinematicAuthorInput): Direction {
  const s=`${input.prompt} ${input.lens??""}`.toLowerCase();
  const has=(...r:RegExp[])=>r.some(x=>x.test(s));
...
async function plan(input:CinematicAuthorInput, fallback:Direction):Promise<Direction>{
  ...
  try{
    const r=await localModelGenerate([{role:"system",content:["You are QRE's senior creative director.","Plan; do not draft scenes.","Use the positive Whiskers grammar: sensory image → physical movement → small reveal → reframe/transformation → payoff.","Sparse creator/social/artist/person prompts are valid. Use aspiration, voice, desire, contradiction, tension, or point of view without inventing concrete life events.","Grounded mode permits language, metaphor, personification, framing and interpretation, not new concrete facts.","Return strict JSON: intent, attentionGoal, emotionalEngine, strongestDetail, sequenceShape, endingMove, targetDensity, avoid, selectedOperators.",`OPERATORS: ${OPERATORS.join(", ")}`].join(" ")},{role:"user",content:JSON.stringify({prompt:input.prompt,lens:input.lens??"",subject:input.subject??"",place:input.place??"",facts:uniq(input.facts,40),sourceMoments:uniq(input.sourceMoments,24),memoryContext:uniq(input.memoryContext??[],20),creativeLearningContext:uniq(input.creativeLearningContext??[],20),fallback})}],"json");
    const p=json<Direction>(r.text); if(!p?.sequenceShape?.length||!p.attentionGoal) return fallback;
    const ops=Array.isArray(p.selectedOperators)?p.selectedOperators.filter(x=>OPERATORS.includes(String(x))).slice(0,7):[];
    return {...fallback,...p,selectedOperators:ops.length?ops:fallback.selectedOperators};
  }catch{return fallback;}
}

function finalize(raw:AuthoredScene[]):AuthoredScene[]{
  const out:AuthoredScene[]=[];
  for(const scene of raw){ const t=clean(scene.text); if(!t||blocked(t)) continue; const parts=t.split(/(?<=[.!?])\s+(?=[A-Z0-9'"“])/).map(clean).filter(Boolean); for(const p of parts){const words=p.split(/\s+/); out.push({...scene,text:words.length<=18?p:words.slice(0,18).join(" ").replace(/[,:;—-]+$/,"")});}}
  return out.slice(0,20).map((s,i,all)=>({...s,kind:s.kind||["hook","movement","discovery","turn","payoff"][Math.min(i,4)],durationHintMs:s.durationHintMs??Math.max(1500,Math.min(5000,1100+s.text.split(/\s+/).length*150)),transitionHint:s.transitionHint??(i===0?"none":i===all.length-1?"cinematic":"fade")}));
}

async function draft(input:CinematicAuthorInput,d:Direction,sparse=false):Promise<AuthoredScene[]>{
  const min=3,max=d.intent==="creator"||d.intent==="social"||d.intent==="artist"||d.intent==="person"?5:d.targetDensity==="deep"?8:6;
  const rules=sparse?["You are QRE's sparse-brief rescue author.","Write 3–5 short cinematic scenes from aspiration, voice, desire, contradiction, tension or point of view.","Do not invent concrete biographical events, achievements, customers, locations, dates, products, reviews or other facts.","Creator = character/point of view. Social = stop-scroll micro-story. Artist = enter the artist's world. Person = human contradiction."]:["You are QRE's elite cinematic sequence author.",`Write ${min}–${max} separate viewer-facing scenes like a miniature film.","ONE SCENE = ONE SHORT THOUGHT = ONE PERCEIVABLE MOMENT.","Use the Whiskers grammar: sensory image → movement → reveal → reframe → payoff.","Prefer 4–14 words per scene; 15–18 only when earned.","Do not explain what the movie is doing. Make the line be the moment.","Every adjacent pair must create a new image, physical move, expectation, reveal, reversal, escalation, or emotional shift."];
  const r=await localModelGenerate([{role:"system",content:[...rules,"Do not mention QRE, AI, prompts, compilers, cognition, models, metadata or the writing process.","Avoid generic filler: beautiful, magical, unforgettable, amazing, cinematic, epic, picture-perfect, luxury, masterpiece, a transformation begins, the final reveal, as we move.",`SEQUENCE: ${d.sequenceShape.join(" → ")}`,`OPERATORS: ${d.selectedOperators.join(", ")}`,`ENDING: ${d.endingMove}`,"Return strict JSON: {\"scenes\":[{\"text\":\"...\",\"kind\":\"hook|movement|discovery|turn|payoff|afterglow\"}]}"].join(" ")},{role:"user",content:JSON.stringify({prompt:input.prompt,lens:input.lens??"",subject:input.subject??"",place:input.place??"",facts:uniq(input.facts,40),sourceMoments:uniq(input.sourceMoments,24),memoryContext:uniq(input.memoryContext??[],20),creativeLearningContext:uniq(input.creativeLearningContext??[],20)})}],"json");
  const p=json<SceneDraft>(r.text); return finalize(Array.isArray(p?.scenes)?p.scenes:[]).slice(0,max);
}

async function critique(input:CinematicAuthorInput,d:Direction,scenes:AuthoredScene[]):Promise<Critique|null>{
  try{const r=await localModelGenerate([{role:"system",content:"You are QRE's ruthless cinematic editor. Diagnose, do not rewrite. Judge visuality, movement, specificity, forward pull, novelty, grounding, transitions, generic language and payoff. Return strict JSON: score, problems, repeats, instructionLeaks, unsupportedDetails, weakScenes, genericLanguage, weakTransitions."},{role:"user",content:JSON.stringify({prompt:input.prompt,direction:d,facts:uniq(input.facts,40),scenes})}],"json");return json<Critique>(r.text);}catch{return null;}
}

async function revise(input:CinematicAuthorInput,d:Direction,scenes:AuthoredScene,c:Critique):Promise<AuthoredScene[]> { return scenes; }

export async function authorCinematicSequence(input:CinematicAuthorInput):Promise<AuthoredScene[]>{
  if(!enabled()) return [];
  const direction=await plan(input,infer(input));
  let scenes=await draft(input,direction,false);
  ...
  const needs=Boolean(c&&(c.score<8||c.problems?.length||c.repeats?.length||c.instructionLeaks?.length||c.unsupportedDetails?.length||c.weakScenes?.length||c.genericLanguage?.length||c.weakTransitions?.length));
  if(needs){ const repaired=await draft(input,direction,false); if(repaired.length>=3) scenes=repaired; }
