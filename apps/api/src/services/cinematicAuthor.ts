...
function finalize(raw:AuthoredScene[]):AuthoredScene[]{const out:AuthoredScene[]=[]; for(const scene of raw){const t=clean(scene.text); if(!t||blocked(t))continue; for(const p of t.split(/(?<=[.!?])\s+(?=[A-Z0-9'"“])/).map(clean).filter(Boolean)){const words=p.split(/\s+/); out.push({...scene,text:words.length<=18?p:words.slice(0,18).join(" ").replace(/[,:;—-]+$/g,"")});}} return out.slice(0,20).map((s,i,all)=>({...s,kind:s.kind||["hook","movement","discovery","turn","payoff"][Math.min(i,4)],durationHintMs:s.durationHintMs??Math.max(1500,Math.min(5000,1100+s.text.split(/\s+/).length*150)),transitionHint:s.transitionHint??(i===0?"none":i===all.length-1?"cinematic":"fade")}));}
