...
- A standalone timestamp must never consume an attention cut.
- Place/geo supplied separately is rendering/context enrichment, not a mandatory scene.
- This graph serves the current sequence-text artifact only; do not introduce a separate production abstraction.
*/
import type {
...
function fragments(values: readonly string[]): string[] {
  ...
  for (const value of values) {
    ...
    if (!source) continue;
    const parts = source.includes("\n") || source.includes("•") || source.includes("|") || source.includes(";") || source.includes("·") ? source.split(/[\n•|;·]+/g) : [source];
    for (const part of parts) { const text = clean(part.replace(/^[-*]\s*/, "")); if (text) out.push(text); }
  }
  return unique(out);
}
...
function explicitEntityNames(text: string, subject?: string): string[] { const names = text.match(/\b[A-Z][A-Za-z0-9'’-]{1,}\b/g) ?? []; return unique([...(subject && lower(text).includes(lower(subject)) ? [subject] : []), ...names]); }
function eventStructure(label: string, eventId: string): RealityEventStructure {
  ...
  const wordList = tokens(label);
  return { eventId, subjects: [], actions, objects: wordList.filter((word) => !actions.includes(word) && !states.includes(word)).slice(0, 8), states, temporalMarkers, sensoryMarkers: [], semanticTags: unique([...(actions.length ? ["action"] : []), ...(states.length ? ["state"] : []), ...(RECURRENCE.test(label) ? ["recurrence"] : []), ...(TIME.test(label) ? ["time"] : []), ...(TRANSITIONS.some(([a, b]) => lower(label).includes(a) || lower(label).includes(b)) ? ["transition"] : []), ...(isCompositionMetadata(label) ? ["presentation-metadata"] : [])]), recurrenceScore: RECURRENCE.test(label) ? .8 : 0, transitionScore: TRANSITIONS.some(([a, b]) => lower(label).includes(a) || lower(label).includes(b)) ? .75 : 0, anomalyScore: 0, salienceScore: Math.min(1, .35 + Math.min(.45, wordList.length / 30) + (RECURRENCE.test(label) ? .15 : 0)) };
}
function makeEvents(parts: string[], subject?: string, place?: string): { events: RealityEvent[]; structures: RealityEventStructure[]; evidence: RealityEvidence[] } {
  const evidenceList = parts.map((part, index) => evidence("fact", part, index)); const events: RealityEvent[] = []; const structures: RealityEventStructure[] = [];
  for (let index = 0; index < parts.length; index += 1) {
    const part = parts[index]!; if (isCompositionMetadata(part)) continue; const id = `event-${events.length + 1}`; const entities = explicitEntityNames(part, subject);
    const event = { id, label: part, sourceIds: [evidenceList[index]!.id], entities: unique(entities.length ? entities : tokens(part).slice(0, 8)), place: place || undefined, time: part.match(/\b(?:today|yesterday|tomorrow|morning|afternoon|evening|night|at\s+\d{1,2}:\d{2}\s*(?:am|pm)?|\d{1,2}:\d{2}\s*(?:am|pm)?)\b/i)?.[0] || undefined, salient: true, provenance: "explicit" as const } satisfies RealityEvent;
    events.push(event); structures.push(eventStructure(event.label, event.id));
  }
...
function buildRelations(events: RealityEvent[], structures: RealityEventStructure[]): RealityRelation[] {
  const relations: RealityRelation[] = []; const add = (from: string, to: string, kind: RealityRelation["kind"], strength: number) => { if (from === to || relations.some((item) => item.from === from && item.to === to && item.kind === kind)) return; relations.push({ from, to, kind, strength }); };
  for (let i = 0; i < events.length - 1; i += 1) {
    const left = events[i]!, right = events[i + 1]!, ll = lower(left.label), rr = lower(right.label);
    if (/\b(?:before|then|after|later|next)\b/.test(rr) || TIME.test(left.label) || TIME.test(right.label)) add(left.id, right.id, "before", .48);
    if (RECURRENCE.test(rr)) add(left.id, right.id, "repeats", .78);
    const lt = new Set(tokens(left.label)), rt = new Set(tokens(right.label)); const shared = [...lt].filter((token) => rt.has(token));
    if (shared.length >= 2) add(left.id, right.id, "recontextualizes", Math.min(.86, .5 + shared.length * .08));
    for (const [a, b] of TRANSITIONS) if (ll.includes(a) && rr.includes(b)) add(left.id, right.id, "changes", .88);
    if (/\b(?:because|caused|so that|therefore|which caused)\b/.test(rr)) add(left.id, right.id, "causes", .82);
  }
  for (let i = 0; i < events.length; i += 1) for (let j = i + 1; j < events.length; j += 1) { const a = events[i]!, b = events[j]!; const overlap = tokens(a.label).filter((token) => tokens(b.label).includes(token)).length; if (overlap >= 3) add(a.id, b.id, "repeats", Math.min(.92, .58 + overlap * .07)); }
  for (const structure of structures) { if (structure.transitionScore < .5) continue; const index = events.findIndex((event) => event.id === structure.eventId); if (index > 0) add(events[index - 1]!.id, structure.eventId, "changes", structure.transitionScore); }
  return relations.slice(0, 160);
}
function continuity(events: RealityEvent[]): RealityEntityContinuity[] {
  const map = new Map<string, string[]>(); for (const event of events) for (const entity of event.entities.slice(0, 8)) { const key = lower(entity); if (key.length < 3) continue; map.set(key, [...(map.get(key) ?? []), event.id]); }
  return [...map.entries()].filter(([, ids]) => ids.length >= 2).map(([name, ids]) => ({ name, mentionCount: ids.length, eventIds: unique(ids), firstEventId: ids[0]!, lastEventId: ids.at(-1)!, kind: "unknown", salienceScore: Math.min(1, .35 + ids.length * .12) }));
}
function patterns(events: RealityEvent[], relations: RealityRelation[]): RealityPattern[] {
  const out: RealityPattern[] = [];
  for (const relation of relations.filter((item) => item.kind === "repeats").slice(0, 12)) out.push({ kind: "recurrence", label: "supplied material recurs", eventIds: [relation.from, relation.to], evidenceIds: unique([...(events.find((event) => event.id === relation.from)?.sourceIds ?? []), ...(events.find((event) => event.id === relation.to)?.sourceIds ?? [])]), strength: relation.strength });
...
export function buildAuthorRealityGraph(input: { prompt: string; subject?: string; place?: string; facts: readonly string[]; sourceMoments?: readonly string[]; memoryContext?: readonly string[]; trajectory?: readonly string[] }): RealityGraph {
  const parts = fragments([...input.facts, ...(input.sourceMoments ?? [])]); const { events, structures, evidence } = makeEvents(parts, clean(input.subject), clean(input.place)); const relations = buildRelations(events, structures);
  const entityContinuity = continuity(events);
  const recurringSignals = unique([...relations.filter((item) => item.kind === "repeats").map(() => "recurrence is present"), ...(input.memoryContext ?? []).filter((item) => RECURRENCE.test(item)), ...(input.trajectory ?? []).filter((item) => RECURRENCE.test(item))]).slice(0, 24);
  ...
  return { evidence, events, relations, eventStructure: structures, entityContinuity, patterns: patterns(events, relations), unresolvedTensions, recurringSignals, sensorySignals: parts.filter((item) => /\b(?:blue|red|green|yellow|white|black|bacon|apple|water|rain|wind|music|sound|light|dark|smell|scent|cinnamon|sunset)\b/i.test(item)).slice(0, 24) };
}
