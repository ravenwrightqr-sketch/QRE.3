...
const SEMANTIC_TAGS: readonly [RegExp, string][] = [
  ...
  [
    /\b(?:return|returned|again|back|weekly|daily|every|remember(?:ed|s|ing)?|same)\b/i,
    "recurrence",
  ],
  ...
  [
    /\b(?:before|after|changed|new|different|fabulous|working|restored|renewed)\b/i,
    "transformation",
  ],
  ...
  [
    /\b(?:nervous|scared|uncertain|strange|weird|odd|quiet)\b/i,
    "uncertainty",
  ],
];
...
function meaningfulContentTokens(
  text: string,
  subject?: string,
): string[] {
...
function eventKind(
  ...
  if (ACTIONS.test(text) || TIME_WORDS.test(text)) {
    return "event";
  }

  if (STATE_WORDS.test(text)) {
    return "state";
  }
...
function extractActions(label: string): string[] {
  return unique(
    [...label.toLowerCase().matchAll(ACTION_RE)].map(
      (match) => match[0],
    ),
  ).slice(0, 8);
}
...
function extractObjects(
  ...
  if (
    ...
    return meaningfulContentTokens(
      label,
      subject,
    ).slice(0, 6);
  }

  const remainder = label.slice(
    firstAction.index +
      firstAction[0].length,
  );
...
function subjectParticipates(
  ...
  if (
    /^(?:the|a|an)\s+.+\s+(?:is|are|was|were|being|been)\s+\w/i.test(
      normalizedLabel,
    )
  ) {
    return false;
  ...
  if (
    /^(?:is|are|was|were|feels?|felt|seems?|seemed|became|became|looks?|looked)\b/i.test(
      normalizedLabel,
    )
  ) {
    return true;
...
function objectPhrases(
  ...
  const pattern =
    /\b(?:a|an|the|same)\s+([a-z][a-z0-9'’-]*(?:\s+[a-z][a-z0-9'’-]*){0,2})/gi;

  for (const match of label.matchAll(pattern)) {
    const words = clean(match[1])
      .split(/\s+/)
      .filter((word) => {
        const token =
          word.toLowerCase();
...
function event(
  ...
  return {
    ...
    entities,
    place:
      clean(place) || undefined,
    emotionalState:
      kind === "state"
        ? clean(label)
        : extractStates(label)[0],
    salient: true,
    provenance: "explicit",
  };
}
...
function buildTemporalRelations(
  events: RealityEvent[],
  relations: RealityRelation[],
): void {
  for (
...
function buildStructuralRelations(
  ...
  for (
    ...
    for (
      ...
      const sharedRecurrenceEvidence =
        currentSet.size > 0 &&
        otherTokens.some(
          (token) =>
...
function buildEventStructure(
  ...
  for (const event of events) {
    for (const token of meaningfulContentTokens(
      event.label,
      subject,
    )) {
      tokenCounts.set(
  ...
  return events.map(
    (event) => {
        ...
        Math.min(
          ...
          0.18 * states.length +
            0.2 *
              actions.filter(
                (action) =>
                  /repair|fix|clean|groom|change|finish|complete|restore|renew/i.test(
                    action,
                  ),
              ).length +
            (semantic.includes(
      ...
      return {
        ...
        recurrenceScore:
          Number(
            recurrenceScore.toFixed(
              3,
            ),
          ),
        transitionScore:
          Number(
            transitionScore.toFixed(
              3,
            ),
          ),
        anomalyScore:
...
function buildEntityContinuity(
  events: RealityEvent[],
  subject: string | undefined,
  structures: readonly RealityEventStructure[],
): RealityEntityContinuity[] {
  type ContinuityAccumulator = {
    eventIds: string[];
    kind: RealityEntityContinuity["kind"];
  };
  ...
  for (const event of events) {
    ...
    const subjectInEvent = Boolean(
      subject &&
        structure?.subjects.some(
          (value) =>
            lower(value) === lower(subject),
        ),
    );
    ...
    for (const name of names) {
      ...
      const item: ContinuityAccumulator =
        existing ?? {
          eventIds: [],
          kind: "unknown",
        };

      if (!item.eventIds.includes(event.id)) {
      ...
      if (
        /\b(?:dog|cat|pet|puppy|horse|bird)\b/.test(
          normalized,
        )
      ) {
        item.kind = "animal";
      } else if (
        /\b(?:watch|table|car|keys?|room|photo|mirror|bow|gift)\b/.test(
          normalized,
        )
      ) {
        item.kind = "object";
  ...
  return [...map.entries()]
    .map(([name, value]) => {
      const scores = value.eventIds.map(
        (eventId) =>
          structures.find(
            (item) =>
      ...
      return {
        ...
        salienceScore: Number(
          Math.min(
            1,
            0.25 +
              value.eventIds.length * 0.15 +
              Math.max(...scores, 0) * 0.35,
          ).toFixed(3),
        ),
      };
    })
...
function buildPatterns(
  ...
  for (const signal of recurringSignals
    .slice(0, 8)) {
    const normalized =
      lower(signal);
  ...
  for (const structure of structures
    ...
    patterns.push({
      kind: "anomaly",
      label: `high-information supplied detail: ${label}`,
      eventIds: [
        structure.eventId,
      ],
      evidenceIds:
...
function deriveTensions(
  ...
  if (
    ...
    tensions.push(
      "recurrence can change the meaning of an earlier detail",
    );
  }

  if (
    /(?:happy|proud|confident|excited)/.test(
      lowerSource,
    ) &&
    /(?:sad|angry|scared|nervous|tired)/.test(
      lowerSource,
    )
  ) {
    tensions.push(
  ...
  if (
    /\b(?:old|vintage|inherited)\b/.test(
      lowerSource,
    ) &&
    /\b(?:new|first|brand new)\b/.test(
      lowerSource,
    )
  ) {
    tensions.push(
      "old meaning meets new context",
    );
  }
  ...
  if (
    ...
    tensions.push(
      "one observation contains multiple salient details that can be reframed together",
    );
  }
  ...
  if (
    ...
    tensions.push(
      "supplied states contain a concrete contrast",
    );
  }
  ...
  if (
    ...
    tensions.push(
      "a supplied return or recurrence can change the reading of an earlier detail",
    );
  }
  ...
  if (
    ...
    tensions.push(
      "an observed state is linked to an observed action",
    );
  }
...
function deriveRecurringSignals(
  fragments: string[],
  memory:
    | readonly string[]
    | undefined,
...
function deriveSensorySignals(
  fragments: string[],
): string[] {
  const sensory =
    /\b(?:smell|scent|noise|sound|music|light|dark|bright|cold|hot|wet|dry|taste|sweet|salty|rough|soft|blue|red|green|yellow|white|black|lavender|bacon|apple|water|rain|wind|mirror|reflection)\b/i;
