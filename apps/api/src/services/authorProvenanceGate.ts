...
function hasForbiddenKind(line: string, facts: GateFact[], pattern: RegExp, expansion: AuthorRealityProvenance["forbiddenExpansions"][number]): boolean {
  if (!pattern.test(line)) return false;
  const supporting = facts.some((fact) => overlap(line, fact.text) >= 0.7 && !provenanceForbids(fact.provenance, expansion));
  return !supporting;
}
...
export function validateAuthorProvenance(lines: string[], facts: GateFact[]): ProvenanceViolation[] {
  ...
  lines.forEach((line, index) => {
    if (hasForbiddenKind(line, facts, PLACE, "invent_place")) {
      violations.push({ line: index + 1, reason: "unsupported_place", detail: "line introduces a place not authorized by its supporting facts" });
    }
    if (hasForbiddenKind(line, facts, OBJECT, "invent_object")) {
      violations.push({ line: index + 1, reason: "unsupported_object", detail: "line introduces an object not authorized by its supporting facts" });
    }
    if (hasForbiddenKind(line, facts, PERSON, "invent_person")) {
      violations.push({ line: index + 1, reason: "unsupported_person", detail: "line introduces a person not authorized by its supporting facts" });
    }
    if (hasForbiddenKind(line, facts, BODY, "invent_body_detail")) {
      violations.push({ line: index + 1, reason: "unsupported_body_detail", detail: "line introduces a body detail not authorized by its supporting facts" });
    }

    if (PRIVATE.test(line) && facts.every((fact) => provenanceForbids(fact.provenance, "invent_private_fact"))) {
      violations.push({ line: index + 1, reason: "unsupported_private_fact", detail: "line asserts private/internal information not observed in the source" });
    }
    ...
    if (matchedIndex >= 0) {
      ...
      if (CHRONOLOGY.test(line) && !provenance.permissions.includes("reorder")) {
        if (matchedIndex < lastFactIndex) {
          violations.push({ line: index + 1, reason: "unsupported_chronology", detail: "line changes the supplied event order" });
        }
      }
