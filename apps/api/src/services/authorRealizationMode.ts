...
const OCCURRENCE = /\b(?:arrived|entered|met|talked|spoke|said|left|came|went|found|lost|cleaned|finished|started|opened|closed|walked|ran|drove|ate|drank|kissed|married|celebrated|played|worked|visited|bought|sold|built|fixed|painted|wore|used|shook|chewed|connected|stayed|waited|called|laughed|cried|looked|felt|saw|observed|rolled|booted|slapped|won|lost|kicked|packed|moved|traveled|travelled|returned|became|changed)\b/i;
...
export function classifyAuthorRealizationMode(input: {
  ...
  if (occurrenceCount >= 2) return "sequence-film";
  if (occurrenceCount >= 1 && timedCount >= 1) return "sequence-film";
  if (occurrenceCount >= 3 && items.length >= 3) return "sequence-film";
