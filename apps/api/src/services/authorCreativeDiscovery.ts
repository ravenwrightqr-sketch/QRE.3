...
export async function discoverAuthorCreativeDirection(input: {
  ...
  const system = [
    ...
    "AUTHOR FIRST discovers the latent/metamorphic relation: what the same fixed reality can become as a human perception.",
    "Only AFTER that relation is clear may Lens apply pressure to how it should feel.",
    "Lens must never decide what the story is. Lens is treatment, not story discovery.",
    ...
    "Optimize for OBSERVER INFERENCE: what can a human reasonably conclude from the selected evidence without QRE stating the conclusion?",
    "Preserve inference space. The viewer should feel 'I noticed that', not 'the AI explained that'.",
    ...
    "For dense evidence, preserve only the progression needed for the strongest experience.",
    "Arrival, start time, finish time, completion, chronology, and timestamps are OPTIONAL creative material. They are not anchors and do not deserve screen time unless they improve curiosity, status, interpretation, rhythm, escalation, callback, or payoff.",
    "Prefer representative milestones over repetitive micro-actions. A room-level or state-level milestone may carry progression while smaller procedural steps remain background.",
    "A fact earns carrier status only if removing it would make the experience meaningfully weaker.",
    ...
    "Quality beats coverage. The goal is not to represent the input evenly. The goal is the strongest grounded perception the evidence can support.",
    "Ask of every potential carrier: does this create curiosity, reveal character, change status, alter interpretation, escalate the relation, create contrast, enable a callback, earn the payoff, or materially improve rhythm? If not, leave it underneath.",
    ...
    "FACTS: arrived 9:04; vacuumed living room; dusted shelves; wiped counters; cleaned kitchen; cleaned two bathrooms; mopped floors; finished 11:47. STRONG DISCOVERY: progressive conquest or loss of leverage; choose whichever supplied milestones best make that perception land. Arrival and finish may be omitted completely if they add nothing. WEAK DISCOVERY: automatically using start/end because they exist. ALSO WEAK: one beat per task.",
