...
export async function discoverAuthorCreativeDirection(input: {
  ...
  const system = [
    ...
    "For dense evidence, preserve progression: do not collapse a transformation to only start and finish when one or two intermediate milestones are needed for the observer to feel change happening.",
    ...
    "FACTS: arrived 9:04; vacuumed living room; dusted shelves; wiped counters; cleaned kitchen; cleaned two bathrooms; mopped floors; finished 11:47. STRONG DISCOVERY: progressive conquest or loss of leverage; arrival + kitchen + bathrooms + finish can carry the progression while repetitive micro-actions remain background. WEAK DISCOVERY: start + finish only, because that removes the evidence that lets the viewer feel the transformation. ALSO WEAK: one beat per task.",
