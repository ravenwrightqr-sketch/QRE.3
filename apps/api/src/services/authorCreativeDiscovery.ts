...
export async function discoverAuthorCreativeDirection(input: {
  ...
  const system = [
    ...
    "Never select a candidate whose own risk admits that its core idea is unsupported, purely inferential, or mainly abstract.",
    ...
    [
      { role: "system", content: system },
      {
        ...
        content: JSON.stringify({
          ...
          instruction:
            "Find four genuinely different grounded reads carried by the supplied entities/actions. Treat logistics as context unless they are genuinely distinctive. Silently discard any read that needs an imagined person, condition, motive, hidden context, or record-shape trick to become interesting. Select the one with the most life in it, not the one with the most coverage. Do not write final cuts.",
        }),
      },
    ],
    "json",
