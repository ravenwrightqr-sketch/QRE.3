...
const INTERPRETIVE = /\b(?:lawyer|boss|ceo|diva|celebrity|negotiat(?:e|ion|or)|rebel|rebellion|defiant|defiance|evidence|case|trial|court|verdict|terms|deal|contract|royalty|queen|king|status|in charge|mission|operation|suspect|legend|undefeated|called the shots|peace|protest|under protest|guarded|attitude|upper hand|power|victory|victorious|mini[- ]?rebel|tiny rebel|not impressed|means business|nervous|fierce|fabulous)\b/i;
...
const TRANSITION = /\b(?:first|then|after|now|still|only|finally|so|until|back|again|already|yet|instead|apparently|once|before)\b/i;
...
function interpretationGrounding(input: AttentionBeatInput, text: string): number {
  const role = clean(input.attentionFunction ?? input.role);
  const labels = [...(input.setsUp ?? []), ...(input.paysOff ?? [])].filter(Boolean);
  const creative = clean(input.creativeMove);
  if (!INTERPRETIVE.test(text) || !labels.length) return 0;
  if (["reframe", "turn", "callback", "payoff", "release", "escalation"].includes(role)) return creative !== "none" ? 0.65 : 0.48;
  return creative !== "none" ? 0.48 : 0.3;
}

function sequenceCohesion(input: AttentionBeatInput, text: string, prior: string[]): number {
  if (!prior.length) return 0.5;
  let score = 0.12;
  const priorSet = set(prior.join(" "));
  const currentSet = set(text);
  const shared = overlap(currentSet, priorSet);
  if (shared > 0.08) score += 0.2;
  if (TRANSITION.test(text)) score += 0.2;
  if (INTERPRETIVE.test(text) && [...(input.setsUp ?? []), ...(input.paysOff ?? [])].length) score += 0.2;
  if (["turn", "reframe", "callback", "payoff", "release"].includes(clean(input.attentionFunction ?? input.role))) score += 0.12;
  if (LABEL_LIKE.test(text) || GENERIC.test(text)) score -= 0.2;
  if (shared > 0.82) score -= 0.1;
  return metric(score);
}
...
function nextBeatPull(
  ...
  if (TRANSITION.test(clean(nextText))) score += 0.08;
  ...
  score = score * 0.6 + plannerTarget * 0.2 + sequenceCohesion({ role, attentionFunction: role } as AttentionBeatInput, text, []) * 0.2;
  return metric(score);
}
...
export function scoreAttentionBeat(input: AttentionBeatInput, priorTexts: string[], evidence: string[]): AttentionBeatScore {
  ...
  const interpretation = interpretationGrounding(input, text);
  const factual = Math.max(factuality(text, evidence), interpretation * 0.5);
  const coverage = Math.max(sourceCoverage(input, text, evidence), interpretation * 0.8);
  ...
  const cohesion = sequenceCohesion(input, text, priorTexts);
  ...
  const count = words(text).length;
  const role = clean(input.attentionFunction ?? input.role);
  const isInterpretiveBeat = ["turn", "reframe", "callback", "payoff", "release", "escalation"].includes(role);

  if (!text) reasons.push("missing-text");
  if (count > 7) reasons.push("too-long");
  if (factual < 0.18 && !interpretation) reasons.push("weak-factual-anchor");
  if (specific < 0.34) reasons.push("weak-specificity");
  if (coverage < 0.12 && !interpretation) reasons.push("weak-source-coverage");
  if (creative < 0.4 && (input.creativeMove ?? "none") !== "none") reasons.push("weak-creative-move");
  if (pull < 0.3) reasons.push("weak-next-beat-pull");
  if (repeated > 0.78) reasons.push("repetitive");
  ...
  if (isInterpretiveBeat && interpretation < 0.35) reasons.push("weak-recontextualization-grounding");
  if (priorTexts.length > 0 && cohesion < 0.3 && role !== "hook") reasons.push("sequence-reset");
  if (isInterpretiveBeat && interpretation > 0 && TRANSITION.test(text)) reasons.push("cumulative-move-detected");

  // Informational diagnostic only: a real cumulative move is valuable evidence,
  // but it must not itself force acceptance.
  const keepReasons = reasons.filter((reason) => reason !== "cumulative-move-detected");
  ...
  return {
    ...
    score,
    keep: keepReasons.length === 0 && score >= 0.56,
    reasons: keepReasons,
  };
}
