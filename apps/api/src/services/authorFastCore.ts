...
export async function authorFast(input:Input):Promise<{plan:Plan;scenes:Scene[]}>{
 ...
 const planResult=await localModelGenerate([{role:"system",content:[
  ...
  "CORE PRODUCT RULE: when the experience comes from a service, the service is the ENABLING STAGE, not automatically a story character. Do not invent or foreground the groomer, cleaner, technician, owner, customer, employee, or provider unless the source explicitly makes them part of the story.",
  "TEMPORARY STAR RULE: the supplied subject/person/pet gets the spotlight during the experience. Make the subject feel like the star of a little movie for the duration of the service. The business benefits because the memory is entertaining and shareable; the service does not need to become the protagonist.",
  "The character/subject is the center of gravity. The input is the world they experience. Make personality, contradiction, attitude, relationship to the input, choice, consequence, or recurring history the creative engine.",
  ...
  "A boring job can become entertaining through the subject's real perspective, attitude, rhythm, relationship, contrast, or meaning. Never invent a provider interaction merely to create drama.",
 ...
 const fallback:Plan={angle:"character-specific contradiction",tension:"the character meets the recurring situation on different terms",movement:"hook → pressure → character turn → consequence",payoff:"the character gets the last word",antiRepeat:"generic transformation language, mechanical name repetition, unsupported events, provider-as-protagonist",beatCount:input.prompt.toLowerCase().includes("living memory")||input.prompt.toLowerCase().includes("chapter")?4:5,beatJobs:[
  {job:"establish the charged subject situation",attention:"create immediate curiosity",grounding:"use only supplied reality; service provider stays background unless sourced",payoffLink:"plant final character consequence"},
  {job:"sharpen the subject's stance",attention:"make next cut necessary",grounding:"no invented provider action",payoffLink:"increase pressure"},
  {job:"change the terms through the subject's perspective",attention:"surprise or reframe",grounding:"transform supplied detail without fabricating events",payoffLink:"set up payoff"},
  {job:"land a subject-specific payoff",attention:"make ending satisfying",grounding:"earned from supplied world",payoffLink:"final consequence"},
 ]};
 ...
 const draftResult=await localModelGenerate([{role:"system",content:[
  ...
  "SERVICE STORY MODE: the service is the invisible stage manager. Do not make the provider the protagonist unless the supplied evidence explicitly does. Temporarily make the subject the star and let the service create the circumstances/opportunity for that subject to be memorable.",
  "Think: SERVICE → SUBJECT SPOTLIGHT → ENTERTAINING MEMORY → SHAREABLE PAYOFF. The business can remain almost invisible inside the story.",
  ...
  "A good line can transform a supplied detail through the character's lens: fact → attitude, object → threat, routine → game, place → memory marker, ordinary service → personality. Do not invent a concrete event while doing this.",
  "CHARACTER GRAVITY WITHOUT NAME ABUSE: the subject should dominate through attitude, resistance, choices, consequences, and history. Do not mechanically repeat the subject name.",
