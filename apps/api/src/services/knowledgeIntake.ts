...
function locationSignature(location?: Location): string {
  if (!location) return "";
  return [location.section, location.shelf, location.row, location.position, location.bbox?.join(",")].filter(Boolean).join(" / ");
}
...
function storedLocation(value: unknown): Location | undefined {
  ...
  const bbox = Array.isArray(raw.bbox) && raw.bbox.length === 4 && raw.bbox.every((item) => typeof item === "number")
  ...
  return {
    ...
    row: typeof raw.row === "string" ? raw.row : undefined,
...
async function setStage(jobId: string, stage: IntakeStage): Promise<void> {
...
async function ensureMedia(input: StoredPayload, jobId: string, contentHash: string): Promise<{ input: StoredPayload; bytes?: Buffer; mimeType?: string }> {
  if (input.storageKey) {
    return { input, bytes: await readKnowledgeMedia(input.storageKey), mimeType: input.mimeType };
  }

  const dataUrl = input.imageDataUrl || (input.content?.startsWith("data:") ? input.content : undefined);
  if (!dataUrl) return { input };
  const decoded = decodeDataUrl(dataUrl);
  if (!decoded) throw new Error("Uploaded content is not a valid data URL.");

  const extension = decoded.mimeType === "application/pdf"
    ? "pdf"
    : decoded.mimeType.includes("spreadsheet") || decoded.mimeType.includes("excel")
      ? "bin"
      : decoded.mimeType.startsWith("image/")
        ? decoded.mimeType.split("/", 2)[1].replace(/[^a-z0-9]/gi, "") || "bin"
        : "bin";
...
async function learnWebsite(input: StoredPayload, evidenceId: string) {
  ...
  const templateData = JSON.parse(JSON.stringify({
    ...
    websiteKnowledge: {
      sourceUrl: learned.url, sourceTitle: learned.title, businessName: world.businessName, businessType: world.businessType,
      description: world.description, services: world.services, differentiators: world.differentiators, signals: world.signals,
      subjectKinds: world.subjectKinds, importantFacts: world.importantFacts, learnedAt: new Date().toISOString(),
    },
  }));
...
async function persistPattern(assetId: string, catalogItemId: string, type: string, statement: string, evidenceId: string, confidence: number, strength: number): Promise<void> {
  const existing = await db.knowledgePattern.findFirst({ where: { assetId, catalogItemId, type }, orderBy: { updatedAt: "desc" } });
  const now = new Date();
  if (existing) {
    const existingEvidenceIds = Array.isArray(existing.evidenceIds) ? existing.evidenceIds.filter((value): value is string => typeof value === "string") : [];
    await db.knowledgePattern.update({ where: { id: existing.id }, data: { statement, confidence, strength, lastObservedAt: now, evidenceIds: [...new Set([...existingEvidenceIds, evidenceId])].slice(-32) } });
  } else {
    await db.knowledgePattern.create({ data: { assetId, catalogItemId, type, statement, confidence, strength, evidenceIds: [evidenceId], firstObservedAt: now, lastObservedAt: now } });
  }
}
...
async function maybeLearnTemporalPattern(assetId: string, catalogItemId: string, itemName: string, evidenceId: string, currentValue: unknown): Promise<void> {
  const previous = await db.knowledgeObservation.findFirst({
    where: { assetId, catalogItemId, type: "VISUAL_OBSERVATION", evidenceId: { not: evidenceId } },
    orderBy: { observedAt: "desc" },
    select: { value: true, observedAt: true },
  });
  if (!previous) return;
  const before = locationSignature(storedLocation(previous.value));
  const after = locationSignature(storedLocation(currentValue));
  if (!before || !after || before === after) return;
  const statement = `${itemName} changed position from ${before} to ${after}.`;
  await persistPattern(assetId, catalogItemId, "POSITION_CHANGE", statement, evidenceId, 0.86, 0.78);
}
...
async function persistFacts(input: StoredPayload, evidenceId: string, facts: Fact[]) {
  ...
  for (const fact of facts) {
    ...
    await db.catalogAttribute.create({ data: { catalogItemId: item.id, key: "value", value: fact.value, unit: fact.unit || undefined, normalizedValue: normalizeName(fact.value), confidence: clampConfidence(fact.confidence), evidenceId } });
    ...
    if (repeated >= 2) await persistPattern(input.assetId, item.id, "REPEATED_OBSERVATION", `${item.name} has been observed repeatedly (${repeated} observations).`, evidenceId, Math.min(0.99, 0.55 + Math.log10(repeated + 1) * 0.45), Math.min(0.99, repeated / (repeated + 2)));
  }
...
async function persistVision(input: StoredPayload, evidenceId: string, items: CatalogVisionItem[]) {
  ...
  for (const item of items) {
    ...
    const metadata = {
      ...
      productIdentity: [item.brand, item.product || item.name, item.variant].filter(Boolean).map(normalizeName).filter(Boolean).join("|"),
      evidence: item.evidence || [],
      location: item.location || undefined,
    };
    const existing = await db.catalogItem.findFirst({ where: { assetId: input.assetId, normalizedName } });
    const catalogItem = existing
      ? await db.catalogItem.update({ where: { id: existing.id }, data: { brand: item.brand || existing.brand || undefined, category: item.category || existing.category || undefined, description: existing.description || item.notes || undefined, metadata: { ...((existing.metadata && typeof existing.metadata === "object" && !Array.isArray(existing.metadata)) ? existing.metadata as Record<string, unknown> : {}), lastRecognition: metadata } } })
      : await db.catalogItem.create({ data: { assetId: input.assetId, kind: item.category || "visual_item", name, normalizedName, brand: item.brand || undefined, category: item.category || undefined, description: item.notes || undefined, metadata } });
    ...
    const observationValue = { name, brand: item.brand, product: item.product, variant: item.variant, category: item.category, attributes, evidence: item.evidence || [], location: item.location, recognitionObservationId: item.observationId, recognitionState: item.state, notes: item.notes };
    ...
    observationIds.push(observation.id);
    await maybeLearnTemporalPattern(input.assetId, catalogItem.id, catalogItem.name, evidenceId, observationValue);
    const repeated = await db.knowledgeObservation.count({ where: { assetId: input.assetId, catalogItemId: catalogItem.id } });
    if (repeated >= 2) await persistPattern(input.assetId, catalogItem.id, "REPEATED_OBSERVATION", `${catalogItem.name} has been observed repeatedly (${repeated} observations).`, evidenceId, Math.min(0.99, 0.55 + Math.log10(repeated + 1) * 0.45), Math.min(0.99, repeated / (repeated + 2)));
  }
  return { catalogIds, observationIds, factCount: items.length };
}

function compactPayload(input: StoredPayload): Record<string, unknown> {
  return { mimeType: input.mimeType, text: input.text, userId: input.userId, storageKey: input.storageKey };
}
...
async function processIntake(jobId: string, originalInput: StoredPayload): Promise<void> {
  ...
  try {
    ...
    const raw = originalInput.imageDataUrl || originalInput.text || originalInput.content || "";
    const contentHash = raw ? sha256(raw) : undefined;
    const media = await ensureMedia(originalInput, jobId, contentHash || jobId);
    const input = media.input;
    const binary = media.bytes;
    const mimeType = media.mimeType || input.mimeType;
    const evidence = await db.knowledgeEvidence.create({
      data: {
        assetId: input.assetId,
        intakeJobId: jobId,
        type: mimeType || input.sourceType,
        source: input.sourceType,
        storageKey: input.storageKey || `intake-job:${jobId}`,
        contentHash: binary ? sha256Bytes(binary) : contentHash,
        text: binary && mimeType?.startsWith("text/") ? binary.toString("utf8").trim() || undefined : textFromInput(input),
        metadata: { originalName: input.originalName, mimeType, userId: input.userId, storage: input.storageKey ? "knowledge_media" : "legacy_job_payload", byteLength: binary?.length },
        confidence: 1,
      },
    });
    ...
    if (input.sourceType === "website") {
      ...
      const imageDataUrl = binary ? mediaDataUrl(binary, mimeType || "image/jpeg") : input.imageDataUrl;
      if (!imageDataUrl) throw new Error("Image media is missing from intake storage.");
      result = await persistVision(input, evidence.id, await analyzeImageForCatalog(imageDataUrl, input.assetId));
      await setStage(jobId, "persisting");
    } else if (input.sourceType === "pdf") {
      await setStage(jobId, "extracting");
      const bytes = binary || decodeDataUrl(input.content || "")?.bytes;
      if (!bytes) throw new Error("PDF upload is not available in durable storage.");
      const extracted = await extractPdfKnowledge(bytes);
      await db.knowledgeEvidence.update({ where: { id: evidence.id }, data: { text: extracted.text, metadata: { ...extracted.metadata, originalName: input.originalName, mimeType } } });
      await setStage(jobId, "persisting");
      result = await persistFacts(input, evidence.id, extracted.facts);
    } else if (input.sourceType === "spreadsheet") {
      await setStage(jobId, "extracting");
      const bytes = binary || decodeDataUrl(input.content || "")?.bytes;
      if (!bytes) throw new Error("Spreadsheet upload is not available in durable storage.");
      const extracted = extractSpreadsheetKnowledge(bytes, input.originalName);
      await db.knowledgeEvidence.update({ where: { id: evidence.id }, data: { text: extracted.text, metadata: { ...extracted.metadata, originalName: input.originalName, mimeType } } });
      await setStage(jobId, "persisting");
      result = await persistFacts(input, evidence.id, extracted.facts);
    } else {
      ...
      await setStage(jobId, "persisting");
      result = text ? await persistFacts(input, evidence.id, simpleFactsFromText(text)) : { catalogIds: [], observationIds: [], factCount: 0 };
    }
    ...
    try { await db.knowledgeIntakeJob.update({ where: { id: jobId }, data: { status: "failed", result: { stage: "failed", evidenceId }, error: message, completedAt: new Date() } }); }
    catch (updateError) { console.error("[KnowledgeIntake] could not record failure", jobId, updateError); }
  }
}
...
async function workerTick(): Promise<void> {
  ...
  workerBusy = true;
  try { await requeueStaleJobs(); const job = await claimQueuedJob(); if (job) await processIntake(job.id, parseStoredPayload(job)); }
  catch (error) { console.error("[KnowledgeIntake] worker tick failed", error); }
...
export function startKnowledgeIntakeWorker(): void {
  ...
  const loop = async () => { await workerTick(); setTimeout(loop, WORKER_POLL_MS); };
  void loop();
}
...
export async function enqueueKnowledgeIntake(input: IntakeInput) {
  ...
  if (input.imageDataUrl || input.content?.startsWith("data:")) {
    const dataUrl = input.imageDataUrl || input.content || "";
    const decoded = decodeDataUrl(dataUrl);
    if (!decoded) throw new Error("Uploaded content is not a valid data URL.");
    ...
    storageKey = `intake/${input.assetId}/${contentHash || sha256Bytes(decoded.bytes)}.${extension}`;
    await writeKnowledgeMedia(storageKey, decoded.bytes);
  }
  ...
  const job = await db.knowledgeIntakeJob.create({
    data: {
      ...
      contentHash,
      payload: compactPayload({ ...input, storageKey }),
    },
  });
