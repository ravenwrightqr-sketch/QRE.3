...
type StoredPayload = { assetId: string; userId: string; sourceType: string; originalName?: string; mimeType?: string; content?: string; imageDataUrl?: string; text?: string };
...
async function setStage(jobId: string, stage: IntakeStage, error?: string) {
...
async function learnWebsite(input: StoredPayload, evidenceId: string) {
  ...
  const payload = {
    ...
    category: "business_world", source: "website", sourceUrl: learned.url, sourceTitle: learned.title || undefined,
    confidence: 0.9, businessName: world.businessName || undefined, businessType: world.businessType || undefined,
    businessDescription: world.description || undefined, services: world.services, differentiators: world.differentiators,
    signals: world.signals, subjectKinds: world.subjectKinds, importantFacts: world.importantFacts,
    sourceExcerpt: learned.sourceExcerpt, evidenceId, updatedBy: input.userId,
  };
  ...
  const templateData = JSON.parse(JSON.stringify({
    ...
    contextualSignals: [...new Set([...(Array.isArray(currentData.contextualSignals) ? currentData.contextualSignals.filter((value): value is string => typeof value === "string") : []), ...world.signals, ...world.differentiators])].slice(0, 48),
    subjectKinds: world.subjectKinds.length ? world.subjectKinds : currentData.subjectKinds || [],
    websiteKnowledge: { sourceUrl: learned.url, sourceTitle: learned.title, businessName: world.businessName, businessType: world.businessType, description: world.description, services: world.services, differentiators: world.differentiators, signals: world.signals, subjectKinds: world.subjectKinds, importantFacts: world.importantFacts, learnedAt: new Date().toISOString() },
  }));
  await db.asset.update({ where: { id: input.assetId }, data: { templateData } });
  return { row, factCount: 1, catalogIds: [], observationIds: [] };
}

async function persistFacts(input: StoredPayload, evidenceId: string, facts: Array<{ label: string; value: string; category?: string; unit?: string; notes?: string; confidence?: number; spatial?: unknown; quantity?: number }>) {
  ...
  for (const fact of facts) {
    ...
    const item = existing
      ? await db.catalogItem.update({ where: { id: existing.id }, data: { category: fact.category || existing.category || undefined, description: existing.description || fact.notes || undefined, metadata: fact.spatial || fact.quantity != null ? { ...(existing.metadata && typeof existing.metadata === "object" && !Array.isArray(existing.metadata) ? existing.metadata as Record<string, unknown> : {}), lastSpatial: fact.spatial, lastQuantity: fact.quantity } : undefined } })
      : await db.catalogItem.create({ data: { assetId: input.assetId, kind: fact.category || "item", name: fact.label || fact.value, normalizedName, category: fact.category || undefined, description: fact.notes || undefined, metadata: fact.spatial || fact.quantity != null ? { spatial: fact.spatial, quantity: fact.quantity } : undefined } });
    ...
    if (fact.spatial != null) await db.catalogAttribute.create({ data: { catalogItemId: item.id, key: "spatial", value: JSON.stringify(fact.spatial), confidence: clampConfidence(fact.confidence), evidenceId } });
    if (fact.quantity != null) await db.catalogAttribute.create({ data: { catalogItemId: item.id, key: "quantity", value: String(fact.quantity), unit: "count", confidence: clampConfidence(fact.confidence), evidenceId } });
    const observation = await db.knowledgeObservation.create({ data: { assetId: input.assetId, catalogItemId: item.id, evidenceId, type: "OBSERVED", value: { label: fact.label, value: fact.value, category: fact.category, unit: fact.unit, notes: fact.notes, spatial: fact.spatial, quantity: fact.quantity }, source: input.sourceType, confidence: clampConfidence(fact.confidence), observedAt: new Date() } });
    ...
    if (repeated >= 2) {
      ...
      if (existingPattern) await db.knowledgePattern.update({ where: { id: existingPattern.id }, data: { statement, confidence, strength, lastObservedAt: new Date() } });
      else await db.knowledgePattern.create({ data: { assetId: input.assetId, catalogItemId: item.id, type: "REPEATED_OBSERVATION", statement, confidence, strength, evidenceIds: [evidenceId], firstObservedAt: new Date(), lastObservedAt: new Date() } });
    }
  }
...
async function processIntake(jobId: string, input: StoredPayload): Promise<void> {
  try {
    ...
    if (input.sourceType === "website") {
      ...
      await setStage(jobId, "persisting");
      result = text ? await persistFacts(input, evidence.id, simpleFactsFromText(text)) : { factCount: 0, catalogIds: [], observationIds: [] };
    }
    ...
    try {
      await db.knowledgeIntakeJob.update({ where: { id: jobId }, data: { status: "failed", result: { stage: "failed" }, error: message, completedAt: new Date() } });
...
async function claimQueuedJob() {
  ...
  const claimed = await db.knowledgeIntakeJob.updateMany({ where: { id: candidate.id, status: "queued" }, data: { status: "processing", startedAt: new Date(), result: { stage: "evidence" } } });
