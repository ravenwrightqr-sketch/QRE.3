...
function relationCoverage(graph: RealityGraph, proposition: AuthorCreativeProposition, text: string): number {
  const relations = graph.relations.filter((relation) =>
    proposition.relationIds.length === 0 || proposition.relationIds.includes(relation.from) || proposition.relationIds.includes(relation.to),
  );
  const relationWords = [
...
function treatmentCoverage(proposition: AuthorCreativeProposition, text: string): number {
  const signals: Record<string, string> = {
    "horror-romance": "danger vulnerability intimacy normalization relationship",
    "heist-comedy": "objective dependency clever operation consequence asymmetry",
    "game-fierce": "priority competition repeated behavior escalation constraint contest",
    "noir-tenderness": "ambiguity memory relationship withheld identity tenderness",
    "documentary-chaos": "system contradiction accumulation anomaly precision consequence",
  };
...
function informationPerCut(graph: RealityGraph, proposition: AuthorCreativeProposition, sequence: SequencePlay): number {
  if (!sequence.cuts.length) return 0;
  let score = 0;
  for (let index = 0; index < sequence.cuts.length; index += 1) {
    ...
    const relation = relationCoverage(graph, proposition, cut.text);
...
export function judgeAuthorSequence(input: {
  ...
  const sourceSpecificity = sequence.cuts.length
    ? sequence.cuts.reduce((sum, cut) => sum + sourceCoverage(graph, cut.text), 0) / sequence.cuts.length
    : 0;
  const relationFidelity = sequence.cuts.length
    ? sequence.cuts.reduce((sum, cut) => sum + relationCoverage(graph, proposition, cut.text), 0) / sequence.cuts.length
    : 0;
  const treatmentFidelity = sequence.cuts.length
    ? sequence.cuts.reduce((sum, cut) => sum + treatmentCoverage(proposition, cut.text), 0) / sequence.cuts.length
    : 0;
  const information = informationPerCut(graph, proposition, sequence);
  ...
  const propositionFidelity = sequence.cuts.length
    ? sequence.cuts.reduce((sum, cut) => sum + overlap(cut.text, propositionTerms), 0) / sequence.cuts.length
    : 0;
  ...
  const inventionRisk = Math.max(
    ...
    Math.min(
      1,
      1 - grounding * 0.55 - specificity * 0.2 - proposition.sourceEventIds.every((id) => eventIds.has(id)) ? 0.1 : 0,
    ),
  );
  const genericity = Math.max(0, Math.min(1, 1 - (sourceSpecificity * 0.45 + relationFidelity * 0.3 + treatmentFidelity * 0.25)));
  ...
  if (proposition.sourceEventIds.some((id) => !eventIds.has(id))) reasons.push("unsupported-proposition-source");
