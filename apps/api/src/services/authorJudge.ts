...
export function judgeAuthorSequence(input:{graph:RealityGraph;proposition:AuthorCreativeProposition;sequence:SequencePlay}):AuthorJudgeResult {
  ...
  const propositionFidelity=input.sequence.cuts.length?input.sequence.cuts.filter(c=>clean(c.attentionDelta).length>0).length/cuts.length:0;
  let moving=0;
  for(let i=1;i<cuts.length;i++) if(clean(cuts[i].text)!==clean(cuts[i-1].text)&&cuts[i].informationGain!==cuts[i-1].informationGain) moving++;
  ...
  if(!input.proposition.text) reasons.push("missing proposition");
  ...
  return {status:reasons.length?"REJECT":"ACCEPT",grounding,specificity:Math.min(1,grounded*0.6+specificity*0.4),movement,propositionFidelity,transformation,inventionRisk,genericity,reasons};
}
