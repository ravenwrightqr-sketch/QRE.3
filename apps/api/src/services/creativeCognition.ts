...
function contradictionPairs(input: CreativeCognitionInput): string[] {
  ...
  const pairs: string[] = [];
  const add = (pattern: RegExp, value: string) => { if (pattern.test(text)) pairs.push(value); };
  ...
  add(/new|brand new|pristine/, "newness versus the uncertainty of what comes next");
  add(/expensive|million|luxury|yacht/, "extreme value versus the human detail that makes it matter");
  add(/night|9 pm|10 pm|11 pm/, "darkness or night atmosphere versus intimacy, calm, or expectation");
...
function inferAttentionCandidates(input: CreativeCognitionInput): string[] {
  ...
  candidates.push("reframing a familiar thing so the viewer sees it differently");
...
function operatorMix(selected: string, round: number): string[] {
  const base = selected.includes("character comedy")
    ? ["sensory_hook", "personification", "status_inversion", "comic_turn", "callback", "afterglow"]
    : selected.includes("reality quietly")
      ? ["understatement", "contrast", "environmental_violation", "escalation", "calm_reaction", "reframe"]
      : selected.includes("provenance") || selected.includes("wear or age")
        ? ["sensory_hook", "zoom_into_detail", "provenance", "callback", "reframe", "afterglow"]
        : selected.includes("private detail")
          ? ["sensory_hook", "zoom_into_detail", "understatement", "callback", "tender_turn", "afterglow"]
          : selected.includes("ordinary service")
            ...
            : selected.includes("tiny physical")
              ? ["sensory_hook", "touch", "scale_contrast", "mystery_turn", "reveal", "afterglow"]
              : ["pattern_break", "sensory_hook", "contrast", "micro_reveal", "reversal", "payoff"];

  if (round > 1) {
    return [...base, "chapter_callback", "meaning_shift"].slice(0, 9);
  }
  return base;
}
...
export function buildCreativeCognition(input: CreativeCognitionInput): CreativeCognitionPlan {
  ...
  const callbacks = round > 1
    ? uniq([
        ...
        ...(input.previousScenes ?? []).slice(-4).map((x) => `revisit prior image without repeating its wording: ${x}`),
      ], 8)
    : [];
  ...
  const sceneDiscipline = [
    "One scene = one viewer-facing thought = one perceivable moment.",
    "Prefer 4–14 words per scene; 15–18 only when necessary.",
    "Never include labels such as 'hook', 'micro-reveal', 'status inversion', or 'afterglow' in scene text.",
    ...
    "Invent metaphor, attitude, framing, and implication instead of unsupported factual events.",
  ];
  ...
  const learning = [
    ...
    contradictions.length ? `CONTRADICTIONS: ${contradictions.join(" | ")}` : "CONTRADICTIONS: none explicit; find a subtle tension without inventing facts",
    `OPERATOR MIX: ${operators.join(", ")}`,
    callbacks.length ? `CALLBACK OPPORTUNITIES: ${callbacks.join(" | ")}` : "CALLBACK OPPORTUNITIES: establish a detail worth remembering",
    ...antiRepetition.map((x) => `ANTI-REPETITION: ${x}`),
    ...sceneDiscipline.map((x) => `SCENE DISCIPLINE: ${x}`),
    `QUALITY GOAL: create an angle that is specific to this world and difficult to predict from the prompt alone`,
