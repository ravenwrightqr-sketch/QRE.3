...
const cases = [
  ...
  },
  {
    ...
    expected: [
      ...
      "played the same song every Sunday",
      "is gone",
    ],
  },
];
...
for (const testCase of cases) {
  ...
  assert.ok(readout.lines.every((line) => !new RegExp(`^${testCase.subject.split(/\s*(?:\+|&|,|\\/)\s*/).filter(Boolean).join("| ")}\\b`, "i").test(line)), "Readout repeats the subject inside event lines");
}
