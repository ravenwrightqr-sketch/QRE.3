...
const ARTIST_CHARTER = [
  ...
  "A tiny film may be 2 cuts. A richer reality may need more. Length is earned by the creative idea.",
...
for(const test of cases){
  ...
  console.log(`Accepted semantic candidates: ${result.diagnostics.acceptedCandidates}`);
  console.log(`Movie score: ${result.diagnostics.experienceJudge?.score??0}`);
  console.log(`Artist diagnostic score: ${filmJudge?.score??0}`);
  ...
  console.log(`Artist diagnostic: ${filmJudge?.accepted?"PASS":"FAIL"} (NON-BLOCKING)`);
  ...
  // The lab protects wiring, provenance and obvious architecture leakage.
  // It does NOT grade taste, literary quality, metaphor, humor, weirdness, or form.
