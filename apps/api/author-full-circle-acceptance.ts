...
const media = [
  {
    id: "before",
    type: "image" as const,
    url: "https://example.invalid/coco-before.jpg",
    role: "evidence" as const,
    observedAt: "2026-08-21T17:00:00Z",
    metadata: { stage: "before", label: "before" },
  },
  {
    id: "moment",
    type: "image" as const,
    url: "https://example.invalid/coco-moment.jpg",
    role: "photo_beat" as const,
    observedAt: "2026-08-21T17:20:00Z",
    metadata: { stage: "moment", label: "blue bow" },
  },
  {
    id: "after",
    type: "image" as const,
    url: "https://example.invalid/coco-after.jpg",
    role: "evidence" as const,
    observedAt: "2026-08-21T18:00:00Z",
    metadata: { stage: "after", label: "after" },
  },
];
...
const context: CognitiveAuthorContext = {
  identity: {
    ...
    sourceEventCount: 2,
    confidence: 0.95,
    generatedAt: "2026-08-21T18:01:00Z",
  },
  geo: {
    role: "experience_place",
    label: "Coco Grooming",
    city: "Riverside",
    region: "CA",
    country: "US",
  ...
  presence: {
    isReturning: true,
    visitNumber: 3,
    summary: ["returning visit"],
    places: ["Coco Grooming"],
  },
  analytics: {
    scans: 3,
    completions: 3,
  ...
  domain: {
    mode: "pet_social",
    signature: "poodle | nervous | loves treats",
    identitySignals: ["poodle"],
    traitSignals: ["nervous"],
    preferenceSignals: ["loves treats"],
    socialSignals: [],
    activitySignals: ["grooming"],
    continuitySignals: ["returns for grooming"],
    tensions: [],
    opportunity: { text: "stole a blue bow", kind: "event", strength: 0.92, sources: ["stole a blue bow"] },
  },
  provenance: [],
  media,
  authorizedCreativeInstructions: [],
  textBeatTarget: 5,
  photoBeatsSilent: true,
};
...
const businessRun = await authorMoviePipeline({
  ...input,
  cta: { text: "BOOK AGAIN" },
});
...
const manualRun = await authorMoviePipeline({
  ...input,
